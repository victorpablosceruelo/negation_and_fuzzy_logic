%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Package  CNEG_TYPES for including constructive in a program %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
cneg_aux(Goal,GoalVarsI,UnivVars):-
	...
	no_unifications(GoalVars,UnivVars), % There is no failure
...
% from the term G that comes from a metaterm and contains the module
% qualification
...
% simple_frontier(Front,Frontier) simplifies the frontier Front.
% It studies the ground subgoals of the clauses of the frontier. It
% eliminates the clauses with a ground subgoal equal to fail in the body
% and remove the subgoals that are equal to true in any body
simple_frontier([],[]).
...
% simple_body(B,B1) returns fail in B1 if any of the ground subgoals of
% B is equal to fail, otherwise B1 has the elements of B except the ground
% subgoals of B that are equal to true.
simple_body([],[]).
...
% conjunction(F1,F2,F3) returns F3 that is the conjunction of the list 
...
% detach_att_if_needed1(Var) detaches the attribute of Var if it has it.
detach_att_if_needed1(Var) :-
        get_attribute(Var,_), !,
        detach_attribute(Var).
detach_att_if_needed1(_).
...
organization_body_diseq([],_Vars,BodyList,I,D,R):- !, %% The disequality is true
						      %% then the negation
...
% eliminate_redundant_var(I,D,R,GoalVars,Iac,Iv,Dv,Rv) eliminates from 
% I the equalities 
...
% Iac is use to acumulate the equalities till the process finish and 
% actualize elimination of redundant variables
...
%% OJO!!!!!!!!!! Esto habria que cambiarlo y devolver todas las desigualdades
%%% en una unica disjuncion en Sol
...
%	!. %% Actually we don't know if it is true or not
...
% substituting existencial quantified variables by skolem constants $(N) 
...
skolem_term(Term,$(_),_UnivVars):- % Due to the global term is a disequality we
	var(Term),!.               % are not going to rename the var by $(N) in
skolem_term(fA(Var),Var,_UnivVars):-   % all the term
...
unify_neg_disj([(T1/_T2) | _DisjExist]):-
	nonvar(T1),
	T1 = $(_N), % It is a existencial variable
	!,
	fail.
unify_neg_disj([(_T1/T2) | _DisjExist]):-
	nonvar(T2),
	T2 = $(_N), % It is a existencial variable
	!,
	fail.
...
% dc(L1,L2) returns in L2 the cartesian product of the elements of L1
dc([], [[]]).
dc([L1|Ls], Lds) :-
        dc(Ls, Lda), 
        dc2(L1, Lda, Lds).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% cneg_types_tr contains predicates to obtain stored_clause/2 facts
% that we need to aply constructive negation
:- load_compilation_module(.(cneg_types_tr)).


% flat_cpred/3 adds a stored_clause/2 fact for each clause or
% fact from the program that includes package cneg_types.pl
% flat_cpred/3 also adds a new predicate for each original one. It
% will have free variables in its arguments. If some of the
% original arguments are typed,then the corresponding new
% predicate will have a body with these types. Only a fact
% in another case

...
cneg_aux(Goal,GoalVarsI,UnivVars):-
	...
	no_unifications(Univs,Univs),
	combine(LSolutions),
	varset(LSolutions,VarsSol),
	no_unifications(VarsSol,Univs),
	put_universal(Univs).
...
negation_conj(I,D,R,GoalVars,ImpVars,ExpVars,UnivVars,SolC):-
	...
	negation_formula(I1,Dimp,Dexp1,Rimp,Rexp1,ImpVars,ExpVars,SolC).
...
% negation_formula(I,Dimp,Dexp,Rimp,Rexp,ImpVars,ExpVars,SolC)
...
% GoalVars, ImpVars and ExpVars are set of useful variables 
negation_formula(I,_Dimp,_Dexp,_Rimp,_Rexp,_ImpV,_ExpV,[Sol]):-
	negate_I(I,Sol). 
negation_formula(I,Dimp,_Dexp,_Rimp,_Rexp,_ImpV,_ExpV,SolC):-
	negate_Dimp(Dimp,Sol),
	append(I,Sol,SolC).
negation_formula(I,Dimp,Dexp,Rimp,Rexp,ImpVars,ExpsVars,SolC):-
...
% negate_I(I,Sol) obtains in Sol a solution of negating I
negate_I(I,Sol):-
	member(Eq,I),
	negate_eq(Eq,Sol).

% negate_eq(Eq,Sol) returns in Sol a solution of the 
% negation of the equality Eq 
negate_eq(T1=T2,dist(T1,T2)).
% 	varset(T1=T2,Vars).            

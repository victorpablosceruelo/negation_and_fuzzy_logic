...
%	         put_attribute/2,
		 ...
		 sustituir/4,
		 conjuncion/3,
		 get_dist_attribute/2,
		 detach_dist_attribute/1
		 ],
		 [mattr_global]).
...
% check_attr(formula(Var,FormVar),Otra):-
%         cumple_cond(Var,Otra, FormVar),
...
combine_attributes(formula(_Var,FormVar),formula(_Otra,FormOtra) , _Vout ):-
        conjuncion(FormVar,FormOtra,FormConj),
...
%sustituir(Term,Var,Valor,Resultado):-
%	copy_term( (Term,Var) , (Resultado,Valor) ).
...
cumple(Var,Form):-
                var(Form),!,
                get_attr(Var,formula(Var,Form)).
cumple(Var,Form):-
                simpl_formula1(Form,FormFinal),
                set_attr(VarAux, formula(VarAux,FormFinal)),
                Var=VarAux.
...
actualizar4(T1,Formula):-
        (esta(Formula,T1) ->
                 set_attr(T1,formula(T1,Formula))
        ;
...
agregar_final([Var|VarsFinal],Vars,Formula):- 
        get_attr(Var,_FormulaOld),!, % Si tiene un atributo
	detach_attr(Var), % Como el atributo que se quiere poner es cierto 
...
agregar3(T1,Form1):-
        var(T1),
        esta(Form1,T1),!,
        (get_attr(T1,formula(T1,Form2)) ->
                conjuncion(Form1,Form2,Form3),
                simpl_formula1(Form3,FormFinal)
        ;
                FormFinal=Form1
        ),
        set_attr(T1,formula(T1,FormFinal)).
agregar3(T1,true):-
        var(T1),!,
        (get_attr(T1,formula(T1,Form2)) ->
...
get_dist_attribute( A , B ) :-
	get_attr( A , B ).

detach_dist_attribute( A ) :-
	detach_attr( A ).
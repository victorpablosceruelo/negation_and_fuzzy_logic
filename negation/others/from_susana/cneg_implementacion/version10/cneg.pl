...
cneg_aux(GoalI,GoalVarsI):-
	...
	combine(LSolutions),
	detect_universal_unifications(GoalI).
...
negate_conj(C,G,GoalVars,SolC):-
	...
	;
	    ...
	    normalization_conj(I,D,R,GoalVars,NowGoalVars,In,Dn,ImpVars,ExpVars), 
...
% normalization_conj(I,D,R,GoalVars,NowGoalVars,In,Dn,ImpVars,ExVars) 
% returns In and Dn that are the equalities of I and the disequalities
% of D but after normalizating them.
normalization_conj(I,D,R,GoalVars,NowGoalVars,I1,D1,ImpVars,ExpVars):-
	varset(R,RVars),
...
% detect_universal_unifications(GoalI) ckecks that the universal variables
% of GoalI haven't been unified during the negation.	
detect_universal_unifications(GoalI):-
	var(GoalI),!.
detect_universal_unifications(fA(Var)):- !,
	var(Var), % If it is a variable, them it has no attribute
	\+ get_attribute(Var,_).
detect_universal_unifications(Term):-
	Term=..[_Functor|Args],
	d_u_u_all(Args).

% d_u_u_all(Args) aplies detect_universal_unifications(Arg) to all the
% elements of the list Arg.
d_u_u_all([]).	
d_u_u_all([Arg|Args]):-
	detect_universal_unifications(Arg),
	d_u_u_all(Args).

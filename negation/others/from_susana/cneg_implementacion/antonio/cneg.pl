...
frontier((X=_Y),[(X=X,[])]):- !.
frontier(Goal,Frontier):-
	setof((Goal,Body),stored_clause(Goal,Body),Frontier1),!,
	normalize_frontier(Frontier1,Frontier).
frontier(_Goal,[]).
...
organization_conj((Head,BodyList),G,_GoalVars,I,D,R):-
	...
	%;
	   %replace_list(HeadVars1,HeadVars,Values,Values1),  %%%%%%TODO ESTO TOÑO???????
...
organization_body([T1=T2|BodyList],[(T1=T2)|I],D,R):-
	...
	setof(Vars,(T1=T2),[_Values]),!,  %%% Problems????????
	%(Values=[] ->
	    organization_body(BodyList,I,D,R).  %%%%%%%TODO ESTO TOÑO?????????????
	%;
...
normalization_conj(I,D,R,GoalVars,I1,D1,ImpVars,ExpVars):-
	...
	remove_duplicates(ImpVars1,ImpVars), %%%%%%% TOÑO??????????????????????
        difference(RVars,ImpVars,ExpVars1),
	eliminate_redundant_var(I,GoalVars,Iv),
        %eliminate_repeated_eq(I,I1),
        eliminate_repeated_eq(Iv,I1),
        remove_duplicates(ExpVars1,ExpVars), %%%%%%% TOÑO??????????????????????
	remove_duplicates(RelVars1,RelVars), %%%%%%% TOÑO??????????????????????
	eliminate_irrelevant_disequalities(D,ImpVars,RelVars,D1). 
 
% unificar(I,GoalVars) unifica las variables que no son de GoalVars con
% las que son de GoalVars según las igualdades de I.
unificar([],_GV).
...
% normalize/3 adds a stored_clause/2 fact for each clause or
% fact from the program that includes package cneg.pl but
% it does it continously
:- add_sentence_trans(normalize/3).

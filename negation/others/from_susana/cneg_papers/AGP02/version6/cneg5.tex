...
{\bf Contraints simplification}. Other way of optimization is the
...
constrainsts are satisfyable and we can eliminate them, other times
...
different names to the head of the formula aplying logic rules:
...
be to substitute de existencial quantified variables (those that do
...
\[ Fe_k  \equiv \exists ~ \overline{x} \bigwedge \neg (Y_{Sk l}^k \neq s_{Sk l}^k)  \equiv \exists ~ \overline{x} \bigwedge (Y_{Sk l}^k = s_{Sk l}^k)  \]
        ...
        successes then we remove the constraint because is redundant
        and we continue with the negation process. If it fails then
        the negation directly successes.
...
could expect for these examples. Futhermore the results are rather
...
it would avoid the inefficience of the constructive negation and it

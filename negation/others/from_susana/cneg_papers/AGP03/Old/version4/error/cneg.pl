...
caca(Goal,Body):-
	stored_pred(Goal,Goal1),
	stored_clause(Goal1,Body).
...
% compound_meta_to_term(M,G) obtains the possibly compound term G
% from the, possibly compound, metaterm M
compound_meta_to_term(M,G):-
	term_to_meta(T,M), !, % It's a metaterm
	compound_to_term(T,G).
compound_meta_to_term(T,G):- % It's a term
	compound_to_term(T,G).
...
compound_to_term(T,G):- % It's a simple term
	stored_pred(T,G).
% compound_to_term(T,T).   % It's a term

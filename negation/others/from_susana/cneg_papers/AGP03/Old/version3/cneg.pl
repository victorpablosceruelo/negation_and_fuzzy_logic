...
cneg_aux(Meta,GoalVars):-
	...
	stored_pred(Term,Goal), %% To recover the name without module
...
frontier((G1;G2),Frontier):- !,
	frontier(G1,F1),
	frontier(G2,F2),
	append(F1,F2,Frontier).
frontier((G1,G2),Frontier):- !,
	frontier(G1,F1),
	frontier(G2,F2),
	conjunction(F1,F2,Frontier).
frontier(Goal,Frontier):-
...
distributive([],_E,[]).
distributive([E1|L1],E2,[E3|L2]):-
        append(E1,E2,E3),
        distributive(L1,E2,L2).
...
% obtain_rep_head(GoalVars,Vs,NewVs,_R,Ir) retuns the list I where the element 
...
organization_body_diseq_aux(ValueVars,Vars,BodyList,I,D,R):-
	member(V,ValueVars), % We take only the first values because the
	obtain_diseq(V,D1), % atribute is the same in all of them
	replace_list(ValueVars,Vars,D1,Dh),

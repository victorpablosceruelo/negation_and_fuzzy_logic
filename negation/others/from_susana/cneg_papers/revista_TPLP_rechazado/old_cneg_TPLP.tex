...
\noindent
\emph{Implementation details}: The predicate $negate\_Rimp/2$ provides the
...
$negate\_Rimp([even(X)],Sol)$ is obtained the solution $cneg(even(X))$ that
later is combined with the positive equalities and disequialities and the
...
will give us so many answers (going deeper and deeper in the recursive calls)
as we want. $\Box$

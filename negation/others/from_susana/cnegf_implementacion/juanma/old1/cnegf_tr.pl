...
:- module(_,[ add_cnegf/3 %, flat_dpred/3,flat_cpred/3,conj_to_list/2
],[assertions]).
...
add_sth(end_of_file, (end_of_file), _):- 
 !.

add_sth(
(:- true pred Predi : Algo => Tipo + Info),
[(:- true pred Predi : Algo => Tipo + Info), finite(P,A)], 
_):-
        conj_to_list(Info, Lista),
        perteneceALista(not_fails,Lista),
        perteneceALista(step_ub(_),Lista),

%        perteneceALista((possibly_fails),Lista),

	functor(Predi,P,A),
%        assertz_fact(finite(P,A)),
	!.

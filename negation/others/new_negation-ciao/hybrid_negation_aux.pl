...
:- module(hybrid_negation_aux, [ term_vars/2, terms_are_equal/2,
			...
			transpose_matrix/3,
			convert_clauses_format/5,
			revert_clauses_format/2,
			obtain_clause_head/2,
			obtain_clause_cj/2,
			obtain_clause_arity/2,
			obtain_clause_body/2,
			obtain_clause_name_and_arity/3,
			obtain_clause_nocjs/2, 
			obtain_clause_number/2,
...
append_prefix_to_atom(Atom, Prefix, NewAtom) :-
	name(Atom, Atom_String),
	name(Prefix, Prefix_String),
	append(Atom_String, Prefix_String, NewAtom_String), 
	name(NewAtom, NewAtom_String).
...
add_to_list_if_not_there(Name/Arity , [], [Name/Arity]) :- !.
add_to_list_if_not_there(Name/Arity , [Name/Arity|Processed_IN], [Name/Arity|Processed_IN]) :- !.
add_to_list_if_not_there(Name/Arity , [Name2/Arity2|Processed_IN], [Name2/Arity2|Processed_OUT]) :- !,
	add_to_list_if_not_there(Name/Arity , Processed_IN, Processed_OUT).
...
convert_clauses_format([], [], [], Processed_IN, Processed_IN) :- !.
convert_clauses_format([Cl], [F_Cl], [], Processed_IN, Processed_OUT) :-
	convert_1_clause_format(Cl, F_Cl, Name/Arity), !,
	add_to_list_if_not_there(Name/Arity , Processed_IN, Processed_OUT), !.
convert_clauses_format([Cl], [], [Cl], Processed_IN, Processed_IN ) :- !.
convert_clauses_format([Cl | Cls], [F_Cl | F_Cls], F_Invalid_Cls, Processed_IN, Processed_OUT) :-
	convert_1_clause_format(Cl, F_Cl, Name/Arity), !,
	add_to_list_if_not_there(Name/Arity , Processed_IN, Processed_TMP), !,
	convert_clauses_format(Cls, F_Cls, F_Invalid_Cls, Processed_TMP, Processed_OUT).
convert_clauses_format([Cl | Cls], F_Cls, [Cl|F_Invalid_Cls], Processed_IN, Processed_OUT) :- !,
	convert_clauses_format(Cls, F_Cls, F_Invalid_Cls, Processed_IN, Processed_OUT).

convert_1_clause_format((:- _Declaration), fail, _Name/_Arity) :- !, fail.

convert_1_clause_format((H :- B), cl(Name, Arity, Args, [], body((B)), (-1)), Name/Arity) :-
	!, % Pred with body.
%	write('% DBG % '),
%	write((H :- B)), nl,
	clause_head(H, Name, Arity, Args).

convert_1_clause_format((H), cl(Name, Arity, Args, [], body(true), (-1)), Name/Arity) :-
	!, % Pred without body.
%	write('% DBG % '),
%	write(H), nl,
	clause_head(H, Name, Arity, Args).
...
obtain_clause_name_and_arity(cl(Name, Arity, _Cj, _NoCjs, _Body, _N), Name, Arity).

obtain_clause_cj(cl(_Name, _Arity, Cj, _NoCjs, _Body, _N), Cj).
%	write(obtain_clause_cj((_Name1, _Arity1, Cj, _NoCjs, _Body), Cj)), nl.

obtain_clause_nocjs(cl(_Name, _Arity, _Cj, NoCjs, _Body, _N), NoCjs).
...
obtain_clause_number(cl(_Name, _Arity, _Cj, _NoCjs, _Body, N), N).

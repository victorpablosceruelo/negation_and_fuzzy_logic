...
call_to_aux(Predicate, Level_In, Trace) :-
	...
	;
	    (           echo_msg_3pm(2, '', 'cneg_rt', 'call_to (L', Level, ') :: G1 \\/ G2 :: G2', 'FAIL'),
			echo_msg_3pm(2, '', 'cneg_rt', 'call_to (L', Level, ') :: G1 \\/ G2', 'FAIL'), 
			fail 
	    )
	).
...
%	Level is Level_In + 1,
%	echo_msg_3pm(2, '', 'cneg_rt', 'call_to (L', Level, ') :: Predicate - FAILED -', Predicate),
%	!, fail. 

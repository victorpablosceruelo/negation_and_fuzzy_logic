...
:- module(_,[ add_sth/3 %,add_cnegf/3, flat_dpred/3,flat_cpred/3,conj_to_list/2
],[]).


%NO :- use_module(library(dynamic)). 
%http://www.clip.dia.fi.upm.es/Software/Ciao/ciao_html/ciao_28.html#SEC134
...
%NO :- module(cneg_tr,[flat_dpred/3,flat_cpred/3,conj_to_list/2],[]).

:- use_module(library(aggregates)). 
%NO :- use_module(library(aggregates),[findall/4]).
%:- use_module(library(term_basic),[functor/3]).
%NO :- use_module(dist,[dist/2]).
...
%aux((:- true _Algo _Predicado(_X) : _Y => _Z + Conjunto), Conjunto).


%add_sth(end_of_file, [Facts,(end_of_file)], _):- 
% !.
% findall(X,(finite(X)),Facts),
add_sth(end_of_file, [(bien(X)),(end_of_file)], _):- retract_fact(bien(X)),
	!.
%add_sth(par(2), par(2), _):- assertz_fact(bien(par(2))), 
% (:- true pred Predicado(X) : Y => Z + Conjunto),
add_sth(
(:-(
true(
[pred, Predicado, :, Y ,=>, Z ,+, Conjunto]
)
))
%( :-(true pred (B)) C D E F G)
%( :-(true pred(par(A) :(mshare([[A]]) =>(ground(A)))) )
%( :-(
%Body pred Head : Algo => Tipo
%:(pred(true,Par),=>(Algo,Tipo))
%(+(=>(:((pred(Par)),true),Mas)),Comp)
%))
, [---------------------------], _):-
%         aux(Body,Conjunto),
%        conj_to_list(Conjunto, Lista),
%        perteneceALista(not_fails,Lista),
%        perteneceALista(step_up(_),Lista),
%        assertz_fact(finite(Predicado)),
        assertz_fact(bien(par(2))),
	!.
...
% ARCHIVO ADD_CNEGF
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%555
%condicionPredicado(Pred) :- finito(Pred).
%condicionPredicado(Pred) :- Pred == s(0).
%condicionPredicado(_).  :- bien(2). %current_fact(bien). 

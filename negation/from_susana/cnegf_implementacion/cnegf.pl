...
cnegf(Pred):-
	...
	normalizar(Lista_a_transformar,GVars,LATN),
        combine_sols(LATN,Lista_soluciones_negada,GVars,[],[],_LGVars,LNoGVars),
...
% listas, convertir lo unifica todo en una unica lista
	...
	%no_gvars_neg(Lista_de_GoalVars_igualadas,GVars,No_GVars),
...
%eliminar_igualdades_inutiles(Lista_igualdades, LNoGVars1, Sol). Si existe un igualdad de Lista_igualdades
%                  del forma X=Y, si Y pertenece a LNoGVars2 y no tiene atrbuto , pero no a LNoGVars1 se 
%                  elimina de Lista_Igualdades, obteniendo al final esa lista en Sol
...
no_p_cneg(V1,V2,VX,VY):-cnegf(p_cneg(V1,V2,VX,VY)). 

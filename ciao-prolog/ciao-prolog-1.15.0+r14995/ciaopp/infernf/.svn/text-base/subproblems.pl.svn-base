...
:- use_module(infernf(nfbool), [push_conj/2]).
...
:- use_module(library(system),
	    [system/1, delete_file/1, file_exists/1]).
:- use_module(library(write),         [write/1, numbervars/3]).
:- use_module(library(iso_byte_char), [char_code/2]).
...
colect_tuples_of_terms([Minset|MinSetList], InTuples, InCobTuples,
	    OutTuples, OutCobTuples) :-
	...
	colect_tuples_of_terms(MinSetList, TemTuples, TemCobTuples,
	    OutTuples, OutCobTuples).
...
solve_subproblem_basic(Tuple, TestList, Ta_Term, Flag, Res) :-
	...
	( ta_term_empty(Intersection) -> Res = true;
...
colect_other_tests([Test|TestList], Ta_Term,  Flag,  InOther, OutOther) :-
	...
	( tuple_included(Flag, Term, minset(MTuple, CobSetList)) ->
		...
		( select_test(Ta_Term, MTuple, RTest, OuTest) ->
		    colect_other_tests(TestList, Ta_Term, Flag,
			[OuTest|InOther], OutOther)
		;
		    colect_other_tests(TestList, Ta_Term, Flag,
			InOther, OutOther)
		)
...
unary_arithmetic_test(Test) :-
	functor(Test, F, A),
	member_0(F/A, [(integer) /1, (number) /1]).
...
num_covers(Tests, Res) :-
	...
	(clp_meta(CIAOTest) -> Res = fail ; Res = true),
...
%% BEGIN COVERING WITH OMEGA TEST.
...
%  set_output(Omega_Stream),
%  popen('omega omega_call', write, Omega_Stream),
%  close(Omega_Stream),
	    omega_test(Res) ).

:- module(error, [error_message/3], [assertions]).
...
error_message(Code,Source,Clause,Module) :-
	error_message_("unknown error ~w: ~w", [Code, Source], Clause, Module).
...
error(dec3,     n, y, "Analysis check: mode is not declared for predicate: ~w"). %JNL
%error(dec3,     n, y, "mode is not declared for predicate: ~w").
error(dec4,     n, y, "Analysis check: measure is not declared for predicate: ~w"). %JNL
...
error(second_order1, w, y, "Dependency analysis: illegal predicate arg ~w in findall predicate"). %JNL

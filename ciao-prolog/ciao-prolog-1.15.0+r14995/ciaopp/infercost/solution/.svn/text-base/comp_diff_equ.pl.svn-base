...
:- use_module(infercost(size(size_)), [up_low_approximation_expr/3]).
:- use_module(infercost(diff_equation(diff_equ)), 
	[
...
:- check comp solve_complexity_equ/6 + not_fails.
...
:- check comp solve_comp_equs1/5 + not_fails.
...
solve_comp_equs2(DE, [],  ST,  Pred, Sol) :-
	DE \== [],
	!,
	(
	...
	;
	    solve_comp_diff_equs(DE, [], ST, Pred, Sol)
	).
solve_comp_equs2([], BE, _ST, _Pred, Sol) :-
	BE \== [],
	!,
...
solve_comp_diff_equs(DE,BE,ST,Pred,Sol) :-
	...
	(
	    BE == [] ->
	    implicit_boundary_condition(ST,Pred,Pos,RBE)
	; % implicit failure

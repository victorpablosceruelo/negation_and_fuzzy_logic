:- module(diff_equ, [solve_diff_equ/7, solve_typed_diff_equ/8], [assertions]).
...
:- use_module(infercost(diff_equation(product)), 
	[
	    product_diff_equ/3,
	    log_base_equs/2,
	    exp_solution/2
	]).
:- use_module(infercost(size(size_diff_equ)), 
	[solve_one_index_size_diff_equ/10]).
:- use_module(infercost(solution(comp_diff_equ)), 
	[solve_one_index_comp_diff_equ/10]).
:- use_module(infercost(diff_equation(first_order)), 
	[
	    first_order_diff_equ/6,
	    solve_fode/5
	]).
:- use_module(infercost(diff_equation(second_order)), 
	[
	    second_order_diff_equ/6,
	    solve_sode/6
	]).
:- use_module(infercost(diff_equation(higher_order)), 
	[
	    higher_order_diff_equ/6,
	    solve_hode/5
	]).
:- use_module(infercost(diff_equation(divide_conquer)), 
	[
	    divide_conquer_diff_equ/6,
	    solve_dcde/6
	]).
:- use_module(infercost(diff_equation(mutual_size)), 
	[
	    mutual_size_diff_equ/6,
	    solve_msde/5
	]).
:- use_module(infercost(diff_equation(diff_equ_implicit_size)), 
	[
	    implicit_size_diff_equ/6,
	    solve_isde/4
	]).
:- use_module(infercost(diff_equation(explicit_size)), 
	[
	    explicit_size_diff_equ/6,
	    solve_esde/4
	]).
:- use_module(infercost(diff_equation(list_size)), 
	[

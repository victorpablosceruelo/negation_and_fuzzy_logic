:- module(normal_form, [normal_form/2, time_normal_form/2], []).
...
:- use_module(infercost(algebraic(maxmin)), [max_expr/3, min_expr/3]).
...
resource_normal_form([resource(Name, X)|Xs], [resource(Name, Y)|Ys]) :-
	normal_form(X, Y),

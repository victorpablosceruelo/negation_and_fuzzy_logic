...
% Own library
:- use_module(ciaopp(tr_parallel(tr_granul(name_basic))), 
	[create_dyn_pred_name/2]).
:- use_module(ciaopp(tr_parallel(tr_granul(classify))), [classify/4]). 
:- use_module(ciaopp(tr_parallel(tr_granul(names))), [create_names/9]). 
:- use_module(ciaopp(tr_parallel(tr_granul(input_sizes))), 
	[ get_needed_body_sizes/2, 
	  needed_input_sizes/2
	]). 
:- use_module(ciaopp(tr_parallel(tr_granul(need_size_table))), 
	[ actualize_nt/4, 
	  find_size/3
	]). 
:- use_module(ciaopp(tr_parallel(tr_granul(names_table))), [find_name_entry/3]). 
:- use_module(ciaopp(tr_parallel(tr_granul(g_utility))), 
	[ ith_element/3,  % (see ciao library)
	  is_builtin_pred/1
	]). 
:- use_module(ciaopp(tr_parallel(tr_granul(gen_expre))), 
...
:- use_module(ciaopp(tr_parallel(tr_granul(thresholds))), [find_size_threshold/4]). 
:- use_module(ciaopp(tr_parallel(tr_granul(dynamic_size))), [generate_initial_litnodes/8]). 
:- use_module(basic_trans_res, 
...
dyn_gran_transform_clause(fact, Clause, _ASizeRel, _ST, NT, _GT, AClause):- 
	functor(Clause, F, A), 
	find_name_entry(NT, F/A, st(_,_,GranName/_,InArList,_,_)),
	dyn_transform_head(GranName, InArList, Clause, AClause, _).
dyn_gran_transform_clause(rule, (Head:-Body), ASizeRel, ST, NT, GT, 
                          (TranHead :- TranBody)):- 
...
dyn_transform_threads([Lit|Litrs], InLitNum, OutLitNum, ST, NT, GT, ASizeRel, 
        ...
        dyn_transform_one_thread(RLiterals, InLitNum, OutLitNum, ST, NT, GT,
                                 ASizeRel, ArgPosDict, TranThreads).
...
dyn_transform_one_thread([Lit1, Lit2], InLitNum, OutLitNum, ST, NT,
                         GT, ASizeRel, ArgPosDict, AnnThreads):-
        translate_to_internal(Lit1, InternalLit1),
        get_pred_literal(InternalLit1, LitPred1),
        % get_pred_literal(Lit1, LitPred1),
        find_symbol_field(ST, LitPred1, time, TimeFuncVect0),
	%%% 
        ( current_pp_flag(para_grain,gr_res) ->
	  get_all_resources( Res_s ),
	  current_pp_flag(para_grain_res,GranRes_s),
	  find_cost_function(TimeFuncVect0,Res_s,GranRes_s,TimeFuncVect1)
        ;
          TimeFuncVect0 = TimeFuncVect1
        ),
        %%%
        (
	    var(TimeFuncVect1) -> 
	...
	; 
	    ...
	    dyn_obtain_test_foreach_resource(TimeFuncVect1, ASizeRel, Lit1, InLitNum, ArgPosDict,
                            TestVect1),
	    ...
	    find_symbol_field(ST, LitPred2, time, TimeFuncVect3),
	    ( current_pp_flag(para_grain,gr_res) ->
	      % TimeFunc3 is a vector of cost functions
	      % get_all_resources( Res_s ),
	      % current_pp_flag(para_grain_res,Gran_Res),
	      find_cost_function(TimeFuncVect3,Res_s,GranRes_s,TimeFuncVect2)
	      
	    ;
	      TimeFuncVect2 = TimeFuncVect3
	    ),
	    (
		var(TimeFuncVect2) -> 
		warning_message("No cost function available for: ~q.",
                                [LitPred2])
	    ; 
		create_lit_granul_version(GT, NT, Lit2, ASizeRel, GraLit2,
                                          ArgPosDict),
                create_lit_sequential_version(GT, Lit2, SeqLit2), 
		dyn_obtain_test_foreach_resource(TimeFuncVect2, ASizeRel, Lit2, LitNum2,
                                ArgPosDict, TestVect2),
		create_annotated_thread_foreach_resource( GranRes_s, TestVect1, TestVect2, 
		                SeqLit1, SeqLit2,GraLit1, GraLit2, AnnThreads)
	    )
	).

create_annotated_thread_foreach_resource([],[],[],_,_,_,_,[]).
create_annotated_thread_foreach_resource([R|Rs], [Test1|Tests1], [Test2|Tests2],
	                SeqLit1, SeqLit2, GraLit1, GraLit2, [AnnThread|AnnThreads]):-
        create_annotated_thread(Test1,Test2,R,SeqLit1,SeqLit2, 
			        GraLit1, GraLit2,AnnThread),
	create_annotated_thread_foreach_resource(Rs,Tests1,Tests2,
 			        SeqLit1, SeqLit2, GraLit1, GraLit2, AnnThreads).


create_annotated_thread(Test1, Test2,Resource, SeqLit1, SeqLit2, GraLit1, GraLit2,
                        AnnThread):-
	Test1 == Test2,
	!,
	AnnThread = resource(Resource,
	            (
...
dyn_obtain_test_foreach_resource([],_,_,_,_,[]).
dyn_obtain_test_foreach_resource([TF|TFs],SizeRel, Literal, LitNum, ArgPosDict,[Test|Tests]):-
	dyn_obtain_test(TF, SizeRel, Literal, LitNum, ArgPosDict, Test),
	dyn_obtain_test_foreach_resource(TFs,SizeRel, Literal, LitNum, ArgPosDict,Tests).
...
transform_dyn_literals_0(LitNodes,_FlatBody,TranBody,TranBody,_ArgPosDict):-
	var(LitNodes), !.
transform_dyn_literals_0(LitNodes,FlatBody,InTranBodyVect,OuTranBody,ArgPosDict):-
	nonvar(LitNodes),
	...
	replace_literal(InTranBodyVect, DynLiteral, TrDynLiteral, TemTranBody),
...
% A fact
transform_dyn_one_clause(fact, TrPred, Label, Claus, ClGraph, TrClaus):-
...
% A rule
transform_dyn_one_clause(rule, TrPred, Label, (Head:-Body), ClGraph, 
                         (TrHead:-TrBody)):-
...
find_cost_function([Cost_Functions],Resources,Gran_Res_s,Cost_Functions_selected):-
	find_cost_function_(Gran_Res_s,Cost_Functions,Resources,Cost_Functions_selected).

find_cost_function_([],_CFs,_Resources,[]).
find_cost_function_([X|Xs],CFs,Resources,[NCF|NCFs]):-
	find_cost_function_indexed_by_resource(CFs,Resources,X,NCF),
	!,
	find_cost_function_(Xs,CFs,Resources,NCFs).
find_cost_function_([X|_],_,_,[]):-
	error_message("No cost function for ~q",[X]),
	!,
	fail.

% PRE: cost functions and resources MUST be sorted in the same order
find_cost_function_indexed_by_resource([F|_],[R|_],R,F).
find_cost_function_indexed_by_resource([_F|Fs],[_R|Rs],R0,RF):-
	find_cost_function_indexed_by_resource(Fs,Rs,R0,RF).

:- module(remotecut,[cleanup_remcut/0,transform_remote_cut/8],[assertions]).

:- use_module(program(clidlist), [clause_key/2]).
:- use_module(program(native),   [native_builtin/2]).
...
/*
:- use_module(program(itf_db), [assert_itf_chapuza/2]).

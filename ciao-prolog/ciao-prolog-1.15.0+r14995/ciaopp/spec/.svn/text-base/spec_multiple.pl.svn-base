...
:- use_module(plai(domains),            [identical_proj/5]).
:- use_module(spec(s_simpspec),         [make_atom/2, newformat/2]).
:- use_module(program(p_unit),          [new_predicate/3, predicate_names/1]).
:- use_module(program(itf_db),          [assert_itf/5, retract_itf/5]).
:- use_module(spec(s_simpspec), [make_atom_list/2, list_format/2]).
:- use_module(program(clidlist),
        [atom2data/5, clid2data/4, pred2data/3]).
:- use_module(plai(plai_db), [complete/7]).
:- use_module(plai(plai_errors)).
...
get_entry_in_parents([(Clause,_N)|Parents],[Name|More]):-
	clid2data(Clause,Name,_Arity,_),!,
	get_entry_in_parents(Parents,More).
get_entry_in_parents([(Pred,_N)|Parents],[Name|More]):-
	pred2data(Pred,Name,_A),
 	get_entry_in_parents(Parents,More).
...
all_versions(Program,Dicts,Abs,Sp_program,Sp_Dicts):-
	...
	inform_user(['{transformed by vers in ', T,' msec.}']).

:- module(_,
	[form_one_rule_with_susp/3,
	 ...
	 decide_arith_simp/2,
	 is_recursive/1
	], [assertions, .(nounfold_stats), .(nomem_usage), .(no_debug)]).
...
:- use_module(plai(domains(typeslib(regtype_basic_lattice))), 
...
form_one_rule_with_susp(clause(Sg,[L|R],Susp),clause(L,Body),clause(Sg,NBody,Susp)):-
	append(Body,R,NBody).
...
can_continue(e(AbsInt,OldSg,OldSv,OldProj),L,_Lit,Sg,Case):-
	...
	abs_exec_regtype_in_clause(SPred,F,A,L,BodyVars,Entry,Sense),!,
...
unfold_literal_one_step(internal,L,UnfClause):-!,
	update_mem_usage,
	collect_one_orig_clause(L,UnfClause,_Id).

% if the execution of the builting does not generate bindings, 
% it is indeed equivalent to a call to predicate true/0.
unfold_literal_one_step(external,L,UnfClause):-!,
%	copy_term(L,L_call),
	'$meta_call'(L),
	inc_unfold_evals(1),
	UnfClause=clause(L,[]),
...
unfold_literal_one_step(findall(T,Goal,List),L,UnfClause):-!,
	findall(T,'$meta_call'(Goal),List),
	UnfClause=clause(L,[]),
	debug('UNFOLDED builtin '),
	debug(L).
unfold_literal_one_step(setof(T,Goal,List),L,UnfClause):-!,
	setof(T,'$meta_call'(Goal),List),
	UnfClause=clause(L,[]),
	debug('UNFOLDED builtin '),
	debug(L).
unfold_literal_one_step(bagof(T,Goal,List),L,UnfClause):-!,
	bagof(T,'$meta_call'(Goal),List),
	UnfClause=clause(L,[]),
	debug('UNFOLDED builtin '),
	debug(L).
unfold_literal_one_step(true,L,UnfClause):-!,
	UnfClause=clause(L,[]),
	debug('ABS EXEC to true '),
	debug(L).

unfold_literal_one_step('basiccontrol:fail',L,_UnfClause):-!,
...
fact_or_residual(clause(_,[],_)).


peel_fact_or_residual(residual(Cl),clause(Head,Body)):-!,
	Cl=clause(Head,Body).
%	filter_pops(Body,NewBody).
peel_fact_or_residual(fact(Cl),clause(Head,Body)):-!,
	Cl=clause(Head,Body).
%	filter_pops(Body,NewBody).
peel_fact_or_residual(Cl,clause(Head,Body)):-
	Cl=clause(Head,[],Body).
%	filter_pops(Body,NewBody).
...
peel_call('hiord_rt:call'(Goal),Body,NGoal,NBody):-
	...
	nonvar(Pred),!,
	functor(Pred,F,A),
	atom_concat(':',F,Suffix),
	atom_concat(Module,Suffix,Name),
...
is_recursive(L):-
	functor(L,F,Ar),
	make_atom([F,Ar],SgKey),
	read_clause(SgKey,r,_),!.

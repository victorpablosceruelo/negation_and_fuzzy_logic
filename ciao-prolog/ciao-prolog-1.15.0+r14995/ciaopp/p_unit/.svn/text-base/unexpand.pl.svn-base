...
:- use_module(api(api_direc_assrt), [add_directive/1]).
...
%	  module_spec(ISpec, IM),
	  ...
	  functor(GoalReexp, F, A),
 	  (current_itf( reexported , GoalReexp , r(IM2,IM) )-> true ; IM2=IM),
...
simplify_qualify(M:F, A, F) :-
	functor( Goal , F , A ),
	(current_itf( reexported , Goal , r(M2,M));M2=M),
...
% 	functor( Goal , F , A ),
% 	current_itf( reexported , Goal , r(_,EM) ),
% 	get_mod_pred( F , _, Pred ),
% 	!,
% 	atom_concat( EM , ':' , EMM ),
% 	atom_concat( EMM, Pred , NF ),

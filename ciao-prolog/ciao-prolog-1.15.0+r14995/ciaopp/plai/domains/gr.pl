...
later projecting the resulting substitution onto @var{Hv}. This is
done as follows: 
@begin{itemize} 
...
gr_call_to_entry(_,_,[],_Head,Fv,_Proj,Entry,no):- !,
	gr_change_values_insert(Fv,[],Entry,ng).
gr_call_to_entry(Sv,Sg,Hv,Head,Fv,Proj,Entry,ExtraInfo):-
...
gr_input_user_interface((Gv_u,Ng_u),Qv,ASub):-
	...
	gr_change_values_insert(Ng,Temp1,Temp2,ng),
...
gr_input_interface(not_ground(X),perfect,(Gv,NGv0),(Gv,NGv)):-
	varset(X,Vs),

...
# Filename      : SH2NSH.pl
# Language      : Perl 
# Purpose : builds a negative database representing positive strings
#           precisely
# 
# Creator       : E.S. Ackley
# Creation Date : Sat Apr 14 11:39:02 2007
# Updated       : Tue Apr 22 14:15:31 2008 
# -------------------------------------------------------------------------
#
...
my $nshfilename      = "NSH0";   # default negative database name
...
my $KNOWN_MISSING=0;        # negative length argument to do subtractive way
my $CALC_MISSING=1;         # 1=calculates missing, if LEN le 32, and SH gt
                            # CUTOVER_PERCENT of powerset, o.w. "adds" known

my $CUTOVER_PERCENT=.5;      # 1 essentially disables subtractive feature 
                            # (needs <1.00), 
...
my $INT_SIZE=32;
my $N=0;
my $MAX_KEY=1;              # updated later
my $C=2;                    # multiplicative constant for max_key as a log of LEN
...
sub usage_abort 
{
    die "Usage: % ./SH2NSH.pl [-]len [SHfilename [NSHfilename]]\n";
}
...
# print "args 0 is <$args[0]>\n";
    ...
    if( $args[0] =~ /-?[1-9][0-9]*/ ) {
        ...
        if ($LEN > $MAXL || $LEN == 0 ) {
            die "SH2NSH: Record length (first arg) is INVALID, can't be greater than $MAXL\n";
        }
    } else {
        die "SH2NSH: Record length (first arg) must be an integer\n";
    }
    ...
    if($argc >= 2) {
        if ( $args[1] =~ /^[a-zA-Z0-9\.\_\/\\-]{1,255}$/ ) {  # alt sh file
            ...
            die "SH2NSH: positive input filename must be alphanumeric and less than 255 letters long>\n";
        }
        if($argc == 3) {
            if ( $args[2] =~ /^[a-zA-Z0-9\.\_\/\\-]{1,255}$/ ) {  # alt nsh file
                $nshfilename = $args[2];
            }else {
                die "SH2NSH: nsh output filename must be alphanumeric and less than 255 letters long>\n";
            }
        }
    }
...
#        $MAX_KEY = int($LEN/2+.5);    # v.17 round up
        $MAX_KEY = &logbase2($LEN) * $C + $G;     # try v.19.11, v.19.12, v.19.13
...
sub read_db_sort_for_missing {
    ...
    open(FULLNSH,">$mdbfn") || die "Unable to create new missing sh $mdbfn\n";
...
# completes the rest of the powerset not in DB, here 
    ...
    close FULLNSH;
    $debug && print("SH2NSH read_db_sort_for_missing: read in $total records, rejected $rej, saved $num\n");

    return $num;
}


sub read_db_solve_for_missing {
    my $dbfn = $_[0];
    my $mdbfn = $_[1];
    my $fn = "$configfilepath/$dbfn";

    print "Solve for missing..\n";
    `$crackpath/crack_ndb.pl $fn -b > $mdbfn`;
}


#read in binary positive strings of the same length that are MISSING
#from SH; return number of records;
sub build_nsh_from_missing {
    ...
    $debug && print("SH2NSH build_nsh_from_missing: read in $total missing records, rejected $rej, removed $N\n");
...
sub read_db_and_build_nsh {
    ...
    $debug && print("SH2NSH read_db_and_build_nsh: read in $total records, rejected $rej, added $N\n");
...
#"removing" to the negative db; return number of records (v.16)
sub read_db_as_graycode {
    ...
    close GRAYDB;
    close INDB;
    $debug && print("SH2NSH read_db_as_graycode: read in $total records, rejected $rej\n");
    return $total;
}

 
sub build_nsh_from_graycode
{   
    my $missingflag = $_[0];     # 1="remove"; =0 "add"
    my $tmpgrayfn = "$tmpfilepath/dbgray.tmp";
    my $total = 0;
    ...
    $debug && print("SH2NSH build_nsh_from_graycode: added/removed <$N> recs\n");
    return $N;
}
...
sub remove_db_rec {
    ...
    if ($? == -1) {
        print "SH2NSH remove_db_rec: ($zit) failed to execute [$!]\n";
        $ans = -1;
    } else {
        $ans = $? >> 8; 
        if($ans == 0) {
            ($debug || $admin) && print("SH2NSH remove_db_rec: ($zit) removed [$rec]\n");
            return $ans;
        }
        if($ans > 1) {
            print("SH2NSH remove_db_rec: ($zit) found error [$ans]\n");
        }
    }
    $debug && print "args to remove_db_rec: rec is <$rec>, zit is <$zit>, ans is <$ans>\n";
    $SANITY && die "SH2NSH: args to remove_db_rec: rec is <$rec>, zit is <$zit>, ans is <$ans>\n";
    return $ans;
}
...
sub add_db_rec {
    ...
    `$cmd $stdoutput $stderrors`;
    if ($? == -1) {
        print "SH2NSH add_db_rec: ($zit) failed to execute [$!]\n";
        $ans = -1;
    } else {
        $ans = $? >> 8; 
        if($ans == 0) {
            ($debug || $admin) && print("SH2NSH add_db_rec: ($zit) added [$rec]\n");
            return $ans;
        }
            if($ans > 1) {
                print("SH2NSH add_db_rec: ($zit) found error [$ans]\n");
            }
    }
    $debug && print "args to add_db_rec: rec is <$rec>, zit is <$zit>, ans is <$ans>\n";
    $SANITY && die "SH2NSH: args to add_db_rec: rec is <$rec>, zit is <$zit>, ans is <$ans>\n";
    return $ans;
}
...
sub new_empty {
    ...
    print(" ($rtn)");
    print "\n";
    $SANITY && die "SH2NSH: new_empty failed, mode is <$mode>\n";
    return $rtn;
}
...
# return TRUE (1) if nsh size is greater than $CUTOVER_PERCENT of a
...
sub check_dbsize {
    ...
    if($LEN <= $INT_SIZE){
        ...
        $debug && print "check_dbsize: <$fn> lines is calculated <$lines>\n";
        if($lines == $ps){
            $rtn = 2;
...
# bkup last NSH to bkup; remove old NSH incremental saves, and
# initialize Z to 1 for future incremental saves
sub bkupNSH {
   my $fn = "$configfilepath/$nshfilename";
   my $bkup = "$configfilepath/bkup-$nshfilename";
...
sub cleanup {
    if(!$debug && !$admin){
        ...
        if($USENEGDB_SCRIPTOPT) {
            unlink("$tmpfilepath/$nshfilename.script");
        }
        rmdir($tmpfilepath) || die "SH2NSH: can't clean up $!";
    }
}
...
sub known_missing{
    ...
    my $fn = "$configfilepath/$dbfn";
    &new_empty(2);
    $debug && print "SH2NSH: known missing..reading the missing from $dbfilename.\n";
    if($USEGRAYCODE) {
        &read_db_as_graycode($fn) && &build_nsh_from_graycode(1);      # 1 means missing
    } else {
        &build_nsh_from_missing($fn);    # no sorting
    }
}
...
sub calc_missing{
    ...
    if($dbszchk == 2) {
        ...
        $debug && print "SH2NSH: calc_missing is determining the missing..\n";
        my $mdbfn = "$tmpfilepath/missing.db";
        &read_db_sort_for_missing($dbfn,$mdbfn);
#        &read_db_solve_for_missing($dbfn,$mdbfn);

        &new_empty(2);
        if($USEGRAYCODE) {
            &read_db_as_graycode($mdbfn) && &build_nsh_from_graycode(1); # 1 means missing
        } else {
            &build_nsh_from_missing($mdbfn);    # no sorting
        }
    } else {
        &additive_way($dbfn);
    }
...
# creates a negative representation by "adding" each positive element
# from db to a negative database (v.13); uses graycode sorting (v.16)
sub additive_way {
    my $dbfn = $_[0];
    my $fn = "$configfilepath/$dbfn";
    ...
    &new_empty(0);
    if($USEGRAYCODE) {
        &read_db_as_graycode($fn) && &build_nsh_from_graycode(0);  # 0 means not missing
    } else {
        &read_db_and_build_nsh($dbfn);
    }
}


sub script_header_select {
    my $script = "$tmpfilepath/$nshfilename.script";
    open(NEGDBSCRIPT,">$script") || die "Unable to create negdb script $script\n";
    select NEGDBSCRIPT;    
    print "#script to create $configfilepath/$nshfilename\n";
    print "B1\n";
    print "Z1\n";
    print "L$LEN\n";
    print "K$MAX_KEY\n";
    print "D0$configfilepath\n";
    print "D1$configfilepath/$nshfilename\n";
    print "O\n";
    return 0;
}
...
sub script_run {
    ...
    `$cmd $stdoutput $stderrors`;
    if ($? == -1) {
        print "SH2NSH script_run failed to execute [$!]\n";
        $ans = -1;
    } else {
        $ans = $? >> 8; 
        if($ans == 0) {
            ($debug || $admin) && print("SH2NSH script_run completed\n");
            return $ans;
        }
        if($ans > 1) {
            print("SH2NSH script_run found error [$ans]\n");
        }
    }
    $SANITY && die "SH2NSH: script run <$script> failed, ans <$ans>\n";
    return $ans;
}
...
# make tmp dir for temporary ndb files
    $tmpfilepath = mkdtemp("$configfilepath/nshtmpSH2NSH_XXXXX");
    ($tmpfilepath =~ /undef/ ) && die "SH2NSH: fail to make temporary data directory\n"; 
    ...
    if($KNOWN_MISSING) {                      # negative length arg 
        ...
        &additive_way($dbfilename);
    }

    if($debug || $admin) {
        my $fsz = &dbsize_stat("$configfilepath/$nshfilename",$LEN);
        print "SH2NSH: final file size is <$fsz>\n";
    }

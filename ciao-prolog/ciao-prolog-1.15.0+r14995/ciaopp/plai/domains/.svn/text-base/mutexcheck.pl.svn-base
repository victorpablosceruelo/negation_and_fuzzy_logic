...
:- use_module(infernf(nfsets), [generate_type_annotated_term/4]).

:- use_module(infernf(cover), 
...
:- use_module(infernf(subproblems), [integer_covers/2]).
...
solve_meta_tests([fail], true):- !.
solve_meta_tests(Meta_Tests, Res):-
    test_list_to_goal(Meta_Tests, Goal),
    ( \+ call(Goal) -> 
          Res = true
     ;
          Res = false
...
% Replaced clpr_call/1 by clp_meta/1 -JFMC

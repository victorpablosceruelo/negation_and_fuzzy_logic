...
:- use_module(plai(fixpo_ops), [fixpoint_id_reuse_prev_success/6, 
...
:- use_module(plai(trust), [apply_trusted0/7, cleanup_trusts/1]).
:- use_module(plai(fixpo_dx_check_basic)).
	...
	-> 
	 ...
	 ; % for the trusts, if any:
	   ...
	   (
	       ...
	       asserta_fact(complete_parent(Id,Fs))       % for fixpo_di
	   ;

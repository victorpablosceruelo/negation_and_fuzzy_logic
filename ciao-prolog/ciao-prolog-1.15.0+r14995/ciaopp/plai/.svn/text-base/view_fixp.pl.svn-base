...
:- use_module(plai(domains), [call_to_entry/9, project/5]).
:- use_module(plai(trace_fixp), [trace_fixp/1]).

/* =======================================================================
   This module is used to display the analysis graph computed during the
   fixpoint. It uses the daVinci product, with which it communicates by
   the standard input and ouput (SICStus3 exec system call is used).
   ...
   Id-Key uniquely identifies a node:
      - for a goal-node which corresponds to a fixpoint node,
      ...
      - for a goal-node which is not yet a fixpoint node,
   ...
   ASub is
      ...
      - for a clause-node, the entry substitution, which may contain 
            the (existential) variables of the body
   A is (Ins,Outs) where:
      - Ins is an open ended list of ASub's
      - Outs is an open ended list of projected success substitutions 
...
view_fixpoint('visit goal',Id0,ClKey,SgKey0,Sg,[Call],AbsInt):- !,
	varset(Sg,Sv),
	project(AbsInt,Sv,Sv,Call,Proj),
	Node=..[SgKey0,Id0],
...
view_fixpoint('exit goal',NodeId,PId,(GoalKey,BodyKey),Sg,[Succ],AbsInt):- !,
	...
	varset(Sg,Sv),
	project(AbsInt,Sv,Sv,Succ,Prime),

...
:- head_cost(ub,res_steps,'reverse_2:f_res_steps').

:- literal_cost(ub,res_steps,0).

:- head_cost(ub,res_steps_1,'reverse_2:f_res_steps_1').
...
:- true pred reverse(X,Y)
        ...
        => ( list(X,num), list(Y,num), size(ub,X,length(X)), size(ub,Y,length(X)) )
         + ( cost(ub,giunif,0.5*exp(length(X),2)+3.5*length(X)+1), cost(ub,gounif,0.5*exp(length(X),2)-0.5*length(X)+1), cost(ub,nargs,1.5*exp(length(X),2)+3.5*length(X)+2), cost(ub,res_steps,0.5*exp(length(X),2)+1.5*length(X)+1), cost(ub,res_steps_1,0.5*exp(length(X),2)+1.5*length(X)+1), cost(ub,steps,0.5*exp(length(X),2)+1.5*length(X)+1), cost(ub,viunif,1.5*exp(length(X),2)+1.5*length(X)), cost(ub,vounif,exp(length(X),2)+length(X)) ).
...
:- true pred concat(X,Y,Z)
        ...
        => ( list(X,num), rt19(Y), rt19(Z), size(ub,X,length(X)), size(ub,Y,length(Y)), size(ub,Z,length(Y)+length(X)) )
         + ( cost(ub,giunif,length(X)+1), cost(ub,gounif,length(X)), cost(ub,nargs,3*length(X)+3), cost(ub,res_steps,length(X)+1), cost(ub,res_steps_1,length(X)+1), cost(ub,steps,length(X)+1), cost(ub,viunif,3*length(X)+1), cost(ub,vounif,2*length(X)+1) ).

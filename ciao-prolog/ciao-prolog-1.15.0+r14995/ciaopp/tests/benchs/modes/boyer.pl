...
:- module(boyer,[ tautology/1 ],[ assertions ]).
:- use_module(library(prolog_sys), [statistics/2]).

:- entry tautology(X) : ground(X).

goal :- wff(X), tautology(X).
...
truep(t,_) :- !.
truep(Wff,Tlist) :- boyer:member(Wff,Tlist).

falsep(f,_) :- !.
falsep(Wff,Flist) :- boyer:member(Wff,Flist).

member(X,[X|_]) :- !.
member(X,[_|T]) :- boyer:member(X,T).

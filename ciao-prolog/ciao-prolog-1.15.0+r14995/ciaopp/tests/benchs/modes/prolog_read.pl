...
:- module(prolog_read,[parse/2],[assertions]).

:- use_module(library(lists)).
:- use_module(library(old_database)).
:- use_module(library(ttyout)).

:- entry parse(X,Y) : ( var(Y), ground(X) ).

goal :- parse([some, tokens, here], _ParseTree).

:- data current_op/3.
:- trust success current_op/3 => ground * ground * ground.

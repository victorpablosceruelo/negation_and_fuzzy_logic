...
%-------------------------------------------------------------------%
make_atom_list([],[]).
make_atom_list([(N/A)|Preds],[Key|Keys]):-
        make_atom([N,A],Key),
        make_atom_list(Preds,Keys).
...
:- doc(version(1*0+544,2004/07/14,19:54*10+'UTC'), "Reimplemented
   @pred{next_or_last_key} so that it behaves correctly in the
   presence of cut literals without keys and without making any
   assumption on the internal representation of keys.  (German
   Puebla)").

:- doc(version(1*0+503,2004/07/02,13:35*08+'CEST'), "changed
   next_or_last_key([A|_],K,K1) (David Trallero Mena)").

:- doc(version(1*0+216,2004/01/18,20:37*40+'CET'), "Predicates
   @pred{list_format/2} and @pred{newformat/2} now live here instead
   of in module spec.  (German Puebla)").

:- doc(version(1*0+35,2003/08/27,12:37*16+'CEST'), "make_atom/2
   now lives in spec(s_simpspec).  (Francisco Bueno Carrillo)").

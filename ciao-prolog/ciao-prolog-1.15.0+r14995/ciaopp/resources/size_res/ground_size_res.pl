...
:- test ground_term_size(A, B, C):(A= size, B= f(a, b, c(-5, 3, 4), b))
	=>C=8.
:- test ground_term_size(A, B, C):(A= size,
	    B= f(a, b, c(-5, 3, 4), b(_))) =>C= bot.
:- test ground_term_size(A, B, C):(A= depth([1, 2]), B= f(a(b(c)),
		c(-5, 3, 4), b(_))) =>C=3.


:- pred ground_term_size(+, ?, -) :: atm * term * term #
...
:- pred ground_int(?, -) # "Determine the size of a ground
...
:- test ground_depth(A, B, C):(A = [1, 2],
	    B = f(a(b), c(d(e)), f(g, h))) => C = 3.
:- test ground_depth(A, B, C):(A = [1, 2],
	    B = f(a, _, c(d(e)), f(_, h))) => C = 1.
:- test ground_depth(A, B, C):(A = [1, 2],
	    B = f(_, _, c(d(e)), f(_, h))) => C = bot.
:- test ground_depth(A, B, C):(A = [1, 2],
	    B = f(_, a(b), c(d(e)), f(_, h))) => C = 2.
...
:- test ground_depth_(A, B, C, D):(A = [1, 2], B = 4,
	    C = f(a, E, c(d(e)), f(F, h))) => D = 0.
:- test ground_depth_(A, B, C, D):(A = [1, 2], B = 4,
	    C = f(E, a(b), c(d(e)), f(F, h))) => D = 1.

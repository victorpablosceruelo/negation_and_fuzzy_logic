:- module(sumprod_res, [sum_expr/5, prod_expr/5],
...
:- pred sum_expr/5 + not_fails.
% rtcheck -- EMM
sum_expr(Var, Low, Upper, Expr, Sum) :-
	sum_expr_(Var, Low, Upper, Expr, Sum),
	!.

sum_expr_(_, _, _,   bot,  bot).
sum_expr_(_, _, bot, Expr, bot) :-
	Expr \== bot.
sum_expr_(_, bot, Upper, Expr, bot) :-
	Expr \== bot,
	Upper \== bot.
sum_expr_(_, Lower, Upper, inf, inf) :-
	Upper \== bot,
	Lower \== bot.
sum_expr_(_, Lower, inf, Expr, inf) :-
	Expr \== bot,
	Expr \== inf,
	Lower \== bot.
sum_expr_(_, inf, Upper, Expr, bot) :-
	Expr \== bot,
	Expr \== inf,
	Upper \== bot,
	Upper \== inf.
sum_expr_(Var, Low, Upper, expr(T, F), Sum) :-
...
sum_term(Var, Low, Upper, T, Sum) :-
	normal_form(Var, Var1),
	Sum = expr([term([sum(Var1, Low, Upper, expr([T], []))],
		    [factor([], 1)])], []).
...
poly_items_coeff([I|Is], Var, Coeff, Exp) :-
	( I = exp(E1, E2), normal_form(Var, E1), general_form(E2, E),
	    integer(E) -> ( Coeff = NCoeff,
		Exp = E2 ) ;
	    ( Coeff = [I|NCoeff],
		Exp = Exp1 ) ),
...
exp_factor_coeff([factor(I, C)], Var, [factor(Coeff, C)], Base,
	    ExpCoe, ExpCon) :-
	exp_items_coeff(I, Var, Coeff, Base, ExpCoe, ExpCon).
*/
...
exp_items_coeff([I|Is], Var, Coeff, Base, ExpCoe, ExpCon) :-
	( I = exp(E1, E2), rel_const_expr(Var, E1), rel_linear_expr(Var,
		E2) ->
	    ( Base = E1,

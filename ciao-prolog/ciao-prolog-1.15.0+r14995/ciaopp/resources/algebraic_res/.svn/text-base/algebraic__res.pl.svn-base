...
:- use_module(resources(algebraic_res(arithm_opers_res)), [pow/3, log2/2]).
...
add_factor([factor(F1, C1)|Fs], factor(F2, C2), NF) :-
	...
	( C =:= 0 ->
	    NF = Fs;
	    NF = [factor(F1, C)|Fs] ).
add_factor([factor(F1, C1)|Fs], factor(F2, C2), [factor(F2, C2),
		factor(F1, C1)|Fs]) :-
	F1 @< F2,
	!.
...
resource_multiply_expr([],     _, []).
resource_multiply_expr([X|Xs], F, [Y|Ys]) :-
	multiply_expr(X, F, Y),
...
multiply_primary([exp(P1, C1)|Ps], exp(P1, C2), 1, [exp(P1, C)|Ps]) :-
	add_expr(C1, C2, C).
multiply_primary([P1|Ps], exp(Expr, Exp), 1,
	    [exp(Expr, NExp)|Ps]) :-
	Expr = expr([P1], [factor([], 1)]),
	normal_form(1, One),
	add_expr(One, Exp, NExp).
multiply_primary([exp(Expr, Exp)|Ps], P1, 1,
	    [exp(Expr, NExp)|Ps]) :-
	Expr = expr([P1], [factor([], 1)]),
	normal_form(1, One),
	add_expr(One, Exp, NExp).
...
multiply_item([I1|Is], I1, 1, [exp(expr([], [factor([I1], 1)]),
	...
	functor(I1, F, N),
	(F, N) \== (exp, 2).
multiply_item([exp(I1, C1)|Is], exp(I1, C2), 1,
	    [exp(I1, C)|Is]) :-
	add_expr(C1, C2, C).
...
multiply_item([exp(I1, C1)|Is1], exp(I2, C2), Done,
	    [exp(I1, C1)|Is]) :-
	I1 \== I2,
...
multiply_item([exp(Expr, Exp)|Is1], I1, Done,
	    [exp(Expr, Exp)|Is]) :-
	Expr \== expr([], [factor([I1], 1)]),

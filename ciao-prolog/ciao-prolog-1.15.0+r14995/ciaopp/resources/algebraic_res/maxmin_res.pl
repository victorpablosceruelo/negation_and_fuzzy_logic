:- module(maxmin_res, [max_expr/3,
		min_expr/3,
		positive_expr/1, %% Added by JNL (it could be temporary)
		negative_expr/1, %% Added by JNL (it could be temporary)
		max_term_by_term/3 %% Added by JNL (it could be temporary)
	    ], [assertions]).
...
%
%max_expr(minus_inf,E,E) :- !.
%max_expr(E,minus_inf,E) :- !.
...
%  Perform the maximum function in normal-form.
%
%min_expr(minus_inf,_,minus_inf) :- !.
%min_expr(_,minus_inf,minus_inf) :- !.

...
solve_esde1(Var, Bn, GIval, Sol) :- Bn \== [], solve_esde2(Bn, Var, GIval,
	    Sol).
...
solve_esde3(factor([arg(_, _)], C), Var, GIval, Sol) :-
% 	an approx. same as d*x
	!, normal_form(((max(C, GIval) /2) *Var*(Var +1)), Sol).
solve_esde3(factor([exp(E1, E2)], C), Var, GIval, Sol) :- % d*2^x
	normal_form(Var, E2), general_form(E1, I), integer(I), !,
	( number(GIval) ->
	    normal_form(max(C, GIval)*(exp(I, Var +1) -1)/(I -1),
		Sol) ;
	    normal_form(C*(exp(I, Var +1) -1)/(I -1)+(GIval*Var),
		Sol) ).
solve_esde3(factor([exp(E1, E2)], C), Var, GIval, Sol) :-
% 	d*i^arg(x,n)
...
explicit_size_solvable1(factor([arg(Expr1, Expr2)], _), Var) :-
% 	arg(x,i)
...
explicit_size_solvable1(factor([exp(Expr1, Expr2)], _), Var) :-
%	d*i^arg(x,n)

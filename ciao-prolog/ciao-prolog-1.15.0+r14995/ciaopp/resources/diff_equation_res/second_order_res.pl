...
second_order_con_diff_equ(Var, Ivalue, A1, A2, B, R1, R2, A1n, A2n, Bn) :-
	...
	Bn = expr([], B),
	check_base_cases(Ivalue, Iindex1, Iindex2),
...
check_base_cases(Ivalue, Iindex1, Iindex2) :-
	Ivalue = [val(Iindex1, _), val(Iindex2, _)|_],
	length(Ivalue, N),
	warning_message("Solving a linear second-order diff. equation"),
	warning_message("Only two base cases are required. There are ~q", [N]),
	!.
check_base_cases(Ivalue, _Iindex1, _Iindex2) :-
	length(Ivalue, N),
	Missing is 2 - N,
	warning_message("Solving a linear second-order diff. equation"),
	warning_message("Two base cases are required. There are only ~q", [
		Missing]),
	!,
	fail.
...
solve_sode(Var, Ivalue, A1, A2, B, Sol) :-
	second_order_con_diff_equ(Var, Ivalue, A1, A2, B, R1, R2, A1n, A2n,
	    Bn),
	!,
...
*/
		...
		normal_form((C/(2*(A1 +2 *A2))) * exp(Var, 2) +
		    ((C*(A1 +4 *A2))/(2 * exp(A1 +2 *A2, 2))) *Var,
		    Sol) ;
		normal_form((C/6) * exp(Var, 3)+(C/2) * exp(Var, 2),
		    Sol) ) ).
second_order_par_sol1(factor([exp(E1, E2)], C), Var, R1, R2, A1, A2, Sol
	...
	( U =:= 0 ->
		...
		normal_form((C/(3*(A1 +2 *A2))) * exp(Var, 3) +
		    ((C*(A1 +4 *A2))/(2 * exp(A1 +2 *A2, 2))) * exp(
			Var, 2) +
		    ( (C*(A1*A1 +4 *A1*A2 +16 *A2*A2))/(6 * exp(A1 +2 *A2,
				3)) ) *
		    Var, Sol) ;
		normal_form((C/12) * exp(Var, 4)+(C/3) * exp(Var, 3) +
		    (5 *C/12) * exp(Var, 2), Sol) ) ).
second_order_par_sol1(factor([exp(E1, E2)], C), Var, R1, R2, A1, A2,
	    Sol) :-
	E1 = expr([], [factor([], D)]),
	normal_form(Var, E2),
	R1 =\= D,
	R2 =\= D,
	normal_form(((C* exp(D, 2))/(exp(D, 2) -A1*D-A2)) * exp(D,
		Var), Sol).
...
second_order_gen_sol(Var, Ivalue, R1, R2, PSol, GSol) :-
	R1 =\= R2,
	Ivalue = [val(Iindex1, Ival1), val(Iindex2, Ival2)|_],
	...
	N2 = exp(R1, Index1)*(Val2-GPSol2) - exp(R1, Index2) *
	(Val1-GPSol1),
	D2 = exp(R2, Index2) * exp(R1, Index1) - exp(R2, Index1) * exp(R1,
	    Index2),
	C2 = N2/D2,

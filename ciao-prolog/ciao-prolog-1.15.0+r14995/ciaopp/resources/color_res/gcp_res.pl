:- module(gcp_res, [gcp/4], [assertions]).
...
:- use_module(resources(top_res(utility_res)), [list/1, member/2]).
:- use_module(resources(color_res(cslpoly_res)), [csl_poly/4]).

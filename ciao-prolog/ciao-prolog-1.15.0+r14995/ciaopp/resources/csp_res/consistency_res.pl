...
:- use_module(resources(csp_res(csp__res)), 
	    [
		find_domain_entry/3,
...
build_consistency_graph([V|Vs], Interval, Domain, Constraint,
	    [cgraph(V, Edge)|Gs]) :-
...
build_consistency_edges([T|Vs], H, Interval, Domain, Constraint,
	    [cvar(T, Edge)|Es]) :-
...
%
build_consistency_edge(T, H, Interval, Domain, Constraint, Edge) :-
	canonical_binary_constraints(Constraint, T, H, Interval,
	    BConstraint),
...
binary_constraint(E1*E2, T, H, Domain, Texpr, Hexpr, dv(C1, C2)) :-
	binary_constraint(E1, T, H, Domain, T1, H1, dv(C11, C12)),
	binary_constraint(E2, T, H, Domain, T2, H2, dv(C21, C22)),
	constraint_term(T1, H1, C11, C12, T2, H2, C21, C22, Texpr, Hexpr, C1,
	    C2).
...
consistency_edges([V|HInterval], TInterval, BConstraint, H,
	    [cedge(V, WEdge)|Es]) :-
...
atom_consistent_values(bc(Op, A, Hexpr, dv(Con1, Con2)), H, V, DInterval,
	    Edge) :-

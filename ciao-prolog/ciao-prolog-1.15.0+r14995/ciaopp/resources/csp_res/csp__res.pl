...
:- use_module(resources(algebraic_res(simpl_form_res)), 
	    [simplification/2]).
:- use_module(resources(csp_res(consistency_res)), 
	    [
...
:- use_module(resources(csp_res(clique_res)), [
		number_n_cliques/4]).
...
%write('rename_variables'),nl,
	rename_variables(Vars, 1, NVars, Domain, NDomain, Constraint,
	    NConstraint),
...
%
rename_variables([], _, [], _, [], Constraint,
	    Constraint).
rename_variables([V|Vars], N, [$(N)|NVars], [L-U|Domain], [ND|NDomain
	    ], Constraint,
	    NConstraint) :-
	ND = domain($(N), L, U),
	substitute(Constraint, V, $(N), Constraint1),
	N1 is N +1,
	rename_variables(Vars, N1, NVars, Domain, NDomain, Constraint1,
	    NConstraint).
...
%
node_consistency([], _, [], Constraint,
	    Constraint).
node_consistency([V|Vars], Domain, [domain(V, NInterval)|NDomain],
	    Constraint,
	    NConstraint) :-

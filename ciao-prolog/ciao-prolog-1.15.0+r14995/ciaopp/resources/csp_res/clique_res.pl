:- module(clique_res, [number_n_cliques/4], [assertions]).
...
:- use_module(resources(algebraic_res(arithm_opers_res)), [min/3]).
%
...
number_n_cliques1([V|Vars], [domain(V, D)|Domain],
	    [cgraph(V, CV)|Graph], Sols) :-
...
adjacent_edges2(M,  [], _, []) :- M
	\== [], !.
adjacent_edges2([weight(M1, W1)|M], [weight(E1, W2)|E], W,
	    [weight(E1, W3)|AE]) :-
	M1 =:= E1, !,
...
graph_addition([cgraph(V, V1)|G1], [cgraph(V, V2)|G2],
	    [cgraph(V, V3)|G3]) :-
...
edges_addition([cedge(V1, E1)|C1], [cedge(V2, E2)|C2],
	    [cedge(V1, E3)|C3]) :-
	V1 =:= V2, !,
...
edge_addition(E,  [], E) :- E \==
	[], !.
edge_addition([weight(V1, W1)|E1], [weight(V2, W2)|E2],
	    [weight(V1, W3)|E3]) :-
	V1 =:= V2, !,
	W3 is W1+W2,
	edge_addition(E1, E2, E3).

:- module(field_value_check,[fieldValue_check/8,
	                     %count_numbers/2,
	                     mfstringValue/7],[iso,dcg]).
...
:- use_module(io, [out/3]).
:- use_module(generator_util, [indentation_list/2, 
	                      remove_comments/4]).
:- use_module(boundary, [boundary_check/3, 
	                boundary_rotation_first/2,
			boundary_rotation_last/2]).
:- use_module(tokeniser, [token_read/3]).
:- use_module(generator, [nodeDeclaration/4]).
:- use_module(parser_util, [create_parse_structure/3, 
	                   inc_indentation/2,
			   dec_indentation/2,
			   get_parsed/2,
			   %push_whitespace/3,
...
%%%%%%%%%%%%%%%%
count_numbers([],0).
count_numbers([_Number|Rest],In) :-
	count_numbers(Rest,New),
	In is New + 1.

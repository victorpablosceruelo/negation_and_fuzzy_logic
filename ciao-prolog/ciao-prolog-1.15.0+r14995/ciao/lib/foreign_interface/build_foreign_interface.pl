...
:- use_module(library(compiler(engine_path))).
...
% todo: to avoid name clashes we could move back one directory and have everything
% in ciao/ so that we do #include <ciao/gluecode.h>, etc.
imports --> [include('ciao_gluecode.h'), format(new_line)].
...
%	format(user_error, "[trace-dir] ~w~n", [Dir]),
	compile_and_link_2(Dir, Decls, CFiles, OFiles, SOFile).
...
compile_and_link_2(Dir, Decls, CFiles, OFiles, SOFile) :-
	...
	link_foreign(Dir, Decls, ExtraLinkerOpts, Libs, OFiles, SOFile), 
	!.
compile_and_link_2(_, _, _, OFiles, SOFile) :-
	delete_files([SOFile|OFiles]), 
	fail.
...
get_foreign_files__2(Dir, Base, Decls, FFiles, OFiles, SOFile, UniqueOFile) :-
...
absolute_base_names(Dir, [File|Files], [AbsFile|AbsFiles], [AbsBase|AbsBases]) :- 
	...
	(
	    AbsBase1 = AbsFile ->
	    filenames:basename(AbsFile, AbsBase)
	;
	    AbsBase = AbsBase1
	), 
	absolute_base_names(Dir, Files, AbsFiles, AbsBases).
...
add_ciaolib_if_required(L0, L) :-
	get_eng_location('dyn'),
	!,
	add_ciaolib(L0, L).
...
%	    format(user_error, "[trace - cuo] ~w~n", Command),
	    system(Command, 0)
	; true
	), 
	!.

...
:- doc(get_alias_path, "Consult the environment variable
   'CIAOALIASPATH' and add facts to predicates
   @pred{library_directory/1} and @pred{file_search_path/2} to define
   new library paths and @concept{path alias}es.  The format of
   'CIAOALIASPATH' is a sequence of paths or alias assignments separated
   by colons, an alias assignment is the name of the alias, an '=' and
   the path represented by that alias (no blanks allowed).  For example,
   given
@begin{verbatim}

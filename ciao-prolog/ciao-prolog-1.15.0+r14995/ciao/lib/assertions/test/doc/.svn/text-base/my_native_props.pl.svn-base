...
:- doc(indep(X), "The variables in the list @tt{@var{X}} are
...
:- doc(not_fail(X), "Calls of the form @var{X} produce at least
...
:- doc(possible_fail(X), "Non-failure is not ensured for any call
...
:- doc(possible_nondet(X), "Non-determinism is not ensured for all
...
:- doc(disjoint(X), "For any call of the form @var{X} at most one
...
:- doc(not_disjoint(X), "Not for all calls of the form @var{X} at
...
:- doc(lower_size(X, Y), "The minimum size of the terms to which
...
:- doc(upper_size(X, Y), "The maximum size of the terms to which
the argument @var{Y} is bound to is given by the expression
@var{Y}. Various measures can be used to determine the size of an
argument, e.g., list-length, term-size, term-depth, integer-value,
etc. @cite{caslog}.").
...
:- doc(lower_time(X, Y), "The minimum computation time (in
...
:- doc(upper_time(X, Y), "The maximum computation time (in
resolution steps) spent by any call of the form @var{X} is given by

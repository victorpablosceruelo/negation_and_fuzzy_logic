...
%% ---------------------------------------------------------------------------
:- pred set_libs(go(OldLibs),go(NewLibs)) :: list(atm) * list(atm) 
...
%%% This is the original version, but something is wrong in 1.13 and 
%%% retract(library_directory(D) fails. Had to add the retractall.
 	findall(D,retract(library_directory(D)),OldLibs),
%% 	findall(D,library_directory(D),OldLibs),

...
bool_t stack_shift_usage(Arg)
     Argdecl;
{
  tagged_t x;
  ENG_LINT time = (ciao_statistics.ss_tick*1000)/
     ...
     int pad, arity;
{
  ...
  if (!b->next_alt) {			/* try */
    ...
    if (i>0){
      CIAO_REGISTER tagged_t *t = (tagged_t *)w->next_node;
...
bool_t is_rem_Hterm(term,w,remote_w)
     ...
     Argdecl;
{
  ...
  newh = checkrealloc(Stack_Start,
		      count*sizeof(tagged_t)/2,
		      count*sizeof(tagged_t));
#if defined(DEBUG)
  if (debug_gc)
...
void stack_overflow_adjust_wam(w,reloc_factor)
     worker_t *w;
     int reloc_factor;
{
  CIAO_REGISTER node_t *n, *n2;
  CIAO_REGISTER tagged_t t1;
  CIAO_REGISTER tagged_t *pt1;

  if (reloc_factor!=0) {
    node_t *aux_node;
    CIAO_REGISTER frame_t *frame;
    int i;
...
#if defined(ANDPARALLEL)
void heap_overflow_adjust_wam(w,reloc_factor,newh,rem_reloc,rem_w)
     worker_t *w;
     int reloc_factor;
     tagged_t *newh;
     bool_t rem_reloc;
...
{
#else
void heap_overflow_adjust_wam(w,reloc_factor,newh)
     worker_t *w;
     int reloc_factor;
     tagged_t *newh;
{
#endif
  CIAO_REGISTER node_t *n, *n2 = NULL;
  CIAO_REGISTER tagged_t t1;
  CIAO_REGISTER tagged_t *pt1;
     ...
     Argdecl;
{
  ...
  while (!OffChoicetop(Gc_Choice_Start,b)) {
    ...
    while (!OffTrailtop(x,tr)){
      CIAO_REGISTER tagged_t t1 /*, *pt */ ; /* unused */

...
bool_t prolog_erase(Arg)
     Argdecl;
{
  CIAO_REGISTER instance_t *node;
  int_info_t *root;
  ENG_INT current_mem;
...
bool_t inserta(Arg)
     Argdecl;
{
    CIAO_REGISTER instance_t *n, **loc;
     ...
     bcp_t p;
{
  ...
  CIAO_REGISTER tagged_t *tp = TagToSTR(t);
  CIAO_REGISTER tagged_t *pp = (tagged_t *)p;
...
bool_t make_bytecode_object(Arg)
     Argdecl;
{
  tagged_t num,list;
  emul_info_t *object;
  ...
  lsize = (sizeof(emul_info_t)-ANY*sizeof(insn_t)+GetInteger(num)+
	   counter_cnt*sizeof(ENG_INT)+3) & ~3;
#else
  lsize = sizeof(emul_info_t)-ANY*sizeof(insn_t)+GetInteger(num);
#endif

  object=(emul_info_t *)checkalloc(lsize);
    ...
    case STR:
      {
        ...
        if(func==functor_functor) {
          ...
          if (TagIsSTR(car) && (TagToHeadfunctor(car)==functor_slash)) {
            ...
            DerefArg(t1,car,1);
            DerefArg(t2,car,2);
            *(tagged_t *)code = SetArity(t1,GetSmall(t2));
            code += BPT;
          }
          break;
        }
...
instance_t *active_instance(Arg,i,itime,normal)
     Argdecl;
     CIAO_REG_1(instance_t *, i);
     int itime;
     bool_t normal;  /* if normal==TRUE follow first-chain else next_forward */
{
  CIAO_REG_2(node_t *, b);
     ...
     Argdecl;
{
  ...
  if (HeapCharDifference(w->global_top,Heap_End) < count*sizeof(instance_clock_t))
    explicit_heap_overflow(Arg,(count*sizeof(instance_clock_t)+3)>>2,DynamicPreserved);
     ...
     instance_clock_t *clocks;
{
  ...
  CIAO_REGISTER int j = SwitchSize(sw);
...
void relocate_clocks(inst,clocks)
     CIAO_REGISTER instance_t *inst;
     CIAO_REGISTER instance_clock_t *clocks;
{
  CIAO_REGISTER int i, j;
  instance_t *next;
...
void expunge_instance(i)
    CIAO_REGISTER instance_t *i;
{
    CIAO_REGISTER instance_t **loc;
    CIAO_REGISTER int_info_t *root = i->root;
    ...
    checkdealloc((tagged_t *)i,i->objsize);
}

     ...
     char *module_name;
{
  ...
  if (all_loaded_objects){                              
    ...
    if (to_remove) {
      ...
      checkdealloc((tagged_t *)to_remove->module_name,
                   strlen(to_remove->module_name)+1);
      checkdealloc((tagged_t *)to_remove, sizeof(loaded_object));
    }
  }
#if defined(DEBUG)
  if (debug_c) 
...
void add_to_loaded_objects(module_name, handle, end_func)
     ...
     void (*end_func)(char *);
{
  loaded_object new_object = (loaded_object)checkalloc(sizeof(lobj_t));
...
bool_t prolog_dynunlink(Arg)                                        /* JFMC */
     Argdecl;
{
  ENG_TTYPRINTF0(

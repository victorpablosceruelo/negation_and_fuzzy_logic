...
case FUNCTION_1Q:
Numstack_End = NULL;
if (ERRORTAG==(Xb(P2) = (tagged_t)C4(Arg,Xb(P3),Pplus6)))
     goto fail;
DISPATCH_R(4+2*BPTP);
r_function_1:
case FUNCTION_1:
Numstack_End = NULL;
if (ERRORTAG==(Xb(P1) = (tagged_t)C3(Arg,Xb(P2),Pplus5)))
     goto fail;
DISPATCH_R(3+2*BPTP);
r_function_2q:
case FUNCTION_2Q:
Numstack_End = NULL;
if (ERRORTAG==(Xb(P2) = (tagged_t)C5(Arg,Xb(P3),Xb(P4),Pplus7)))
     goto fail;
DISPATCH_R(5+2*BPTP);
r_function_2:
case FUNCTION_2:
Numstack_End = NULL;
if (ERRORTAG==(Xb(P1) = (tagged_t)C4(Arg,Xb(P2),Xb(P3),Pplus6)))
     goto fail;
DISPATCH_R(4+2*BPTP);
r_builtin_1q:
case BUILTIN_1Q:
...
#if defined(PARBACK)
case RESTART_POINT:
Arg->global_top = TagToPointer(Arg->node->term[0]);
LoadH;
P = (bcp_t)CTagToPointer(Arg->node->term[0]);
w->next_insn = Arg->node->next_insn;
pop_choicept(Arg);
goto enter_predicate;
#endif

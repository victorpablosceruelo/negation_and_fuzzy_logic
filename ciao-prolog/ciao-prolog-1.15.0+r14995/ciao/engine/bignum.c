...
#define Getint(p) (((unsigned int)Getshort(p,1))+ \
                   (((unsigned int)Getshort(p,2))<<HalfUnit))

/* Only intended to be used outside this file -- e.g., by routines located
   at ciao_prolog.c */

ENG_INT bn_length(CIAO_REGISTER Bignum *x)
{
  return BignumLength(x);
}


bool_t bn_positive(x)
     CIAO_REGISTER Bignum *x;
{
  return BignumPos(x);
}


/* PREcondition_t: The result must fit in x.
   +x might occupy one more word than -x */
static void bn_negate(x)
     CIAO_REGISTER Bignum *x;
{
  CIAO_REGISTER int i, k;
...
int bn_add(x,y,z,zmax)
     CIAO_REGISTER Bignum *x, *y, *z;
     Bignum *zmax;
{
  CIAO_REGISTER int i;
  CIAO_REGISTER Bignum *w, xi, wi;
  unsigned long sign_extension; 
  int min, max;
  bool_t xs= BignumPos(x), ys= BignumPos(y);
...
int bn_lshift(x,dist,z,zmax)
     ...
     Bignum *zmax;
{
  CIAO_REGISTER Bignum xi;
  bool_t xs= BignumPos(x);
  int xlen = BignumLength(x);
  int shift = Getint(dist);
  int div, rem;
  CIAO_REGISTER int i;
     ...
     int xlen, ylen;
{
  ...
  CIAO_REGISTER unsigned short yj;
  ...
  for (j=1; j<=ylen; j++)
    if ((yj=Getshort(y,j)))
      {
	CIAO_REGISTER unsigned long t=0;
...
int bn_multiply(x,y,z,zmax)
     Bignum *x, *y, *z, *zmax;
{
  bool_t sx = BignumPos(x);
  bool_t sy = BignumPos(y);
  int xlen = BignumLength(x);
     ...
     Bignum *zmax;
{
  ...
  CIAO_REGISTER int i, j, k;
  unsigned long carry, v1qhat, v2qhat;
  CIAO_REGISTER unsigned short
		 *u,		/* dividend, size=ulen+1 */
                 *v,		/* divisor, size=vlen */
                 *q;		/* quotient, size=ulen-vlen+1 */
  CIAO_REGISTER unsigned short v1, v2, qj;
...
int bn_from_string(x,z,zmax,base)
     CIAO_REGISTER char *x;
     CIAO_REGISTER Bignum *z;
     Bignum *zmax;
     int base;
{
  bool_t sx;

...
#define USE_OC_EXCEPTIONS 1

/* Access macros for the principal WAM registers, bytecode offsets, etc. */

#define SIZEOFWORKER(X) (sizeof(worker_t) + ((X)-ANY)*sizeof(tagged_t))

/*

Macros CIAO_REG_n, used to declare a variable and its type, force the
GCC compiler to store a variable in a register.  In principle, they
are ranked in ascending order of effectiveness, according to empiric
measurements.  This can of course change if compilers (GCC, in this
case) change the way registers are assigned, and it makes sense to
have an order because there are processors whose instruction set is
not orthogonal to register names.  n currently goes from 1 to 4, but
it can be enlarged for additional architectures.  CIAO_REG_Z just
gives the compiler the hint that some variable has been stored in a
register.

Undefined the macro MAP_TO_HW_REGISTERS to disable this mapping a fall
back to what the compiler thinks is reasonable.

Register assignments (for registers 1 to 3) have been made for i386.

*/

#define MAP_TO_HW_REGISTERS

// #define_ CIAO_REGISTER
#define CIAO_REGISTER register


#define CIAO_REG_Z(Type, Var) CIAO_REGISTER Type Var

#if defined(__GNUC__) && defined(MAP_TO_HW_REGISTERS) 
# if defined(i86) && (defined(LINUX) || defined(BSD))
...
#  define CIAO_REG_2(Type, Var) CIAO_REG_Z(Type, Var)
#  define CIAO_REG_3(Type, Var) CIAO_REG_Z(Type, Var)
#  define CIAO_REG_4(Type, Var) CIAO_REG_Z(Type, Var)
# else
/* Default register definitions - leave it to the compiler */
#  define CIAO_REG_1(Type, Var) CIAO_REG_Z(Type, Var)
#  define CIAO_REG_2(Type, Var) CIAO_REG_Z(Type, Var)
#  define CIAO_REG_3(Type, Var) CIAO_REG_Z(Type, Var)
#  define CIAO_REG_4(Type, Var) CIAO_REG_Z(Type, Var)
# endif
#else
/* Same as above: default register definitions - leave it to the compiler */
# define CIAO_REG_1(Type, Var) CIAO_REG_Z(Type, Var)
# define CIAO_REG_2(Type, Var) CIAO_REG_Z(Type, Var)
# define CIAO_REG_3(Type, Var) CIAO_REG_Z(Type, Var)
# define CIAO_REG_4(Type, Var) CIAO_REG_Z(Type, Var)
...
#define Arg w
#define Argdecl \
	CIAO_REGISTER worker_t *w
#define WAMENV \
	Argdecl = self

#define WToX0		(sizeof(worker_t)/sizeof(tagged_t)-ANY)
...
#define InitialValueTrail (-(sizeof(node_t)/sizeof(tagged_t)-ANY))

#define EToY0		(sizeof(frame_t)/sizeof(tagged_t)-ANY)
#define Yb(I)		(*CharOffset(E,I)) /* I as bytecode operand */
#define Y(I)		(E->term[I]) /* I as zero-based */

/* for tracing: */
#define X_OffsetToIndex(O)	((O)/sizeof(tagged_t)-WToX0)
#define Y_OffsetToIndex(O)	((O)/sizeof(tagged_t)-EToY0)
...
// Value types for controlling agents execution when expanding stack sets
#define RELEASED                          0
#define TOSUSPEND                         1
#define SUSPENDED                         2
#define WAITING                           3
...
/* #if defined(Solaris) */
#if defined(DARWIN) || defined(LINUX) || defined(IRIX) || defined(Solaris)|| defined(Win32) || defined(BSD)
#undef  USCLK_EXISTS
#define NSCLK_EXISTS
#define TIME_INIT   
...
#undef  NSCLK_EXISTS
#define TIME_INIT
#define TIME  ((usertime_visandor() - time_at_event_start)*1.0e6)
#endif
 ...
 { \
   if ( gen_event_file ) { \
     if ( (TmpEvPtr(w) = (NextEvent(w))++) == LastEvent(w) ) { \
          fprintf(stderr, "Agent %x event table overflow.\n", \
                  (unsigned long int) w); \
          gen_event_file = FALSE; \
     } \
     else { \
          TmpEvPtr(w)->timestamp = (unsigned long) TIME; \

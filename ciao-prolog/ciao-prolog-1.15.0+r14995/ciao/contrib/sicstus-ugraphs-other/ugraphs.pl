...
%   one way around.  The cost is O(N^2).
...
%   edge (u,v) in Graph, add its symmetric edge (v,u).  Takes O(N^2)
...
top_sort(Graph, Sorted) :-
	vertices(Graph, Vs),
	top_sort(Vs, Graph, [], [], _, Reverse, []),
	reverse(Reverse, Sorted).

top_sort([], _, _, Total, Total) --> [].
top_sort([V|Vs], Graph, Above, Total0, Total) -->
	{ord_union(Total0, [V], Total1, New)},
	(   {New == []} -> {Total1 = Total2}
	;   {ord_add_element(Above, V, Above1)},
	    {neighbors(V, Graph, Neibs)},
	    {ord_disjoint(Neibs, Above1)},
	    top_sort(Neibs, Graph, Above1, Total1, Total2), [V]
	),
	top_sort(Vs, Graph, Above, Total2, Total).
...
%   of cost Cost from V1 to V2 in Graph.  Takes O(N^2) time.

min_path(Initial, Final, Graph, Path, Cost) :-
	min_paths(Initial, Graph, Tree),
	min_path([Initial], Final, Tree, Path, 0, Cost), !.

min_path([V|_], Final, _, [Final], Cost, Cost) :- V==Final.
min_path([V|_], Final, Tree, [V|Path], Cost0, Cost) :-
	neighbors(V, Tree, Neibs),
	Cost1 is Cost0+1,
	min_path(Neibs, Final, Tree, Path, Cost1, Cost).
min_path([_|Vs], Final, Tree, Path, Cost0, Cost) :-
...
%   each vertex has a path to every other vertex. Algorithm due to
%   Aho.

reduce(Graph, Reduced) :-
	strong_components(Graph, Comps, Map),
	reduced_edges(Graph, Map, Edges, []),
	vertices_edges_to_ugraph(Comps, Edges, Reduced).

strong_components(Graph, Comps, Map) :-
	vertices(Graph, Vs),
	strong_vertices(Vs, Graph, [], _, Vertices, []),
	reverse(Vertices, Vertices1),
	transpose(Graph, Graph1),
	strong_components(Vertices1, Graph1, [], Comps),
	component_map(Comps, Keylist),
	ord_list_to_assoc(Keylist, Map).

component_map([], []).
component_map([C|Cs], Map) :- component_map(C, C, Cs, Map).

component_map([], _, Cs, Map) :- component_map(Cs, Map).
component_map([V|Vs], C, Cs, [V-C|Map]) :- component_map(Vs, C, Cs, Map).

strong_components([], _, _, []).
strong_components([V|Vs], Graph, Reach, [Comp|Comps]) :-
	ord_disjoint([V], Reach), !,
	reachable(V, Graph, Comp0),
	ord_union(Reach, Comp0, Reach1, Comp),
	strong_components(Vs, Graph, Reach1, Comps).
strong_components([_|Vs], Graph, Reach, Comps) :-
	strong_components(Vs, Graph, Reach, Comps).

strong_vertices([], _, R, R) --> [].
strong_vertices([V|Vs], Graph, Reach0, Reach) -->
	{ord_union(Reach0, [V], Reach1, New)},
	(   {New == []} -> {Reach1 = Reach2}
	;   {neighbors(V, Graph, Neibs)},
	    strong_vertices(Neibs, Graph, Reach1, Reach2), [V]
	),
	strong_vertices(Vs, Graph, Reach2, Reach).

reduced_edges([], _) --> [].
reduced_edges([V-Neibs|Graph], Map) -->
	{get_assoc(V, Map, V1)},
	{ord_subtract(Neibs, V1, Neibs1)},
	reduced_edges(Neibs1, V1, Map),
	reduced_edges(Graph, Map).

reduced_edges([], _, _) --> [].
reduced_edges([V|Vs], V1, Map) --> [V1-V2],
	{get_assoc(V, Map, V2)},
...
random_ugraph(P, N, Graph) :-
	...
	;   prolog:illarg(domain(float,0.0-1.0), random_ugraph(P,N,Graph), 1)
	),
	(   integer(N), N >= 0 -> true
	;   prolog:illarg(domain(integer,0-inf), random_ugraph(P,N,Graph), 2)
	),
...
clique(Graph, K, Clique) :-
	(   integer(K), K >= 0 -> true
	;   prolog:illarg(domain(integer,0-inf), clique(Graph,K,Clique), 2)
	),
	J is K-1,
...
coloring(Graph, K, Coloring) :-
	(   integer(K), K >= 0 -> true
	;   prolog:illarg(domain(integer,0-inf), coloring(Graph,K,Coloring), 2)
	),

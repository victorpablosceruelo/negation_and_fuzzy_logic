...
% Prolog meta-interpreter with explicit failure and choice points.
...
%   Ans takes values over {yes,no}.

% PE annotations
:- trust comp exec/3 : i_sig * pt_state * i_sig + pe_type.
:- regtype pt_state/1.
pt_state(st(PP,G,CPs,OutVars,Ans,NCPs)) :- 
	f_sig(PP),f_sig(G),f_sig(CPs),f_sig(OutVars),f_sig(Ans),f_sig(NCPs).

:- trust comp main/4 + (memo,bind_ins,sideff(free)).
...
exec(_,st(_PP,[],[],OutVs,yes,NCPs),st(_,_,_,OutVs,yes,NCPs)).
exec(_,st(_PP,[],[_CP|_CPs],OutVs,yes,NCPs),st(_,_,_,OutVs,yes,NCPs)).
exec(Args,st(pp(Pred/Ar,_,_),[],[CP|CPs],_,yes,TNCPs/0),Sf) :- 
	CP = pp(Pred/Ar,ClId,_Pt),
	build_fresh_args(Args,Args_p,_,OutVars_p),
	build_goal(Pred/Ar,ClId,Args_p,Goal),
	TNCPs_p is TNCPs - 1,
	exec(Args,st(CP,Goal,CPs,OutVars_p,yes,TNCPs_p/0),Sf).
exec(_,st(_PP,_,[],OVs,no,TNCPs/0),st(_,_,_,OVs,no,TNCPs/0)).
exec(Args,st(_,_,[CP|CPs],_,no,TNCPs/0),Sf) :- 
	CP = pp(Pred/Ar,ClId,_Pt),
	build_fresh_args(Args,Args_p,_,OutVars_p),
	build_goal(Pred/Ar,ClId,Args_p,Goal),
	TNCPs_p is TNCPs - 1,
	exec(Args,st(CP,Goal,CPs,OutVars_p,yes,TNCPs_p/0),Sf).
exec(Args,st(pp(Pred/Ar,ClId,Pt),[A|As],CPs,OutVars,yes,TNCPs/ENCPs),Sf) :-
	internal(A),
	functor(A,A_f,A_ar),
	A =..[A_f|A_args],
	next(Pt,Pt2),
	main(A_f/A_ar,A_args,Ans,ENCPs_p),
%	res_call(main(A_f/A_ar,A_args,Ans,ENCPs_p)), % should be main(...)
	TNCPs_p is TNCPs + ENCPs_p, ENCPs_pp is ENCPs + ENCPs_p,
	exec(Args,st(pp(Pred/Ar,ClId,Pt2),As,CPs,OutVars,Ans,TNCPs_p/ENCPs_pp),Sf).
exec(Args,st(pp(Pred/Ar,ClId,Pt),[A|As],CPs,OutVars,yes,NCPs),Sf) :-
	builtin(A),
	next(Pt,Pt2),
	run_builtin(pp(Pred/Ar,ClId,Pt),A,Ans), % I assume builtins are deterministic
	exec(Args,st(pp(Pred/Ar,ClId,Pt2),As,CPs,OutVars,Ans,NCPs),Sf).
...
build_initial_state(Pred/Ar,Args,OutVars,S0) :-
	...
	TNCPs is NCl - 1,
	S0 = st(pp(Pred/Ar,1,1),Goal,CPs,OutVars_p,yes,TNCPs/0).

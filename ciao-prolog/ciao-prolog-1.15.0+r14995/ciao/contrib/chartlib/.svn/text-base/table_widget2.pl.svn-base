:- module(table_widget2,[],[assertions,regtypes,isomodes]).

:- doc(author,"Isabel Mart@'{i}n Garc@'{i}a").


:- export(tablewidget2/4).
:- export(tablewidget2/5).
...
:- doc(module,"In addition to the features explained in the
   ...
   which the font weight for the elements placed in the first row is
   bold. The remaining elements are in medium weight font.
...
:- pred tablewidget2(+title,+header,+table,+footer).

:- doc(tablewidget2(Title,Header,ElementTable,Footer),"Shows a regular
...
tablewidget2(Title,Header,ElementTable,Footer):-
	valid_table(ElementTable,'tablewidget2/4'),
	new_interp(Interp),
...
:- pred tablewidget2(+title,+header,+table,+footer,+image).  

:- doc(tablewidget2(Title,Header,ElementTable,Footer,BackgroundImage),
	"This predicate and @pred{tablewidget2/4} are quite similar, except
	that in the already one defined you must set the background image.
...
tablewidget2(Title,Header,ElementTable,Footer,BackgroundImage):-
	valid_table(ElementTable,'tablewidget2/5'),
	new_interp(Interp),
	length(ElementTable,R),
	length_element(ElementTable,Columns),

:- module(table_widget4,[],[assertions,regtypes,isomodes]).

:- doc(author,"Isabel Mart@'{i}n Garc@'{i}a").


:- export(tablewidget4/4).
:- export(tablewidget4/5).

:- use_module(library(chartlib(genbar1)), [header/1, title/1, footer/1]).
:- use_module(library(chartlib(bltclass))). 
:- use_module(library(chartlib(table_widget1)), [table/1, image/1]).
:- use_module(library(chartlib(test_format))).
:- use_module(library(chartlib(install_utils))).
:- use_module(library(lists)).

:- doc(module,"In addition to the features explained in the
   introduction, predicates exported by this module display tables in
   which the font weight for the elements placed in the first row and
   column is bold. The remaining elements are in medium weight font.
...
:- pred tablewidget4(+title,+header,+table,+footer).

:- doc(tablewidget4(Title,Header,ElementTable,Footer),"Shows a regular
	table in a window. The system sets a default background image for
	the widget.
...
tablewidget4(Title,Header,ElementTable,Footer):-
	valid_table(ElementTable,'tablewidget4/4'),
	new_interp(Interp),
...
:- pred tablewidget4(+title,+header,+table,+footer,+image).

:- doc(tablewidget4(Title,Header,ElementTable,Footer,BackgroundImage),
	"This predicate and @pred{tablewidget4/4} are comparable, except that
...
tablewidget4(Title,Header,ElementTable,Footer,BackgroundImage):-
	valid_table(ElementTable,'tablewidget4/5'),
	new_interp(Interp),
	length(ElementTable,R),
	length_element(ElementTable,Columns),
...
label_row([Element|Es],2,CurrentColumn,Interp):-
	number_codes(2,CR),
	...
	label_row(Es,2,CurrentColumn2,Interp).

...
the ISO standard for that error.
     ...
     Type error:
		...
		type_error(clause,(foo:-4)) instead of type_error(callable,(4)).
	     ...
	     for the predicate open/4 (pag 79 standard), when the Stream is
	     	not a variable:
	     	ISO standard format: error(type_error(variable,Stream),Imp_dep)    
	    	Ciao format: error(type_error(variable),Imp_dep).
		
		For example if the predicate is open(f,write,bar)) the Ciao error would be
		type_error(variable) instead of type_error(variable,bar).

:- module(srcbyrd_tr, [srcdbg_expand/4], [assertions]).
...
srcdbg_expand_(debug, debug, _, _, _, Xs, Xs, _):- !.
srcdbg_expand_(trace, trace, _, _, _, Xs, Xs, _):- !.
srcdbg_expand_(nodebug, nodebug, _, _, _, Xs, Xs, _):- !.
srcdbg_expand_(notrace, notrace, _, _, _, Xs, Xs, _):- !.
srcdbg_expand_(spy(X), debugger_lib:spy(X), _, _, _, Xs, Xs, _):- !,
	displayq(X).
srcdbg_expand_(nospy(X), nospy(X), _, _, _, Xs, Xs, _):- !.
srcdbg_expand_(nospyall, nospyall, _, _, _, Xs, Xs, _):- !.
srcdbg_expand_(this_module(X), this_module(X), _, _, _, Xs, Xs, _):- !.
srcdbg_expand_(debug_module(X), debug_module(X), _, _, _, Xs, Xs, _):- !.
srcdbg_expand_(Goal, srcdbg_byrd(Goal, Pred, Src, L0, L1, Dict, Number), 

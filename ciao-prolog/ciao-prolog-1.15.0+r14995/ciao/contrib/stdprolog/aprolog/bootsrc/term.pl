...
% ----------------
parse_special_number(39, L, N) :- %%%% pts %%%% 0'\' for SWI-Prolog
...
% ----------------
parse_quoted_char([39|L], N) :- !, %%%% pts %%%% 0'\' for SWI-Prolog
	L = [39], %%%% pts %%%% 0'\' for SWI-Prolog
	N = 39. %%%% pts %%%% 0'\' for SWI-Prolog
parse_quoted_char([92|L], N) :- !, %%%% pts %%%% 0'\\ for SWI-Prolog
	L = [C|L1],
	parse_escape(C, L1, N).
...
parse_escape(C, L, N) :-
	digit_based(8, C, N0), !,
	parse_based(L, 8, N0, N, [92]). %%%% pts %%%% 0'\\ for SWI-Prolog
parse_escape(0'x, [C|L], N) :- !,
	digit_based(16, C, N0),
	parse_based(L, 16, N0, N, [92]). %%%% pts %%%% 0'\\ for SWI-Prolog
parse_escape(C, [], N) :-
	escape_equiv(C, N).
...
add_digit_based(B, N0, V, N) :- % special case of 'MIN_INT'
	...
	%%%% pts %%%% ^^^ Dat: added parsens around (-N0) for SWI-Prolog compatibility
...
unify_with_occurs_check(0, _X, _Y) :- !.
unify_with_occurs_check(N, X, Y) :-
	'$arg'(N, X, Xn),
	'$arg'(N, Y, Yn),

%%%% pts %%%% Testing with ciao 1.10-6

% Consult the named files
%:- initialization(ensure_loaded([utils_ciao,iso_test,testbed,test_cases,isosec,format,regexp,show_clause])).
% vvv !! Dat: won't respect `:- op' declarations in iso_test.pl otherwise
:- include(utils_ciao).
:- include(iso_test).
:- include(testbed).
:- include(test_cases).
...
% !! doc: impossible to consult(user).
...
% !! Imp: better, `foo:asserta(bar)' seems to be strage
...
integer_domain(unlimited). % !! although `**' crates real numbers; Imp: test for it
...
abolish_static(Mod:NPerA) :-
	%functor(Head, N, A),
	%'$abolish'(Head).
	abolish(Mod:NPerA). % !! doesn't work
...
load_program_file(Name) :-
	ensure_loaded(Name). % !! was: consult, not present in ciao
	% ^^^ Dat: ensure_loaded(_) is OK, it reloads the file if it has changed since last reload
...
context_info(caret,                def).     % ^/2 is not defined as X^Goal :- Goal.
...
% !! context_info(has_user_error, yes)

...
test_case(call: 6, iso_doc,     call(b(3)),                     type(callable,(write(3),3)),    [prog(call),
										 variant(literally_iso,[ % !! document literally_iso
										   no -txtout(''),
										   yes-txtout('3')])
										]).
                                                                                % ERROR in the std:
...
% 8.6.1.4
% vvv gprolog syntax error !! doc about commented-out
...
test_case(retract: 7, iso_doc,  retract((foo(A) :- A,call(A))), sto(undefined),			[prog(dynpreds11)]).
						...
						% !! thus sto(undefined) makes it always succeed
				%  assert((foo(X):-call(X), call(X))).
				% | ?- retract((foo(_) :- A, call(user:A))).
...
test_case(abolish: 7, eddbali,  (insect(X), abolish(insect/1)), [[X <-- ant],
                                                 [X <-- bee]],                  [prog(abolish)]).
% !! vvv 8..14 makes swi-prolog crash
...
test_case(putbyte:13, pts,   put_byte(user_output, 'ty'), 	type(byte, ty), 		[binout(my_file,[],[])]). % !! Dat: see also putbyte:4, we mustn't report that user_output is a text stream
...
% !! swi-prolog dies somewhere here....
...
% missing: read(X), get_code(C) with input a.%

% 8.14.2.4
test_case(write: 1, iso_doc,    write_term(S, [1,2,3], []),	success,			[txtout('[1,2,3]'),
										 pre(current_output(S))]).
% !! are we allowed to write '.' here? -- or just a dot w/o apostrophe?
...
%%%% pts %%%%^ !! add conditional test cases, add `unsupported' in addition to `skipped'
...
test_case(currop: 4, sics, 	current_op(_, 0, _), 		type(atom,0),                   []). % !! nonstandard?!
...
% vvv !! why are these commented out
...
test_case(curpflg: 2, iso_doc,  current_prolog_flag(F, V), 	{[F<--debug, V<--off],
						 ...
						 [F<--unknown, V<--error],...}, []). % !! YAP: unknown, fail
...
% !! vvv commented out: from Examples in original ISO Prolog std 9.1.7
% !! 9.1.7 Example inconsistent with 7.9.2c in std (corrected by SzeP) !! others
...
% vvv Dat: -2 or -3, depends on the rounding function used
...
% vvv !! ciao: this makes the whole ciao hang, all further tests are skipped
...
% ---------------------------------------------------------------------------
% Macros
% Dat: moved together so SICStus can load it

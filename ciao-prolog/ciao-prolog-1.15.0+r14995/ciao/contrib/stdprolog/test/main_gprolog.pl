...
:- initialization(consult([utils_gprolog,iso_test,testbed,test_cases,isosec,format,regexp,show_clause])).
...
sto_behaviour(plain). % Dat: X=X+1 segfaults
...
is_dynamic(N/A) :-
	predicate_property(N/A, dynamic).
%	functor(Head, N, A), !! fix in other places too

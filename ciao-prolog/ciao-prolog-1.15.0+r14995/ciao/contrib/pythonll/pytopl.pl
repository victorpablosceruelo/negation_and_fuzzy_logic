...
:- doc(title,"Python to Prolog interface").
...
%% REGTYPES
%%------------------------------------------------------------------
:- regtype command(X) # "@var{X} is a command received from the python
...
:- pred prolog_server/0
	# "Prolog server entry point. Reads from the standard input
	  the node name and port number where the python client resides,
...
get_port(Stream,Port):-
	current_input(CU),
	display('get port (plserver)'), display(CU),
	set_input(Stream),
	read(Stream, Port),
	set_input(CU).
...
shell_s :-
	...
	write_answer(0,prolog_exception(pytopl('Read or process failure.'))),
	!,  %% Avoid choicepoints (none should have been pushed--just in case)
	shell_s.
...
process_command(Id,prolog_use_module(Module)) :-
	...
	catch(use_module(Module), Error, write_answer(Id,prolog_exception(0,pytopl(Error)))),
...
process_command(Id,Command) :-
	...
	write_answer(Id,prolog_exception(pytopl('unexpected command', Command ))),
	!.
...
solve2(Query,Id) :-
	...
	python_debug_redo(solve2(Query,Id)),
	Query,
	python_debug_redo(solve2(Query,Id)),
%%
	...
	python_debug_redo('before next_request'(Query)),
	next_request(Id,T), % if next solution requested just fails.
	python_debug_redo('after next_request'(Query)),
	T = terminate,
	retract_fact_nb(running_queries(Id, Query)).

solve2(Query,Id) :-
	python_debug('solve2 failing'),
...
:- pred prolog_parse(+String, -Term)
	:: string * term
		...
		from python side, to parse strings to Prolog terms.".
%---------------------------------------------------------------------------
prolog_parse(S,Term) :-

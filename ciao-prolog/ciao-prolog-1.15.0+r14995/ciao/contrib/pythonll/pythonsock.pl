...
interface, to be used by pythonrt and pytopl. Includes all the code
...
Java. Use @lib{pythonrt} (Prolog to Java interface) or @lib{pytopl} (Java
...
:- use_module(library(pythonll(pytopl))).

:- regtype machine_name(?Name) # "@var{Name} is a valid host name.".
machine_name(X) :- atm(X).

:- pred python_stream(PJStream, JPStream, Address,Stream)
...
%% -----------------------------------------------------------------------
%% -----------------------------------------------------------------------
:- pred python_query(ThreadId,Query)
...
%% -----------------------------------------------------------------------
:- concurrent python_query/2.

%% -----------------------------------------------------------------------
:- pred python_response(Id,Response)
...
:- pred start_socket_interface(+Address,+Stream) 
	:: term * stream
	 ...
	 to connect to the python process, and starts the threads needed
	 to handle the connection.". 
%% -----------------------------------------------------------------------

start_socket_interface(Node:SocketId,Stream):-
	python_debug('------------------------ START SOCKET INTERFACE'(Node:SocketId)),
	 python_client(Node:SocketId,Stream),
	!,
	start_threads.
...
start_threads:-
	...
	set_fact(python_threads(PjIn,PjOut,JpIn,JpOut,PS)).

%% -----------------------------------------------------------------------
:- pred stop_socket_interface # "Closes the sockets to disconnect from 
	 the python process, and waits until the threads that handle the
	 connection terminate.".
%% -----------------------------------------------------------------------
stop_socket_interface :-
	assertz_fact(python_query(0,'$terminate')),
	assertz_fact(prolog_response(0,'$terminate')),
	join_socket_interface,
	retract_fact(python_threads(PjIn,PjOut,JpIn,JpOut,PlServer)),
	eng_release(PjIn),
	eng_release(PjOut),
	eng_release(JpIn),
	eng_release(JpOut),
	eng_release(PlServer),
%	 retract_fact(python_stream(DataStream,EventStream,_,StdStream)),
%	 close(DataStream),
%	 close(EventStream),
	retractall_fact(python_query(_,_)),
	retractall_fact(python_response(_,_)),
...
%% -----------------------------------------------------------------------
join_socket_interface:-
	python_threads(PjIn,PjOut,JpIn,JpOut,PlServer),
	eng_wait(PjIn),
	eng_wait(PjOut),
	eng_wait(JpIn),
	eng_wait(JpOut),
	eng_wait(PlServer),
%	!,
	retract_fact_nb(python_stream(DataStream,EventStream,_,_)),
	 close(DataStream),
	 close(EventStream).
...
:- pred pj_socket_reader/0 # "Predicate that runs in a separate thread
	reading from the prolog-to-python socket. If receives a disconnect
	 or terminate request, just finish the thread.".
%% -----------------------------------------------------------------------
pj_socket_reader :-
	repeat,
	  python_fast_read(pj,pj(Id,R)),
	  python_debug('pj_socket_reader: pj'(Id,R)),
	  (termination_check(Id,R) -> 
	   assertz_fact(python_query(Id,R)),
	   true
	  ;
	   assertz_fact(python_response(Id,R)),
	   fail
	  ),
	  !.
...
:- pred pj_socket_writer/0 # "Predicate that runs in a separate thread
	writing to the prolog-to-python socket. If receives a disconnect
	 or terminate request, just finish the thread.".
%% -----------------------------------------------------------------------
pj_socket_writer :-
	retract_fact(python_query(Id,Q)),
	python_fast_write(pj,pj(Id,Q)),
...
:- pred jp_socket_reader/0 # "Predicate that runs in a separate thread
	reading from the python-to-prolog socket. If receives a disconnect
	 ...
	 from pythonrt in prolog_query/2 predicate.".
%% -----------------------------------------------------------------------
jp_socket_reader :-
	 repeat,
	   python_fast_read(jp,jp(Id,Q)),
	  python_debug('jp_socket_reader: jp'(Id,Q)),
	  (termination_check(Id,Q) -> 
...
:- pred jp_socket_writer/0 # "Predicate that runs in a separate thread
	writing to the python-to-prolog socket. If receives a disconnect
	 or terminate request, just finish the thread.".
%% -----------------------------------------------------------------------
jp_socket_writer :-
	retract_fact(prolog_response(Id,R)),
	python_debug('jp_socket_writer:jp'(Id,R)),
	python_fast_write(jp,jp(Id,R)),
	(termination_check(Id,R) -> 
	 true
	;
	 fail
...
%% -----------------------------------------------------------------------
:- pred python_client(+Address,+Stream) 
	:: term * stream # "Opens a connection at an address, and asserts the 
	 @tt{python_stream} corresponding fact.".
%% -----------------------------------------------------------------------

python_client(Address,Stream) :-
	python_debug('%%%%%%% Address and stream:'(Address, Stream)),
	 open_client(Address,DataStream,EventStream),
	set_fact(python_stream(DataStream,EventStream,Address,Stream)).

%% -----------------------------------------------------------------------
:- pred open_client(+Address, -Stream, -Stream)
	:: term * stream * stream
	 # "Given an address (@tt{Host:Port} format), creates and synchronizes
	  the sockets to the python process.".
%% -----------------------------------------------------------------------

open_client(Host:Port,PJStream,JPStream) :-
	open_pj(Host, Port, PJStream),
	python_debug('PJStream is open'),
	open_jp(Host, Port, JPStream),
	python_debug('JPStream is open').

open_pj(Host, Port, PJStream) :-
	connect_to_socket(Host, Port, PJStream),
	python_fast_write0(PJStream,pj(0,data)),
	python_fast_read0(PJStream, pj(0,data)).

open_jp(Host, Port, JPStream) :-
	connect_to_socket(Host, Port, JPStream),
	python_fast_write0(JPStream,jp(0,event)),
	python_fast_read0(JPStream, jp(0,event)).
...
:- pred bind_socket_interface(+Port) 
	:: int
	 ...
	 the Java side, creates the sockets to connect to the python
...
%% -----------------------------------------------------------------------
:- pred python_server(+Port)
	:: int
	 # "Given a @var{Port}, waits for a connection request from a
	  Java client and synchronizes the sockets to the python
	  process.".
%% -----------------------------------------------------------------------
:- use_module(library(sockets(sockets_io)), [serve_socket/3]).

python_server(Port) :-
	python_debug('inside open_server'),
	set_fact(python_stream(_,_,_,_)),
	 bind_socket(Port,1, Sock),
	eng_call(serve_socket(Sock,binding,sock_error),create,create),
	wait_connection,
	python_debug('after open_server').

binding(Stream):-
	 python_fast_read0(Stream, Term),
	(Term = pj(0,data) ->
		bind_pj(Stream)
		;(Term = jp(0,event) ->
			bind_jp(Stream)
			;
			format(user_error,
			'{ERROR: Socket error accepting connections from python!~n}',[]),
			!,
		fail
	 )).
...
%	 python_fast_read0(PJStream, pj(0,data)),
	python_fast_write0(PJStream,pj(0,data)),
	python_stream(_,JPStream,Address,Stream),
	set_fact(python_stream(PJStream,JPStream,Address,Stream)).


bind_jp(JPStream) :-
%	 python_fast_read0(JPStream, jp(0,event)),
	python_fast_write0(JPStream,jp(0,event)),
	python_stream(PJStream,_,Address,Stream),
	set_fact(python_stream(PJStream,JPStream,Address,Stream)).

sock_error(Error):-
	format(user_error,
	'{ERROR: Socket error while trying to connect to python!~n~w}',[Error]),
	fail.

wait_connection:-
	python_debug('waiting for connection established'),
	python_stream(PJStream,JPStream,_,_),
...
%% -----------------------------------------------------------------------
:- pred python_fast_write(+Type, +Term) 
	...
	reflects the socket (prolog-to-python or python-to-prolog).
	The second argument is the term to be sent to the Java side.".
%% -----------------------------------------------------------------------
python_fast_write(pj,T) :-
	current_fact(python_stream(PJStream,_,_,_)),
	python_debug('Before fast_write0..'),
	python_fast_write0(PJStream,T),
	!.  %% Avoid choicepoints (none should have been pushed---just in case)

python_fast_write(jp,T) :-
	current_fact(python_stream(_,JPStream,_,_)),
	python_fast_write0(JPStream,T),
	!.  %% Avoid choicepoints (none should have been pushed---just in case)

python_fast_write0(Stream,T) :-
	...
	;
	format(user_error,
	'{ERROR: the connection with python has been shut down!}',[]),
	fail
	).

%% -----------------------------------------------------------------------
:- pred python_fast_read(+Type, -Term) :: atom * term # "It reads from the
	...
	(prolog-to-python or python-to-prolog). The second argument is
	unified with the data received from the socket.".
%% -----------------------------------------------------------------------

python_fast_read(pj, T) :-
	current_fact(python_stream(PJStream,_,_,_)),
	python_fast_read0(PJStream,T),
	!.  %% Avoid choicepoints (none should have been pushed---just in case)

python_fast_read(jp, T) :-
	current_fact(python_stream(_,JPStream,_,_)),
	python_fast_read0(JPStream,T),
	!.  %% Avoid choicepoints (none should have been pushed---just in case)

python_fast_read0(Stream,T) :-
	(current_stream(_N,socket,Stream) ->
	current_input(CU),
	set_input(Stream),
	fast_read(T), % It never ends (next line is never printed)
	python_debug('fast read done:'(T)),
	set_input(CU)
	;
	format(user_error,
	'{ERROR: the connection with python has been shut down!}',[]),

...
data(X) :- findall(S,sentence(S),X).

seq(L,true) :- chat_parser_seq(L).
par_nondet(L,true) :- chat_parser_par_nondet(L).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

chat_parser_seq(L) :-
	many_sentences_seq(L).

chat_parser_par_nondet(L) :-
	many_sentences_par_nondet(L).

many_sentences_seq([]) :- !.
many_sentences_seq([H|R]) :- 
	determinate_say(H,_),
	many_sentences_seq(R).

many_sentences_par_nondet([]) :- !.
many_sentences_par_nondet([H|R]) :- 
	determinate_say(H,_) &
	many_sentences_par_nondet(R).

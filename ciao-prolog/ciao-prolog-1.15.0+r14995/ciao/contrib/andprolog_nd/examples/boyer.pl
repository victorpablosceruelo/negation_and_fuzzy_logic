...
data(X) :- size(N), wff(N,X).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

seq(L,true) :- tautology_seq(L).
par(L,true) :- gc(GC), tautology_par_gc(L,GC).
par_nondet(L,true) :- gc(GC), tautology_par_nondet_gc(L,GC).
...
tautology_par_gc(Wff,Gran) :-
	p_rewrite_par_gc(Wff,Gran,NewWff),
	tautology(NewWff,[],[]).

tautology_par_nondet_gc(Wff,Gran) :-
	p_rewrite_par_nondet_gc(Wff,Gran,NewWff),
	tautology(NewWff,[],[]).
...
p_rewrite_par_gc(A,_Gran,A) :-
	A = (A1,_),
	atomic(A1),
	!.
p_rewrite_par_gc(A,Gran,B) :-
	A = (A1,_),
	functor(A1,C,D),
	p_rewrite_args_par(D,Gran,C,A,E),
	'p_rewrite_par/2/2/$disj/1'(B,Gran,_F,E),
	!.

p_rewrite_args_par(D,Gran,C,A,E) :-
	A = (A1,Size),
	p_rewrite_args_par_(D,Gran,Size,A1,L),
	E = ~join(D,C,L).

p_rewrite_args_par_(0,_,_,_,[]).
p_rewrite_args_par_(N,Gran,Size,A,[Arg11|L]) :-
	arg(N,A,Arg),
	N1 is N - 1,
	(
	    Size > Gran ->
            p_rewrite_args_par_(N1,Gran,Size,A,L) '&!'
	    p_rewrite_par_gc(Arg,Gran,Arg1)
	;
            p_rewrite_args_par_(N1,Gran,Size,A,L),
	    p_rewrite_par_gc(Arg,Gran,Arg1)
	),
...
'p_rewrite_par/2/2/$disj/1'(B,Gran,F,E) :-
	equal(E,F),
	p_rewrite_par_gc(F,Gran,B).
'p_rewrite_par/2/2/$disj/1'(B,_Gran,_F,E) :-
	B=E.

p_rewrite_par_nondet_gc(A,_Gran,A) :-
	A = (A1,_),
	atomic(A1),
	!.
p_rewrite_par_nondet_gc(A,Gran,B) :-
	A = (A1,_),
	functor(A1,C,D),
	p_rewrite_args_par_nondet(D,Gran,C,A,E),
	'p_rewrite_par_nondet/2/2/$disj/1'(B,Gran,_F,E),
	!.

p_rewrite_args_par_nondet(D,Gran,C,A,E) :-
	A = (A1,Size),
	p_rewrite_args_par_nondet_(D,Gran,Size,A1,L),
	E = ~join(D,C,L).

p_rewrite_args_par_nondet_(0,_,_,_,[]).
p_rewrite_args_par_nondet_(N,Gran,Size,A,[Arg11|L]) :-
	arg(N,A,Arg),
	N1 is N - 1,
	(
	    Size > Gran ->
            p_rewrite_args_par_nondet_(N1,Gran,Size,A,L) &
	    p_rewrite_par_nondet_gc(Arg,Gran,Arg1)
	;
            p_rewrite_args_par_nondet_(N1,Gran,Size,A,L),
	    p_rewrite_par_nondet_gc(Arg,Gran,Arg1)
	),
...
'p_rewrite_par_nondet/2/2/$disj/1'(B,Gran,F,E) :-
	equal(E,F),
	p_rewrite_par_nondet_gc(F,Gran,B).
'p_rewrite_par_nondet/2/2/$disj/1'(B,_Gran,_F,E) :-
	B=E.

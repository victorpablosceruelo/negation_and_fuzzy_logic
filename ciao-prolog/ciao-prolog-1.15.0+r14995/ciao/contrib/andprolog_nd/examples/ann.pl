...
size(20).
data(X) :- size(N), prepare(N,X).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

seq(Cls,X) :- analyze_all_seq(Cls,X).
par(Cls,X) :- analyze_all_par(Cls,X).
par_nondet(Cls,X) :- analyze_all_par_nondet(Cls,X).
...
analyze_all_par([],[]) :- !.
analyze_all_par([X|Y],[X1|Y1]) :-
	analyze(X,X1) '&!'
	analyze_all_par(Y,Y1).
...
data_([clause(f(X,Z),(p(X,Y),q(Y,Z))),

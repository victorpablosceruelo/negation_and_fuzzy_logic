...
seq(L,X) :- qsort_seq([5500|L],X).
par(L,X) :- size(N), gc(GC), N1 is N + 1, 
	qsort_par_gc([5500|L],N1,GC,X).
par_nondet(L,X) :- size(N), gc(GC), N1 is N + 1, qsort_par_nondet_gc([5500|L],N1,GC,X).

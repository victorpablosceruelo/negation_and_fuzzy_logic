...
%
%! author: Adapted from shared code written by Richard A. O'Keefe.
%! author: Changes by Mats Carlsson, Daniel Cabeza, 
%! author: Manuel Hermenegildo, and Manuel Carro.
...
%! define_flag/3: 
%
%          Defines flags as follows: @includedef{define_flag/3} (See
%          @ref{Changing system behaviour and various flags}).  If
%          flag is @tt{on}, lists which may be written as strings
%          are.
%--------------------------------------------------------------------
...
# "Outputs the term @var{Term} to the stream @var{Stream}, with the
   list of write-options @var{OptList}. See @pred{write_option/1} type
   for default options.".
%--------------------------------------------------------------------
...
# "Behaves like @tt{current_output(S), write_term(S,Term,OptList)}.".
%--------------------------------------------------------------------
...
%! write_option/1: 
% 
%  @var{Opt} is a valid write option which affects the predicate
%  @tt{write_term/3} and similar ones. Possible write_options are:
% 
%  @begin{itemize} 
%  
%  @item @bf{quoted(}@em{bool}@bf{):} If @em{bool} is @tt{true}, atoms
%        and functors that can't be read back by @pred{read_term/3}
%        are quoted, if it is @tt{false}, each atom and functor is
%        written as its name. Default value is @tt{false}.
% 
%  @item @bf{ignore_ops(}@em{flag}@bf{):} If @em{flag} is @tt{true},
%        each compound term is output in functional notation, if it is
%        @tt{ops}, curly bracketed notation and list notation is
%        enabled when outputing compound terms, if it is @tt{false},
%        also operator notation is enabled when outputing compound
%        terms. Default value is @tt{false}.
% 
%  @item @bf{numbervars(}@em{bool}@bf{):} If @em{bool} is @tt{true}, a
%        term of the form @tt{'$VAR'(N)} where @tt{N} is an integer,
%        is output as a variable name consisting of a capital letter
%        possibly followed by an integer, a term of the form
%        @tt{'$VAR'(Atom)} where @tt{Atom} is an atom, as this atom
%        (without quotes), and a term of the form @tt{'$VAR'(String)}
%        where @tt{String} is a character string, as the atom
%        corresponding to this character string.  See predicates
%        @tt{numbervars/3} and @tt{prettyvars/1}.  If @em{bool} is
%        @tt{false} this cases are not treated in any special way.
%        Default value is @tt{false}.
%  
%  @item @bf{portrayed(}@em{bool}@bf{):} If @em{bool} is @tt{true},
%        then call multifile predicates @pred{portray/1} and
%        @pred{portray_attribute/2}, to provide the user handlers for
%        pretty printing some terms.  @tt{portray_attribute/2} is
%        called whenever an attributed variable is to be printed,
%        @tt{portray/1} is called whenever a non-variable term is to
%        be printed.  If either call succeeds, then it is assumed that
%        the term has been output, else it is printed as usual.  If
%        @em{bool} is @tt{false}, these predicates are not
%        called. Default value is @tt{false}.  This option is set by
%        the toplevel when writting the final values of variables, and
%        by the debugging package when writting the goals in the
%        tracing messages.  Thus you can vary the forms of these
%        messages if you wish.
% 
%  @item @bf{max_depth(}@em{depth}@bf{):} @em{depth} is a positive
%        integer or cero. If it is positive, it denotes the depth
%        limit on printing compound terms. If it is cero, there is no
%        limit. Default value is @tt{0} (no limit).
% 
%  @item @bf{priority(}@em{prio}@bf{):} @em{prio} is an integer
%        between 1 and 1200. If the term to be printed has higher
%        priority than @em{prio}, it will be printed parenthesized.
%        Default value is 1200 (no term parenthesized).
% 
% @end{itemize}.
...
:- pred portray_attribute(Attr, Var) : nonvar * var 
# "@em{A user defined predicate.} When an attributed variable
   @var{Var} is about to be printed, this predicate receives the
   variable and its attribute @var{Attr}.  The predicate should either
   print something based on @var{Attr} or @var{Var}, or do nothing and
   fail. In the latter case, the default printer (@pred{write/1}) will
   print the attributed variable like an unbound variable,
   e.g. @tt{_673}.".
...
% 
%   Writes out a Term given Options at nesting depth Depth in a
%   context of priority Priority (that is, expressions with greater
%   priority must be parenthesized), and prefix operators =< PrePrio
%   must be parenthesized, where the last token to be written was of
%   type Ci, and reports that the last token it wrote was of type Co.
...
%   Writes the remaining arguments of a Term with Arity arguments all
%   told in SynStyle, LexStyle, given that DoneSoFar have already been
%   written.
...
# "Outputs the clause @var{Clause} onto @var{Stream}, pretty printing
   its variables and using indentation, including a period at the
   end. This predicate is used by @tt{listing/0}.".
%--------------------------------------------------------------------
...
# "Unifies each of the variables in term @var{Term} with a term of the
   form @tt{'$VAR'(I)} where @tt{I} is an integer from @var{N}
   onwards. @var{M} is unified with the last integer used plus 1. If
   the resulting term is output with a write option
   @tt{numbervars(true)}, in the place of the variables in the
   original term will be printed a variable name consisting of a
   capital letter possibly followed by an integer. When @var{N} is 0
   you will get the variable names A, B, ..., Z, A1, B1, etc.".
%--------------------------------------------------------------------

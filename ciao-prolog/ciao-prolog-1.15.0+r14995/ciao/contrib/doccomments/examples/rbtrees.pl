...
:- use_module(library('dialect/commons')).

:- meta_predicate rb_map(+,2,-), rb_partial_map(+,+,2,-), rb_apply(+,+,2,-).

:- hmtype rbtree(K,V) ---> t(tree(K,V),tree(K,V)).
:- hmtype tree(K,V)   ---> black(tree(K,V),K,V,tree(K,V))

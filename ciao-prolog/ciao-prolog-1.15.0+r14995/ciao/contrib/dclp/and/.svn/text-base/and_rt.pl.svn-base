...
% Ej: No_good=[x(x1, X1,V1), x(x2, X2,V2),...]
	...
	consistent_vars(Cs),
% Es necesario contrastar tambien con el agent_view:
...
process_message(prop_ended, N, K) :-
	...
	;
	 ...
	 send_up_prop_ended(N, RVs),
	 do_begin_labeling(N)).
process_message(enforce_backtrack, N, _S) :-  
% 	store(N, L,_),
% 	current_fact(a(N, A)),
% 	local_backtrack(N, S, L, A, []).
	backtrack(N).
...
add_propagation(A,N,L,K) :-
	...
	display(tmp_a), nl,
% For figures only (without propagation)
%	(unify(L,A) ->
	(true ->
...
change_assignments(N, L, Rs, NewL) :-
	...
	labeling(CVs),
	display(lab(CVs)), nl,
% Este chequeo esta mal. Solo deberian desecharse soluciones parciales 
% (local+agent_view) que esten en la lista de no_goods. Claro que, a lo mejor
% lo que en ese momento era un no_good ahora ya no lo es. Pfff! De todos 
% modos, creo que basta con comprobar que el conjunto local+agent_view no se 
% repite, porque aunque represente un orden total, la combinacion de ellos da
% un orden global. No se, algo hay que hacer con esto porque es especialmente
% perjudicial cuando hay btk largo en el que al final lo que estaba mal era la
% asignacion del ag. de mayor prio. y hay que restaurar las otras asignaciones
...
% Quiza la solucion a lo anterior sea, tal como esta, retraer en este punto
% los hechos de old_assignment(N,_). Asi se permite ejecutar labeling/1 las 
% veces que sea necesarias hasta encontrar una nueva asignacion pero no se 
% hipoteca la recuperacion de soluciones locales en el caso descrito arriba. 
% Alla va:
%% Nota posterior: Esto hacia que se repetieran soluciones parciales no 
%% validas. Lo dejo fuera de momento.
%%	retractall(old_assignment(N, CVs)),
...
clean_frame :-
	current_fact('$frame_is_clean'), !.
clean_frame :-
	retractall(vars_dic(_,_,_,_)), 
	retractall(body_part(_,_)),
	retractall(store(_,_,_)),
	retractall(p_store(_,_,_)),
	retractall(l_store(_,_,_)),
	retractall(source(_)),
	asserta_fact('$frame_is_clean').

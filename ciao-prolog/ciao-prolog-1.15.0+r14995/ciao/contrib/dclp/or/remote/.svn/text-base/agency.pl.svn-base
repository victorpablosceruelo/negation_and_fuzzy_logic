...
%	display(asserted(sh(W, ok))), nl,
	asserta_fact(msg(W, share)).
%	display(asserted(msg(W, share))), nl.
...
add_constraint(V, Point, r_s(W, Vs, Done, Politics, LDS)) :-
	...
	display('<'(V,Point)),nl,
	include_r_s(LDS, W, r_s(W, Vs, ['<'(Point)|Done], Politics, LDS)).
add_constraint(V, Point, r_s(W, Vs, _Done, Politics, _LDS)) :-
	V = Point,
	display('='(V,Point)),nl,
	exhaustive_local_search(W, Vs, Politics).
%	include_r_s(LDS, W, r_s(W, Vs, ['='(Point)|Done], Politics, LDS)).

exhaustive_local_search(W, Vs, Politics) :-
	do_exhaustive_local_search(W, Vs, Politics).

do_exhaustive_local_search(W, Vs, Politics) :-
	labeling(Vs),
	(current_fact_nb(msg(W, stop)) ->
	 process_message(stop, W), !,
	 display(stopped),nl
	;
	 m2manager(collect(Vs, Politics)),
...
do_exhaustive_local_search(_,_,_).

check_share(W) :-
	retract_fact_nb(msg(W, share)),
	retract_fact_nb(sh(W, ok)),
	eng_call(work_to_share(W), create, create).
...
%	collect_nodes(RevAvWork, 1, L), 
	last(AvWork, r_s(W, Work2Share, Done, Politics, LDS)), 
	retract_fact(r_s(W, _Work2Share, Done, Politics, LDS)),
	L = [e(Work2Share, Done, Politics, LDS)],
	m2manager(share(L)).
work_to_share(_) :-

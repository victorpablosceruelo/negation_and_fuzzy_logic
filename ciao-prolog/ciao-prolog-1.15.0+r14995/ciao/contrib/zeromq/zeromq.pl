...
:- comment(summary, "This module provides Ciao Prolog bindings for the
	...
	library.
        ...
        For more information on ZeroMQ,
	...
	API library are the following:
      ...
      @bf{Important notice:} To use these binding, the ZMQ shared
      library, of course, has to be compiled and installed first.  The
      good news is that ZMQ is pretty much self-contained, and does
      not have significant external references.  The bad news is that
      on some architectures like LINUXi86_64, that requires compiling
      ZMQ as 32-bit code (the same as Ciao), using e.g. GCC options
      @tt{-m32} and @tt{-march=i686}.").

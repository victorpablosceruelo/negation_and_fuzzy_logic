...
:- doc(title,"Shared Read/Write Locks").

:- doc(subtitle, "Providing Support for Strict Two-Phase 
...
:- doc(usage,"This module was written for internal use by the
...
:- pred name_arity_to_atom(FA,Pred_Atom): predname * var #
   "Instantiates @var{Pred_Atom} as an atom whose name is the
   concatenation @var{F} + @var{'/'} + @var{A}, supposing @{FA} is
   @{F/A}.  The instantiated atom is what we use as the canonical
   predicate identifier.  This representation was chosen to agree with
   the concept of a @concept{transactional predicate}.".
...
:- pred write_lock(FA,Txn_ID) : predname * nnegint
	#"Attempts to obtain write lock for @var{FA}.  The successful
...
%%if unlocked: take lock and return success
:- pred read_lock(FA,Txn_ID) : predname * nnegint
	#"Attempts to obtain read lock for @var{FA}.  The successful
	  invocation will complete immediately if currently unlocked 
...
:- pred un_lock(FA, Txn_ID) : predname * nnegint

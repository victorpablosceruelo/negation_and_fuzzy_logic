#if defined(TABLING)
void freeze_stacks(Argdecl, node_tr_t *orig_node_tr, node_tr_t *last_node_tr) 
{
  //Updating new values
  ...
  for (ind = Arg->node; 
      ...
      for (; !TrailYounger(ind->trail_top,itrail); itrail--)
	{
	  if (TagIsHVA(CTagToPointer(itrail)))
	    {
	      if (!HeapYounger(ind->global_top,CTagToPointer(itrail)))
		NullifyTrailEntry(itrail);
	    }
	  else if (TagIsSVA(CTagToPointer(itrail)))
	    {
	      if (!StackYounger(ind->local_top,CTagToPointer(itrail)))
		NullifyTrailEntry(itrail);
	    }
	}	
      ind->global_top = (tagged_t *)&(HeapFReg);

...
:- doc(title,"Reification of attributed variables").
:- doc(module,"This library has several predicates for the 
...
:- doc(bug,"The reification predicates don't work with cyclic 
    (rational) input terms.").
:- doc(bug,"Also, they may cause for example
    @tt{clp_dump:dump_constraints/3} to fail. Reason unknown.").

:- doc(copy_extract_attr(X,T,L),"Returns in @var{L} the dictionary
...
@end{verbatim}").
:- true pred copy_extract_attr(X,T,L) => list(L,eq_t).

:- true pred eq_t/1 + regtype.

eq_t(V=T) :- var(V), term(T).
...
:- true pred reinstall_attributes(L) : list(eq_t)
...
:- doc(appendix,"When a term resulting from the reification of 

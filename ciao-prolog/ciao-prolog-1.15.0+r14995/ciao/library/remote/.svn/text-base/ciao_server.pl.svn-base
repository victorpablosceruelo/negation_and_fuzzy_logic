...
serve_connection('$connection'(Socket, Port)) :-
	asserta_fact(is_boundto(Port)),
 	wait_for_commands(Socket).	
...
execute(user_goal(Goal), _Stream, continue):- 
	!,
...
set_connection(Port, '$connection'(Socket, Port)) :-
	do_set_connection(Port, Socket).

...
qsort_par_ndet([X|L], R) :-
	partition(L, X, L1, L2),
	qsort_par_ndet(L2, R2) &
        qsort_par_ndet(L1, R1), 
        append(R1, [X|R2], R).
...
qsort_det_gc([X|L], InputLen, GLev, R) :-
        (
            ...
            N2 is InputLen - N1,
            qsort_det_gc(L1, N1, GLev, R1) '&!'
	    qsort_det_gc(L2, N2, GLev, R2),
            append(R1, [X|R2], R)
        ;
            qsort_seq([X|L], R)
        ).

qsort_nondet_gc([], _InputLen, _GranLevel, []).
qsort_nondet_gc([X|L], InputLen, GLev, R) :-
        (
            InputLen > GLev ->
            partition_gc(L, X, L1, L2, 0, N1),
            N2 is InputLen - N1,
            qsort_nondet_gc(L1, N1, GLev, R1) &
	    qsort_nondet_gc(L2, N2, GLev, R2),
            append(R1, [X|R2], R)
        ;
            qsort_seq([X|L], R)
        ).

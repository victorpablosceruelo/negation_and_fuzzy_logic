...
initial_gv_value( chr_id            , 0 ).
initial_gv_value( chr_global        , _ ).
initial_gv_value( chr_debug         , mutable(off)).  % XXX
initial_gv_value( chr_debug_history , mutable([],0)). % XXX
...
'chr lock'(T) :-
	( var(T)
	-> put_attr(T, chr_runtime, x)
        ;  term_variables(T,L),
           lockv(L)
	).

lockv([]).
lockv([T|R]) :- put_attr( T, chr_runtime, x), lockv(R).

'chr unlock'(T) :-
	( var(T)
	-> del_attr(T, chr_runtime)
	;  term_variables(T,L),
           unlockv(L)
	).

unlockv([]).
unlockv([T|R]) :- del_attr( T, chr_runtime), unlockv(R).

'chr none_locked'( []).
'chr none_locked'( [V|Vs]) :-
	( get_attr(V, chr_runtime, _) ->
		fail
	;
		'chr none_locked'(Vs)
	).
...
%% Ciao begin
:- meta_predicate 'chr allocate_constraint'( goal, ?, ?, ?).
%% Ciao end
...
%% Ciao begin
:- meta_predicate 'chr insert_constraint_internal'(?, ?, goal, ?, ?).
:- meta_predicate insert_constraint_internal(?, ?, ?, goal, ?, ?).
%% Ciao end
...
%% --- DTM: to make chr_messages not to issue warnings.
'chr debug command'( _ , _ ).

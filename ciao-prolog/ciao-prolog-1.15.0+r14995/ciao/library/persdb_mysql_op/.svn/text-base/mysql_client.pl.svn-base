...
:- use_module(library(persdb_mysql_op(db_client_types))).
...
%jcf-begin
:- true pred store_result(in(DbConnection), go(DbQueryConnection)) :: address * address + (foreign(mysql_store_result), returns(DbQueryConnection)).
%jcf-end
%:- use_foreign_source('/home/clip/Systems/ciao/library/persdb_mysql_op/mysql_client').
:- use_foreign_source(library(persdb_mysql_op(mysql_client))).
:- use_foreign_library(mysqlclient).

%% Fixed path to the MySQL client libraries
%% (Automatically generated in makedir/makedir_part_ciao.pl).

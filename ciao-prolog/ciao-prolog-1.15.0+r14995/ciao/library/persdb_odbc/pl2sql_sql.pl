...
@begin{verbatim}
@includeverbatim{examples/pl2sql_example.pl}
@end{verbatim}
...
:- doc(querybody/1,"@var{DBGoal} is a goal meant to be executed in
   ...
   @lib{pl2sql_sql}:@pred{comparison/2}) and whose arguments must be
   ...
   @lib{pl2sql_sql}:@pred{arithmetic_functor/2}).
   ...
   @lib{pl2sql_sql}:@pred{aggregate_functor/2} for the admissible
   aggregate functions.
...
sqlterm2string(SQLQueryTerm,SQLQueryString) :-
   telling(O),
   mktemp_in_tmp('sqlqueryfileXXXXXX',TMP),
   tell(TMP),
   write('"'),

...
protected:
	...
	bool_t Send(CConnectedSocket* pSocket, LPCTSTR message);
	bool_t Receive(CConnectedSocket* pSocket);
	
	virtual void ParseMessage(CConnectedSocket* pSocket, CString Message);

public :
	bool_t Start(UINT port);
	void Stop();
...
// third parameter indicates if a user name is given
	void DoLogon(CString& Params, CString& Reply, bool_t UserGiven);
/**********  21-July-1998 Modification : **********/

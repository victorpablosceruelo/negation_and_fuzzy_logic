...
:- doc(module,"This library provides a socket-based interface to
   @concept{SQL Databases}, using the @concept{database mediator
   server} ODBC interface developed by @author{C. Taboch} and
   @author{Ignacio Caballero}. The interface currently works for databases
   running in @concept{Win95/NT machine}s via @concept{ODBC}. This
   low-level interface was defined with two goals in mind:
   ...
   @bf{Example:} 
@begin{verbatim}
@includeverbatim{examples/db_client_example.pl}
@end{verbatim}
").
...
:- doc(dbhandle/1,"@includedef{dbhandle/1}").
...
:- doc(stmthandle/1,"@includedef{stmthandle/1}").
...
:- doc(bug,"Need to fix the problem with parametric types being

...
:- use_module(library(messages), [debug/2, error/2]).

:- doc(bug,"Need to separate db predicate names by module.").

:- doc(bug,"Warning: using kludgy SQL term to string
   conversion bacause of bugs in sqlterm2string/2.").

:- doc(bug,"Need to fix types to be compatible with ciao, i.e.,
...
@begin{verbatim}
@includeverbatim{../../../persdb_odbc/examples/pl2sql_example.pl}
@end{verbatim}
...
pl2sqlstring(ProjectionTerm,DatabaseGoal,SQLQueryString):-
   ...
   debug("converting term ~w to string", [SQLQueryTerm]),
   sqlterm2string(SQLQueryTerm,SQLQueryString),
   !.

:- type querybody(DBGoal) # "@var{DBGoal} is a database query goal.".
...
:- type projterm(DBProjTerm) # "@var{DBProjTerm} is a database
   projection term.".
...
:- type sqlstring(S) # "@var{S} is a string containing SQL code.".
...
pl2sqlterm(ProjectionTerm,DatabaseGoal,SQLQueryTerm):-
   ...
   debug("compiling ~w :- ~w",[ProjectionTerm,DatabaseGoal]),

   debug("initialize var identifiers and range vars for relations", []),
   init_gensym(variable),
   init_gensym(rel),

   debug("tokenizing projection term and database goal", []),
   tokenize_term(DatabaseGoal,TokenDatabaseGoal),
   tokenize_term(ProjectionTerm,TokenProjectionTerm),

   debug("lexical analysis - reorder goals for disjunctive normal form", []),
   convert_to_disj_norm_form(TokenDatabaseGoal,Disjunction),
   debug("disjunction is ~w",[Disjunction]), %%%% Ignacio
   debug("code generation", []),
   query_generation(Disjunction,TokenProjectionTerm,SQLQueryTerm),
   debug("SQL Query term is ~w",[SQLQueryTerm]),%%%% Ignacio
   debug("pl2sqlterm/3 done", []),
   !.
pl2sqlterm(ProjectionTerm,DatabaseGoal,_SQLQueryTerm) :-
   error("SQL translation failed for ~q / ~q",[ProjectionTerm,DatabaseGoal]),
   fail.
...
check_projection_term_variables(ProjectionTerm,_InitDict) :-
   error("in projection term ~w",[ProjectionTerm]),
   fail.
...
check_translate_conjunction(Conjunction,_SQLFrom,_SQLWhere,_InitDict,_Dict) :-
   error("in conjunction ~w",[Conjunction]),
   fail.
...
check_query_generation(Conjunctions,ProjectionTerm,_Queries) :-
   error("in ~w or ~w",[ProjectionTerm,Conjunctions]),
   fail.
...
# "Where 
   ...
   @ar{Function} is an aggregation function, and

   @ar{ClauseCode} is the code corresponding to the appropriate clause
   of an SQL query, and
...
sqlterm2string(SQLQueryTerm,SQLQueryString) :-
   telling(O),
   get_tmp_dir(TmpDir),
   atom_concat(TmpDir, 'sqlqueryfileIgnacio', TMP),
   tell(TMP),
   write('"'),
...
do_the_read(SQLQueryTerm,I,_SQLQueryString)  :-
   error("could not convert to string ~w",[SQLQueryTerm]),
   seen,
   see(I),
   fail.
...
check_type_union(TypeA,TypeB,_TypeC) :- 
	error("incompatible types ~w, ~w",[TypeA,TypeB]),
	fail.

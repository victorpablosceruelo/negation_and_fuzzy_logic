%% OUTSTANDINGS: First, check types in buildqueries as in printqueries. Begin in build_column / print_column
...
%% Modified from   -rw-rw----   1 ignacio  clip        55054 Jun 25 18:07 pl2sql.pl in persdb_sql.pl


:- use_module(library(persdb_sql(sqltypes)), 
	  [sybase2sqltype/2,sybase2sqltypes_list/2, sybasetype/1,
	   get_type/2, type_union/3, type_compatible/2, accepted_type/2]).
:- reexport(library(persdb_sql(sqltypes)),[sqltype/1]).
...
:- doc(bug,"Warning: using kludgy SQL term to string
   conversion because of bugs in sqlterm2string/2.").
...
@begin{verbatim}
@includeverbatim{persdb_sql/pl2sql_example.pl}
@end{verbatim}
...
query_generation([Conjunction|Conjunctions],ProjectionTerm,[Query|Queries]):-
   ...
   debug_message("Projection term variables checked",[]),
   check_translate_conjunction(Conjunction,SQLFrom,SQLWhere,InitDict,Dict),
   debug_message("Conjunction has been translated",[]),   
   check_translate_projection(ProjectionTerm,Dict,SQLSelect),
   debug_message("Projection Term has been translated",[]),   
...
translate_goal(\+(NegatedGoals),[],SQLNegatedSubquery,Dict,Dict):-
   ...
   debug_message(" Translating negated functor",[]),
...
translate_goal(\+(ComparisonGoal),[],SQLCompOp,Dict,Dict):-
   ...
   debug_message(" Translating negated comparison",[]),
...
translate_goal(ComparisonGoal,[],SQLCompOp,Dict,Dict):-
   ...
   ComparisonGoal =.. [ComparisonOperator,LeftArg,RightArg],
   comparison(ComparisonOperator,SQLOperator),
   debug_message(" Translating comparison",[]),
...
translate_comparison(LeftArg,RightArg,CompOp,Dict,Comparison):-
   ...
   debug_message("Checking compatibility in types",[]),
   check_type_compatible(LeftArgType,RightArgType),
   debug_message("Compatibility in types checked",[]),
...
translate_arguments([Arg|Args],SQLTable,Position,SQLWhere,Dict,NewDict):-
   debug_message("Argument ~w th to translate is ~w",[Position,Arg]),
   translate_argument(Arg,SQLTable,Position,Where,Dict,TmpDict),
   debug_message("Argument ~w th translated",[Position]),
...
translate_argument('$const$'(Constant),rel(SQLTable,RangeVar),Position,
   ...
   attribute(Position,SQLTable,Attribute,Type),
   debug_message("~w stored type is ~w",[Attribute,Type]),
   get_type('$const$'(Constant),ConstType),
   debug_message("Checking compatibility between ~w and ~w",[ConstType,Type]),
...
evaluable_expression(AggregateFunctionTerm,Dictionary,
                     AggregateFunctionExpression,num):-
...
%% NEXT: COMMENTED UNTIL queries_dstring
...
%   seeing(I),
%   see(TMP),
%   do_the_read(SQLQueryTerm,I,SQLQueryString,TMP).

%do_the_read(_SQLQueryTerm,I,SQLQueryString,TMP)  :-
%   read(SQLQueryString),
%   seen,
%   see(I),
%   !,
...
%    ->
%      true
%%%%%%% TO DELETE: uncomment the next line to delete the temporal file
%      %%delete_file(TMP)
%   ;
%      debug_message("File ~w doesn't exist!",TMP)
%   ).

%do_the_read(SQLQueryTerm,I,_SQLQueryString,TMP)  :-
%   error_message("could not convert to string ~w",[SQLQueryTerm]),
%   seen,
%   (  file_exists(TMP)
%    ->

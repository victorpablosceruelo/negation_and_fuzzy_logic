...
:- module(window_class_doc,[],[class,objects,assertions,isomodes,regtypes]).
...
:- doc(title, "The Tcl/Tk Class Interface").

:- doc(author,"Montserrat Iglesias Urraca").
...
:- doc(summary, "This manual includes a complete reference to the
   objects oriented version of the Prolog Tcl/Tk interface.").

:- doc(module,"This library implements an object-orented graphical
   library with a number of predefined objects, using the Prolog
   Tcl/Tk interface. This interface allows creating and destroying
   objects and modifying their properties.  The @lib{window_class}
   contains three clases: widget class, menu class, and canvas
   class. The constructor class is window_class. 

   @bf{Note}: This library (and the documentation) are still under
   development. ").
...
:- doc(widget/1,"Each @var{Widget} type is characterized in two ways: 
...
%%------------------------------------------------------------------------
:- pred show
	# "Adds widgets, menus and canvas to the window object.".
%%------------------------------------------------------------------------
show :-
...
%%------------------------------------------------------------------------
:- pred title(+X) :: string
	# "@var{X} specifies the title for window. The default window title

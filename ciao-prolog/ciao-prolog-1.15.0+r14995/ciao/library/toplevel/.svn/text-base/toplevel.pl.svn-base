...
:- use_module(user,
	    ['$shell_module'/1, '$shell_call'/1]).
:- use_module(library(toplevel(toplevel__scope)), []).
...
interpret_args(['--version'], _) :- !,
	...
	halt.
interpret_args(['-l', File|R], opts(_, DV)) :- !,
...
interpret_args(_WinMesh, _) :-
	using_windows, !, % For windows shortcuts
	include_if_exists('~/.ciaorc').
...
include_st(Type, Stream, Base) :-
	...
	repeat,
	( member(Sentence2, Rest) ; do_read_sentence(Stream, Sentence2) ),

...
process_request(XmlTerm):-
%%%%%%%	<<aqui hay que introducir el codigo que, a partir de XmlTerm, genere XmlResponse>>
	pm_server(Host,Port),
	send_response(XmlResponse,Host,Port).

send_response(XmlResponse,ServerHost,ServerPort):-
	soap_message(SoapStr,XmlResponse),
	http_request(Request,Host,Port,SoapStr),
	fetch_url(Request,[],Response),
%	set_prolog_flag(write_strings,on),
%	writeq(response(Response)), nl,
	true.

%%OJO: esto hay que cambiarlo para que sea el servidor PM.
pm_server('www.clip.dia.fi.upm.es',31202).

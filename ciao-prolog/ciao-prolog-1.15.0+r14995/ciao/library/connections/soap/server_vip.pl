...
process(Message,[Answer,Content]):-
	http_message(Message,SoapStr),
	soap_message(SoapStr,XmlTerm), !,
	http_answer(good,Answer),
	% AQUI PASAR EL XMLTERM A UN FICHERO
	%PROCESAR EL FICHERO
	% Y RECIBIR EL FICHERO QUE CONTIENE LA SALIDA
	% CONSTRUIR EL XMLTERM DEL FICHERO FILE_STRING(FILE,STRING), XML2TERMS(STRING,XMLTERM)2
	soap_message(SoapStr,XmlTerm),   %%%%% Construye el mismo mensaje, pero AQUISERA XMLTERM2 AQUI SERA SOAPSTR2
	Content = content(SoapStr),      %%%%% deberia construir la respuesta.AQUISERA SOPASTR2
% 

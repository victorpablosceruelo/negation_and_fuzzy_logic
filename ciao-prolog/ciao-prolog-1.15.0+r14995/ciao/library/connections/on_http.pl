...
:- doc(close(SideId),"Closes the connection @var{SideId}, either input
...
:- doc(doinclude,service_connection/1).
:- regtype service_connection/1.
:- doc(service_connection/1,"@includedef{service_connection/1}").

service_connection(Host:Port):- atom(Host), number(Port).

:- doc(doinclude,server_id/1).
:- regtype server_id/1.
:- doc(server_id/1,"@includedef{server_id/1}").

...
catch_test(3,_X) :- catch(true, _, 3).
catch_test(4,_X) :- catch(true, _C, ( write(demoen), throw(bla))).
catch_test(5, Y) :- catch(car(_X), Y, true).
catch_test(6,_Y) :- catch(number_chars(_X, ['1','a','0']), error(syntax_error(_),_), true).
catch_test(7, _Y) :- log_nl, log( 'Should now write h1'),
                    log_nl, catch(g7_8, _C, log(h1)), log_nl.
catch_test(8,Y) :-  catch(coo(_X), Y, true).

report_uncaught(B) :- log( 'Uncaught error: '), log( B), log_nl.

catch_tests :-
	test_true(catch_test(1,_Y1)),
	test_true(catch_test(2,_Y2)),
        test_true(catch_test(3,_Y3)),
	test_true(catch_test(4,_Y4)),
        test_true(catch_test(5,_Y5)),
	test_true(catch_test(6,_Y6)),
        test_true(catch_test(7,_Y7)),
	test_true(catch_test(8,_Y8)).

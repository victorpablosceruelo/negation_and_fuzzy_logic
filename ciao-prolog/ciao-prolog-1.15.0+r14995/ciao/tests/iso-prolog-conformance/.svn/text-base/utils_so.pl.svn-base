...
error_test(Goal, ExpectedError) :-
	...
	catch(catch(try(G, Goal), error(E,_Impdef), true), 
              Thrown, 
...
report_difference(Goal, Expected, Actual):-
	log('Goal: '), log(Goal), log(' attempted.'), 
       log_nl,
        ...
        log( error(Expected,_ImpDef)),log_nl,
...
resume_log(LogFile) :-
	open(LogFile, append, _S, [alias(log)]),
        set_output(log).

...
test_errors :-
	...
	error_test((functor(X,foo,_N)), instantiation_error),
	...
	error_test((X=..[_Foo,bar]), instantiation_error),
...
test_arg :-
	...
	test_true((arg(1, foo(X,b), _Y))),
	test_false((arg(1, foo(a,b), b))),
	test_false((arg(0, foo(a,b), foo))),
	test_false((arg(3, foo(3,4), _N))).

test_univ :-

	test_true((foo(a,b) =.. [foo,a,b])),
	test_true((X =.. [foo, a, b])),
	test_true((foo(a,b) =.. _L)),
	test_true((foo(X,b) =.. [foo, a, _Y])),
	test_true((1 =.. [1])),
...
test_copyterm :-
	...
	test_true((copy_term(X+X+Y,_A+B+B))),

...
test_open :-
	...
	test_true((
        ...
        test_true((
                   open('charfile.txt', read, _S3, [type(text), alias(fred)]),
                   get_char(fred, X),
                   close(fred)
                  )).
...
test_open_errors :-
	error_test(open('notafile.txt', read, _A1),
                   existence_error(source_sink, 'notafile.txt')),
        error_test(open('charfile.txt', read/write, _A2),
                   type_error(atom, read/write)),
        error_test(open(_X, append, _A3), instantiation_error),
        error_test(open('charfile.txt', read, _A4, [type(text) | _]),
                   instantiation_error),
        error_test(open('charfile.txt', read, _A5, [type(text), eof(action)]),
...
test_close_errors :-
	error_test((current_output(Stdout),close(Stdout, notalist)),
        ...
        error_test(close(_A), instantiation_error).
...
test_flush_output_errors :-
	error_test(flush_output(_X), instantiation_error),
        error_test(
...
test_stream_property :-
	test_true(stream_property(_S, output)),    % only care that it succeeds
...
test_stream_property_errors :-
	error_test(stream_property(not/stream, _Y), domain_error(stream, not/stream)),
        error_test(stream_property(_S,not_a_stream_property), 
...
test_set_stream_position_errors :-
	...
	open('charfile.txt', read, S1, [reposition(true)]),
        ...
        error_test(set_stream_position(_X, Start), 
                   instantiation_error),
        error_test(set_stream_position(S1, _Any),
                   instantiation_error),
         close(S1)
       )
      ;

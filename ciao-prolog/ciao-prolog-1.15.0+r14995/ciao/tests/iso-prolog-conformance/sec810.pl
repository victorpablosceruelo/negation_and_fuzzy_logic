...
test_findall :-
	...
	error_test(findall(X,_Goal,S), instantiation_error),
...
test_bagof :-
	...
	test_true(bagof(X,(X=Y;X=Z), _S1)),
	test_false(bagof(X,fail,_S2)),
	test_true(bagof(1,(Y=1;Y=2), L)),
	test_true(bagof(f(X,Y), (X=a;Y=b), _L1)),
	test_true(bagof(X, Y^((X=1,Y=1);(X=2;Y=2)), _L2)),
	test_true(bagof(X, Y^((X=1;Y=1);(X=2;Y=2)), _L3)),
	test_true(bagof(X, Y^((X=1;Y=2);X=3), _Si1)),
	test_true(bagof(X, (X=Y;X=Z;Y=1), _S3)),

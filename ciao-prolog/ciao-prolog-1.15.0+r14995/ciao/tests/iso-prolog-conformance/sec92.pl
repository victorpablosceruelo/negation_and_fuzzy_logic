...
test_92 :- 
       ...
       ->
       (
     	error_test(_X52 is +(MI,1),evaluation_error),
     	error_test(_X53 is -(+(MI,1),1),evaluation_error),
     	error_test(_X54 is -(-1,MI),evaluation_error),
     	error_test(_X55 is *(MI,2),evaluation_error),
     	R is float(MI) * 2,
     	error_test(_X56 is floor(R),evaluation_error)
      )
       ;
     true
     ),

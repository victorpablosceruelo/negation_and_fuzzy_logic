...
test_read_write_term :-
	test_true(write_read1(foo(bar, baz), 'termio.tmp', foo(bar, baz))),
        ...
        test_val(write_read_term3(3 + _X, 'termio.tmp', TermIn, 
...
test_read_write_term_errors :-
	error_test(read(_X, _Term1), instantiation_error),
        error_test(read(fred, _Char2), existence_error(stream, fred)),
        error_test(read(nostream/noalias, _Char3), 
        ...
        error_test(read(outputstream, _Y), permission_error(input, stream, outputstream)),
        close(S1), close(S2)
       )
      ;
       true.
...
test_op1 :-
	test_true(rw_op(a, because, b, because(a,b))),
        ...
        test_false((op(0, fx, ++), current_op(_X, fx, ++))).
...
test_op_errors :-
	error_test(op(2000, xfx, a), domain_error(operator_priority, 2000)),
        error_test(current_op(200, afy, _X), domain_error(operator_specifier, afy)).
...
test_char_conversion_errors :-
	error_test(char_conversion(_X,a), instantiation_error).

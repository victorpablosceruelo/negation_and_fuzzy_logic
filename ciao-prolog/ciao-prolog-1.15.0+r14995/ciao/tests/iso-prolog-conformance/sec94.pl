...
test_and :-
   ...
   test_true(_X1 is 15 /\ 12),
   test_val(X2 is 10 /\ 12,X2,8),
   error_test(_X3 is 3 /\ _N ,instantiation_error),
   error_test(_X3a is 3.5 /\ 2, type_error(integer, 3.5)),
   error_test(_X4 is foo/\2 ,type_error(evaluable, foo/0)).
...
test_or :-
   ...
   test_true(_X1 is 10  \/ 12),
   test_val(X2 is 10 \/  12, X2, o14),
   error_test(_X3 is 3 \/ _N ,instantiation_error),
   error_test(_X4 is 5.6\/2 ,type_error(integer, 5.6)),
   error_test(_X5 is foo\/2 ,type_error(evaluable, foo/0)).
...
test_ones_complement :- 
	...
	test_true(_X1 is \10),
        ...
        error_test(_X3 is \ _N , instantiation_error),

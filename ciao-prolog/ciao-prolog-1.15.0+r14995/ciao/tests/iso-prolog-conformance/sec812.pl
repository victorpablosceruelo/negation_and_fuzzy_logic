...
test_get_put_char_errors :-
	error_test(get_char(_X,_Char1), instantiation_error),
        error_test(get_char(fred, _Char2), existence_error(stream, fred)),
        error_test(get_char(nostream/noalias, _Char3), 
        ...
        error_test(get_char(outputstream, _Y), permission_error(input, stream, outputstream)),
        close(S2), close(S1)
       )
      ;
      true
...
test_get_put_code_errors :-
        error_test(get_code(_X,_Code1), instantiation_error),
        error_test(get_code(fred, _Code2), existence_error(stream, fred)),
        error_test(get_code(nostream/noalias, _Code3), 
                     domain_error(stream_or_alias,nostream/noalias)),
        (
           do_catch(open('charfile.txt', read, S1, [alias(inputstream)]),
                    B,
        ...
        open('out.tmp', write, S2, [alias(outputstream)]),
        error_test(get_code(outputstream, _Y), permission_error(input, stream, outputstream)),
        close(S1), close(S2)
       )
       ;
       true
...
test_peek_char_errors :-
	error_test(peek_char(X,_Char1), instantiation_error),
        error_test(peek_char(fred, _Char2), existence_error(stream, fred)),
        error_test(peek_char(nostream/noalias, _Char3), 
                     domain_error(stream_or_alias,nostream/noalias)),
        (
...
test_peek_code_errors :-
	error_test(peek_code(X,_Code1), instantiation_error),
        error_test(peek_code(fred, _Code2), existence_error(stream, fred)),
        error_test(peek_code(nostream/noalias, _Code3), 
                     domain_error(stream_or_alias,nostream/noalias)),
       (
         do_catch( open('peekfile.txt', write, S1, [alias(outputstream)]),
                  B,
       ...
       ->
       (
        error_test(peek_code(outputstream, X), permission_error(input,
	    stream, outputstream)),
        close(S1)
       )
        ; 

...
test_clause  :- 
	...
	test_true(clause(insect(_J), _T)),
	test_false(clause(x, _Body2)).
...
test_clause_errors :-
	defined(clause/2),
	error_test(clause(_,_B), instantiation_error),
	error_test(clause(4,_X), type_error(callable, 4)),
	error_test(clause(elk(_N), Body), 
...
test_current_predicate :-
	...
	test_true(current_predicate(elk/_Arity)),
	test_false(current_predicate(foo8/_A)),
	test_true(current_predicate(_Name/1)),

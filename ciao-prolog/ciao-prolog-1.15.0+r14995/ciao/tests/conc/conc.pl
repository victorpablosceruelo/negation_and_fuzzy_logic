...
main:-
        conc:message("Testing concurrency primitives"),
        conc:message("Creating agents"),
        agent_creation(10),
        conc:message("Waiting for an agent to finish"),
        wait_for_agent,
        conc:message("Waiting for remote failing goal"),
        wait_for_failure,
        conc:message("Asking a remote agent for backtracking"),
        rem_back,
        conc:message("Cutting the execution in a remote agent"),
        rem_cut,
        conc:message("Starting a daemon with a new thread per query"),
        daemon(10),
        conc:message("Waiting for facts"),
        wait_facts,
        conc:message("Two-way communication using the database"),
        two_way,
        conc:message("Add the numbers from 1 to N using two threads"),
        count(2000),
        conc:message("The 5 philosophers"),
        philo_start(100),
        conc:message("Concurrency test finished!"),
	!.

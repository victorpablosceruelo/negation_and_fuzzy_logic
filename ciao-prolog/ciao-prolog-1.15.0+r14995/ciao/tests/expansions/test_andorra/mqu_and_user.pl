...
:- determinate(place(A,B), (A==0;nonvar(A), A ?= s(_), instantiated(B,[1]))).
...
:- determinate(board(A,B,C,D,_E,F,G), 
...
:- determinate(queens_4(_A,_B,_C,_D),true).

...
dump__2(What, Name) :- text_type(What), !,
	atom_concat(['cat ', Name], Cmd), % TODO: requires external tool
	system(Cmd, 0).
...
% Note: it depends on external tools ('otool' in Mac OS X and
%       'objdump' in Linux).
...
archdump(Name) :-
	( get_os('DARWIN') ->
	    atom_concat(['otool -Vt ', Name], Cmd)
	; atom_concat(['objdump -d ', Name], Cmd)
	),
	system(Cmd, 0).

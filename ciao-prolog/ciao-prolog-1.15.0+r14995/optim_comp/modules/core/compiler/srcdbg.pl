...
:- public srcdbg_expand/6.
:- pred srcdbg_expand/6 # "Expansion needed to perform source-level
   debugging.".

srcdbg_expand(Errs, Old_H, Old_B, Old_H, New_B, Dict) :-
	trust(Errs instance_of errlog),
	Errs.get1_loc([], Loc),
	Loc = loc(_, Src, Ln0, Ln1),
	\+ Src = '',
	!,
	% do not expand if no source info is available (i.e. we are expanding
        % code inserted by the compiler)
	search_head(Old_H, Xs, Dict),
	srcdbg_expand__2(Old_B, New_B, Src, Ln0, Ln1, Xs, _, Dict).
srcdbg_expand(_Errs, Old_H, Old_B, Old_H, Old_B, _Dict).

srcdbg_expand__2(','(A,B), ','(NewA, NewB), Src, L0, L1, Xs, Zs, Dict) :- 
	!,
	srcdbg_expand__2(A, NewA, Src, L0, L1, Xs, Ys, Dict),
	srcdbg_expand__2(B, NewB, Src, L0, L1, Ys, Zs, Dict).
srcdbg_expand__2('->'(A,B), '->'(NewA, NewB), Src, L0, L1, Xs, Zs, Dict) :- 
	!,
	srcdbg_expand__2(A, NewA, Src, L0, L1, Xs, Ys, Dict),
	add_pred_to_list('->',Ys,Ks),
	srcdbg_expand__2(B, NewB, Src, L0, L1, Ks, Zs, Dict).
srcdbg_expand__2(';'(A,B), ';'(NewA, NewB), Src, L0, L1, Xs, Zs, Dict) :- 
	!,
	srcdbg_expand__2(A, NewA, Src, L0, L1, Xs, Ys, Dict),
	srcdbg_expand__2(B, NewB, Src, L0, L1, Ys, Zs, Dict).
srcdbg_expand__2(if(A, B, C), if(NewA, NewB, NewC), Src, L0, L1, Xs, Ks, Dict) :- 
	!,
	add_pred_to_list('if',Xs,Ts),
	srcdbg_expand__2(A, NewA, Src, L0, L1, Ts, Ys, Dict),
	srcdbg_expand__2(B, NewB, Src, L0, L1, Ys, Zs, Dict),
	srcdbg_expand__2(C, NewC, Src, L0, L1, Zs, Ks, Dict).
srcdbg_expand__2(\+(A), \+(NewA), Src ,L0, L1, Xs, Ys, Dict) :- 
	!,
	srcdbg_expand__2(A, NewA, Src, L0, L1, Xs, Ys, Dict).
srcdbg_expand__2(!, !, _, _, _, Xs, Xs, _) :- !. 
srcdbg_expand__2(true, true, _, _, _, Xs, Xs, _) :- !. 
srcdbg_expand__2(Goal, srcdbg_spy(Goal, Pred, Src, L0, L1, Number), 
...
% Lists
% TODO: wrong!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! cuts the second clause

% ===========================================================================
% (Generic bytecode compiler for several backends) 
% TODO: MERGE! (currently deactivated)
% ===========================================================================
...
yvarmem(X) :-
	...
	trust(X instance_of termvar),
	~X.getp(mem) = y(_).
ayvarmem(X) :- % TODO: NEW! A or Y register
	nonvar(X),
	trust(X instance_of termvar),
	aymem(~X.getp(mem)).
xvarmem2(X, R) :-
...
aymem(cargmem(_)). % TODO: not sure if it belongs here
%aymem(strmem(_,_)). % TODO: not sure if it belongs here
aymem(a(_)).
aymem(y(_)).

% TODO: extract one index per argument? (and build the index tree on demand)
% TODO: make it a parameter!
idxmem(a(0)).
idxmem(x(0)).

idxvarmem(X) :- % TODO: NEW! A or Y register
	nonvar(X),
	trust(X instance_of termvar),
	idxmem(~X.getp(mem)).
...
:- public set_varmems_d/2.
set_varmems_d([], []).
set_varmems_d([X|Xs], [Mem|Mems]) :-
	Mem = ~my_var_mem(X),
	g = ~my_var_d(X), % TODO: mark as initialized?
	set_varmems_d(Xs, Mems).
...
%
%	cframe(Integer)
%	kall(PredLabel,Integer [,As])
%	lastcall(PredLabel [,As])
%	proceed
%	failins
%	blt1(x(Arg1),Name)
...
%	un_void(I)
%	un_var(I, x(Arg))
%	un_var(I, y(Arg))		% not after first call
%	un_fval(I, y(Arg))	% after first call
%	un_val(I, x(Arg))
%	un_val(I, y(Arg))
%	un_lval(I, x(Arg))
%	un_lval(I, y(Arg))
...
:- if(defined(args_in_call)).
my_call(nonlast, PredId, Size, As) := kall(PredId, Size, As).
my_call(last, PredId, _Size, As) := lastcall(PredId, As).
:- else.
my_call(nonlast, PredId, Size, _As) := kall(PredId, Size).
my_call(last, PredId, _Size, _As) := lastcall(PredId).
:- endif.

% TODO: merge with bytecode__compiler
:- if(defined(args_in_call)).
is_call(kall(_,_,_)).
is_call(lastcall(_,_)).
is_nonlastcall(kall(_,_,_)).
is_lastcall(lastcall(_,_)).
call_predid(kall(PredId,_,_), PredId).
call_predid(lastcall(PredId,_), PredId).
call_size(kall(_,Size,_), Size).
mark_lastcall(kall(PredId,_,As), lastcall(PredId,As)) :- !.
mark_lastcall(X,X) :- X = lastcall(_,_).
:- else.
is_call(kall(_,_)).
is_call(lastcall(_)).
is_nonlastcall(kall(_,_)).
is_lastcall(lastcall(_)).
call_predid(kall(PredId,_), PredId).
call_predid(lastcall(PredId), PredId).
call_size(kall(_,Size), Size).
mark_lastcall(kall(PredId,_), lastcall(PredId)) :- !.
mark_lastcall(X) :- X = lastcall(_).
:- endif.

% ---------------------------------------------------------------------------
% Replace all the variables in the code with new variables that
% contain the 'd' and 'mem' field.
% TODO: wrong name, it does more things
	...
	renamevars_clause_(Args0, Body0, Args, Body).
{
    ...
    renamevars_body(Xs, Ss) :-
	    ...
	    trust(S instance_of strgoal),
            PredId = ~S.predid,
            G = ( is_ipred(PredId) ? icall(S) | pcall(S, _) ),
            goals.add(G)
          )
        ), Xs).
    ...
    renamevars_goal(V, Tran) :-
        ...
        rdic.lookup(~V.name, Tran),
        Tran = ~my_var_new(_,_).
    renamevars_goal(X, S) :- % (special case for $metargs/1)
        trust(X instance_of strgoal),
	~X.get_id == '$metargs/1',
	[MetArgs] = ~X.args,
	!,
        S = ~X.set_args([~renamevars_terms(MetArgs)]).
    renamevars_goal(X, S) :-
        trust(X instance_of strgoal),
        S = ~X.set_args(~renamevars_terms(~X.args)).
}.
...
%	),
	NA = ~G.get_id,
	( NA = 'term_basic:=/2', Args = [A2, B]
	; NA = 'term_basic:=/2', Args = [B, A2]
	),
...
compile_clauses([], _, []).
compile_clauses([C0|Cs], Args0, [C1|Cs2]) :-
	compile_clause(C0, Args0, C1),
	compile_clauses(Cs, Args0, Cs2).
...
compile_clause(Body0000, Args0, clause(Code2, Data)) :-
	renamevars_clause(Args0, Body0000, Args1, Body000),
	extract_metargs(Body000, _MetArgsP, Body00b), % TODO: metargs are ignored
...
extract_type(Gs, Choice, Trivial, Args, Type0, Type, Gs2) :-
	...
	trust(S instance_of strgoal),
	GArgs = ~S.args,
	type_ck(~S.get_id, GArgs, Args, TypeG, Auto),
	!,
	( Trivial = true, Auto = 1 ->
...
mk_occurrences_body(Choice, Args, Gs) :- !,
	...
	bc_record_occurrences(Choice),
	bc_record_occurrences_args(Args).

mk_occurrences_list([]) :- !.
mk_occurrences_list([icall(G)|Gs]) :- !,
	mk_occurrences_list(Gs),
	bc_record_occurrences_goal(G).
...
bc_record_occurrences(X) :-
	trust(X instance_of termstr),
	bc_record_occurrences_args(~X.args).

bc_record_occurrences_args(Xs) :-
	maplist(([u(chn)] -> ''(X) :- bc_record_occurrences(X)), Xs).
}.
...
extract_index([move(X,Y,p)|Code0], Code, Queue, Head, A) :-
	xvarmem(X), xvarmem(Y), eqvarmem(X, Y), !, % TODO: cannot coalesce a(_) and x(_)!
...
duplicatable_insn(un_var(_, _)) :- !.
duplicatable_insn(move(_, _, _)) :- !.
duplicatable_insn(move(_, _)) :- !.
:- if(defined(args_in_call)).
duplicatable_insn(lastcall(_,_)) :- !.
:- else.
...
index_insn(u_cons(X,K), Type, Key) :- idxvarmem(X), !,
	typemask_of_functor(K/0, Type),
	key_of_constant(K, Key).
index_insn(u_blob(X,K), Type, Key) :- idxvarmem(X), !,
	typemask_of_functor(K/0, Type),
	key_of_constant(K, Key).
index_insn(u_str(X,S), Type, S) :- idxvarmem(X), !, % TODO: only pl2wam
	typemask_of_functor(S, Type).
index_insn(u_str_is(X,S,_), Type, S) :- idxvarmem(X), !, % TODO: only for js
	typemask_of_functor(S, Type).
index_insn(u_str_os(X,S,_,_), Type, S) :- idxvarmem(X), !, % TODO: only for js
	typemask_of_functor(S, Type).
...
safe_insn(move(A,B,p)) :-
	xvarmem(A), xvarmem(B), eqvarmem(A, B), !. % TODO: cannot coalesce a(_) and x(_)!
safe_insn(init(A,B)) :-
	xvarmem(A), xvarmem(B), eqvarmem(A, B), !. % TODO: cannot coalesce a(_) and x(_)!
...
key_of_constant(K, K) :- is_string(K), !.
key_of_constant(K, K) :- K instance_of predicate_s, !.
...
% The kind of clause:
%  - unit: no literals
%  - iterative: a lastcall, not requiring a frame
%  - recursive: calls which require frame
kind_of_clause([], unit).
...
:- '$ctxprj'(c_icall/1, [cscope, wcode, u(xdic)]).
c_icall(G) :-
	trust(G instance_of strgoal),
	Args = ~G.args,
	c_icall__2(~G.get_id, ~G.predid, Args).

:- '$ctxprj'(c_icall__2/3, [cscope, wcode, u(xdic)]).
c_icall__2('basiccontrol:$caller_choice/1', _, [X]) :- !,
	( X instance_of termvar -> true ; errlog:bug(['nonvar in caller_choice']), fail ),
	DefaultChoice = ~my_var_new(_, x(-1)),
	movexg(DefaultChoice, X).
c_icall__2('basiccontrol:$cut/1', _, [X]) :- !,
...
c_icall__2(_Name, PredId, Args) :- !,
	Imp = ~ipred__imp(PredId),
	c_icall__3(Imp, PredId, Args).

:- '$ctxprj'(c_icall__3/3, [cscope, wcode, u(xdic)]).
c_icall__3(det, PredId, Args) :- !,
	XV = ~my_var_new(_,x(_)),
	( Args = [X, Value] ->
	    XX = ~my_var_new(_,x(_)),
...
c_icall__3(semidet_re, PredId, Args) :- !, % TODO: this name seems to be wrong; this should be 'detfun_re'?
	XV = ~my_var_new(_,x(_)),
	( Args = [X, Value] ->
	    XX = ~my_var_new(_,x(_)),
	    c_put_arg_builtin(X, XX),
	...
	% TODO: use this or create a new var? g = ~my_var_d(XV),
	c_eq_unify(~my_var_new(g,~my_var_mem(XV)), Value).
c_icall__3(semidet, PredId, Args) :-
	( Args = [X] ->
	    XX = ~my_var_new(_,x(_)),
	    c_put_arg_builtin(X, XX),
...
:- '$ctxprj'(c_eq_instance/2, [cscope, wcode, u(xdic)]).
c_eq_instance(X, V) :-
	Du = ~my_var_d(X),
...
:- '$ctxprj'(c_guards/4, [cscope, wcode, u(xdic)]).
...
:- '$ctxprj'(c_goals/2, [cscope, wcode, u(xdic)]).
c_goals([], 0) :-
...
% Compile a body goal.
:- '$ctxprj'(c_goal/3, [cscope, wcode, u(xdic)]).
c_goal(Position, G, N) :-
...
:- '$ctxprj'(goal_args/5, []).
goal_args(pcall(G, Size), PredId, N, Size, Args) :-
	trust(G instance_of strgoal),
	PredId = ~G.predid,
	Args = ~G.args,
	N = ~psymbol_arity(PredId).

% Head arguments unification.  Match args that are vas first.
% This seems to need fewer temporaries than the source sequence.
:- '$ctxprj'(c_head_args/1, [cscope, wcode, u(xdic)]).
c_head_args(Args) :-
	As = ~headargs(Args),
	head_arguments(Args, As, Arga),
	c_head_args_c(Arga).

:- '$ctxprj'(c_head_args_c/1, [cscope, wcode, u(xdic)]).
c_head_args_c([]).
...
:- '$ctxprj'(head_arguments/3, [cscope, wcode, u(xdic)]).
head_arguments([], _, []).
head_arguments([Arg|Args], [A|As], S) :- D = ~my_var_d(Arg), var(D), !,
	call((
          wcode :: accum <- G0,
	  c_get_arg(Arg, A),
	  G = ~wcode
        )),
	head_arguments(Args, As, S),
	G0 = ~wcode, wcode <- G.
head_arguments([Arg|Args], [A|As], [ha(Arg,A)|S]) :-
	head_arguments(Args, As, S).
}.
...
c_goal_args(Xs, As) :-
	...
	goal_arguments_nonvar(Xs, As),
	% then compile var arguments
	goal_arguments_var(Xs, As).

goal_arguments_nonvar([], []).
goal_arguments_nonvar([Arg|Args], [_|As]) :-
	Arg instance_of termvar,
	!,
	goal_arguments_nonvar(Args, As).
goal_arguments_nonvar([Arg|Args], [A|As]) :-
	c_put_arg(Arg, A),
	goal_arguments_nonvar(Args, As).

goal_arguments_var([], _).
goal_arguments_var([Arg|Args], [A|As]) :-
	Arg instance_of termvar,
	!,
	c_put(Arg, A),
	goal_arguments_var(Args, As).
goal_arguments_var([_|Args], [_|As]) :-
	goal_arguments_var(Args, As).
}.
...
headargs(Args) := ~headargs_(Args, 0).

headargs_([], _) := [].
headargs_([_|Args], I) := [A|As] :-
	A = ~headarg(I),
	I1 is I+1,
	As = ~headargs_(Args, I1).

% Variable for head/call arguments
headarg(I) := ~my_var_new(_,a(I)) :- use_argmems, !.
headarg(I) := ~my_var_new(_,x(I)).
...
:- fluid wcode :: accum.
:- fluid xdic :: xreg_cache + u.
c_unify_args(Xs) :- i :: m_int <- 0, c_unify_args_(Xs).

{
:- fluid i :: m_int.
c_unify_args_([]).
c_unify_args_([X|Xs]) :- c_unify(X), i.inc(1), c_unify_args_(Xs).

c_unify(S) :- S instance_of termvar, !,
	D = ~my_var_d(S),
	( var(D) ->
	    D = g,
	    wcode.add(un_var(~i, S))
	; xdic.ref(S, S2),
	  c_unify__un_val(S2)
	).
c_unify(S) :-
	...
	S.get_functor(K, 0), !,
	( is_blob(K) ->
	    wcode.add(un_blob(~i, K))
	; wcode.add(un_cons(~i, K))
	).
c_unify(S) :- !,
	trust(S instance_of termstr),
	NA = ~S.predid,
	Args = ~S.args,
	wcode.add(un_str(~i, NA)),
	c_unify_args(Args).

c_unify__un_val(X) :-
	( g = ~my_var_d(X) ->
	    wcode.add(un_val(~i, X))
	; % local value
	  wcode.add(un_lval(~i, X))
	).
}.
}.
...
body_lifetime_2(X, _, unit, Heap) :-
	X = lastcall(PredId),
	trust(PredId instance_of predicate_s),
	~PredId.id = 'basiccontrol:true/0',
	!,
	Heap = 0, % TODO: no heap analysis in JS-backend
	live <- [].
...
x_def_use(un_var(_, A), x_d1u0p(V)) :- xvarmem2(A, V), !.
x_def_use(un_val(_, A), x_d0u1(V)) :- xvarmem2(A, V), !.
x_def_use(un_lval(_, A), x_d0u1(V)) :- xvarmem2(A, V), !.
x_def_use(_, x_d0u0).

{
:- fluid cscope :: scope.
% TODO: in JS-backend we do not measure heap
ins_heap(blt1(_,_)) := H :- !, H = 1.
ins_heap(blt2(_,_,_)) := H :- !, H = 1.
ins_heap(blt3(_,_,_,_)) := H :- !, H = 1.
ins_heap(init(A,B)) := H :- xvarmem(A), xvarmem(B), !, H = 1.
ins_heap(globunsafe(_,_)) := H :- !, H = 1.
ins_heap(u_blob(_,_)) := H :- !, H = 1.
ins_heap(u_str(_,_)) := H :- !, H = 1.
ins_heap(ld_blob(_,_)) := H :- !, H = 1.
ins_heap(ld_str(_,_)) := H :- !, H = 1.
ins_heap(un_blob(_, _)) := H :- !, H = 1.
ins_heap(un_str(_, _)) := H :- !, H = 1.
ins_heap(_) := 0.
}.
	...
	peep_xs(Code0).
{
    ...
    peep_1(X) :- is_nonlastcall(X), !, ensure_call(~call_size(X)), wcode.add(X).
    ...
    peep_1(X) :-
        is_lastcall(X), PredId = ~call_predid(X),
	trust(PredId instance_of predicate_s),
	~PredId.id = 'basiccontrol:true/0',
        ~env = noenv, !, wcode.add(proceed).
     % TODO: why fail? true/0 is necessary to catch heap overflows (see the tests/heap_proceed test) but fail/0 
    peep_1(X) :-
        is_lastcall(X), PredId = ~call_predid(X),
	trust(PredId instance_of predicate_s),
	~PredId.id = 'basiccontrol:fail/0',
	~env = noenv, !, wcode.add(failins).
    peep_1(X) :- is_lastcall(X), !, ensure_no_env, wcode.add(X).
    ...
    peep_1(un_var(I, A)) :- xvarmem2(A, void), !, wcode.add(un_void(I)).
    peep_1(un_var(I, X)) :- ~env = env(_), yvarmem2(X, B), !, wcode.add(un_fval(I, y(B))).
    peep_1(un_var(I, X)) :- yvarmem2(X, B), !, ensure_env(B), wcode.add(un_var(I, X)).
    ...
    c_ensure_space(_, _, _). % TODO: disabled in JS-backend
    ...
    ensure_call_(noenv, env2(_,_), _Size) :-
            use_opt(uninit_y), % allow frames with uninitialized Y entries
            !,
            wcode.add(alloc),
            wcode.add(init([])).
...
:- if(defined(args_in_call)).
precollapse_init_([alloc, init(_), kall(L,S,As)|Xs0], [alloc_init_fcall(L,S,As)|Xs]) :- !,
	precollapse_init_(Xs0, Xs).
precollapse_init_([init([]), kall(L,S,As)|Xs0], [fcall(L,S,As)|Xs]) :- !,
	precollapse_init_(Xs0, Xs).
precollapse_init_([init(Ys), kall(L,S,As)|Xs0], [init(Ys), fcall(L,S,As)|Xs]) :- !,
	precollapse_init_(Xs0, Xs).
:- else.
...
move_alloc_([X, alloc, Y|Xs0], [alloc, X, Y|Xs]) :-
	Y = un_var(_, A), yvarmem(A), move_alloc__2(X), !,
	move_alloc_(Xs0, Xs).
...
move_alloc__2(un_var(_, A)) :- xvarmem(A), !.
move_alloc__2(un_val(_, A)) :- xvarmem(A), !.
move_alloc__2(un_lval(_, A)) :- xvarmem(A), !.

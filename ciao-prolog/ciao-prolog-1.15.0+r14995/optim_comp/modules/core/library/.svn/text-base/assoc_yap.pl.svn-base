...
% (copied from the SWI port of the YAP library).
...
assoc_to_list(Assoc, List) :-
	assoc_to_list_(Assoc, List, []).

assoc_to_list_(t(Key,Val,_,L,R), List, Rest) :-
	assoc_to_list_(L, List, [Key-Val|More]),
	assoc_to_list_(R, More, Rest).
assoc_to_list_(t, List, List).
...
assoc_to_keys(Assoc, List) :-
	assoc_to_keys_(Assoc, List, []).

assoc_to_keys_(t(Key,_,_,L,R), List, Rest) :-
	assoc_to_keys_(L, List, [Key|More]),
	assoc_to_keys_(R, More, Rest).
assoc_to_keys_(t, List, List).
...
assoc_to_values(Assoc, List) :-
	assoc_to_values_(Assoc, List, []).

assoc_to_values_(t(_,Value,_,L,R), List, Rest) :-
	assoc_to_values_(L, List, [Value|More]),
	assoc_to_values_(R, More, Rest).
assoc_to_values_(t, List, List).
...
get_assoc(Key, t(K,V,_,L,R), Val) :-
	compare(Rel, Key, K),
	get_assoc_(Rel, Key, V, L, R, Val).

get_assoc_(=, _, Val, _, _, Val).
get_assoc_(<, Key, _, Tree, _, Val) :-
	get_assoc(Key, Tree, Val).
get_assoc_(>, Key, _, _, Tree, Val) :-
	get_assoc(Key, Tree, Val).
...
get_assoc(Key, t(K,V,B,L,R), Val, t(K,NV,B,NL,NR), NVal) :-
	compare(Rel, Key, K),
	get_assoc__(Rel, Key, V, L, R, Val, NV, NL, NR, NVal).

get_assoc__(=, _, Val, L, R, Val, NVal, L, R, NVal).
get_assoc__(<, Key, V, L, R, Val, V, NL, R, NVal) :-
	get_assoc(Key, L, Val, NL, NVal).
get_assoc__(>, Key, V, L, R, Val, V, L, NR, NVal) :-
...
list_to_assoc(List, Assoc) :-
	list_to_assoc_(List, t, Assoc).

list_to_assoc_([], Assoc, Assoc).
list_to_assoc_([Key-Val|List], Assoc0, Assoc) :-
	put_assoc(Key, Assoc0, Val, AssocI),
	list_to_assoc_(List, AssocI, Assoc).
...
map_assoc(Pred, T0, T) :-
	map_assoc__(T0, Pred, T).

map_assoc__(t, _, t).
map_assoc__(t(Key,Val,B,L0,R0), Pred, t(Key,Ans,B,L1,R1)) :-
	map_assoc__(L0, Pred, L1),
	'$trust_metatype'(Pred, pred(2)),
	Pred(Val, Ans),
	map_assoc__(R0, Pred, R1).
...
max_assoc(t(K,V,_,_,R), Key, Val) :-
	max_assoc_(R, K, V, Key, Val).

max_assoc_(t, K, V, K, V).
max_assoc_(t(K,V,_,_,R), _, _, Key, Val) :-
	max_assoc_(R, K, V, Key, Val).
...
min_assoc(t(K,V,_,L,_), Key, Val) :-
	min_assoc_(L, K, V, Key, Val).

min_assoc_(t, K, V, K, V).
min_assoc_(t(K,V,_,L,_), _, _, Key, Val) :-
	min_assoc_(L, K, V, Key, Val).
...
insert(t(Key,Val,B,L,R), K, V, NewTree, WhatHasChanged) :-
	compare(Rel, K, Key),
	insert_(Rel, t(Key,Val,B,L,R), K, V, NewTree, WhatHasChanged).

insert_(=, t(Key,_,B,L,R), _, V, t(Key,V,B,L,R), no).
insert_(<, t(Key,Val,B,L,R), K, V, NewTree, WhatHasChanged) :-
...
del_min_assoc(Tree, Key, Val, NewTree) :-
	del_min_assoc_(Tree, Key, Val, NewTree, _DepthChanged).

del_min_assoc_(t, _, _, t,no).
del_min_assoc_(t(Key,Val,_B,t,R), Key, Val, R, yes) :- !.
del_min_assoc_(t(K,V,B,L,R), Key, Val, NewTree, Changed) :-
	del_min_assoc_(L, Key, Val, NewL, LeftChanged),
...
del_max_assoc(Tree, Key, Val, NewTree) :-
	del_max_assoc_(Tree, Key, Val, NewTree, _DepthChanged).

del_max_assoc_(t, _, _, t,no).
del_max_assoc_(t(Key,Val,_B,L,t), Key, Val, L, yes) :- !.
del_max_assoc_(t(K,V,B,L,R), Key, Val, NewTree, Changed) :-
	del_max_assoc_(R, Key, Val, NewR, RightChanged),
...
delete(t(Key,Val,B,L,R), K, V, NewTree, WhatHasChanged) :-
	compare(Rel, K, Key),
	delete_(Rel, t(Key,Val,B,L,R), K, V, NewTree, WhatHasChanged).

% delete_(+KeySide, +Subtree, +SearchedKey, ?SearchedValue, ?SubtreeOut, ?WhatHasChanged)
...
delete_(=, t(Key,Val,_B,t,R), Key, Val, R, yes) :- !.
delete_(=, t(Key,Val,_B,L,t), Key, Val, L, yes) :- !.
delete_(=, t(Key,Val,>,L,R), Key, Val, NewTree, WhatHasChanged) :-
	% Rh tree is deeper, so rotate from R to L
	del_min_assoc_(R, K, V, NewR, RightHasChanged),
	deladjust(RightHasChanged, t(K,V,>,L,NewR), right, NewTree, WhatHasChanged), !.
delete_(=, t(Key,Val,B,L,R), Key, Val, NewTree, WhatHasChanged) :-
	% Rh tree is not deeper, so rotate from L to R
	del_max_assoc_(L, K, V, NewL, LeftHasChanged),
	deladjust(LeftHasChanged, t(K,V,B,NewL,R), left, NewTree, WhatHasChanged), !.

delete_(<, t(Key,Val,B,L,R), K, V, NewTree, WhatHasChanged) :-

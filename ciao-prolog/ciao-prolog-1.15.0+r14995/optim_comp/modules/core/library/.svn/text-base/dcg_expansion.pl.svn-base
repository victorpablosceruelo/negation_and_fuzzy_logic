...
:- doc(title,"Definite clause grammars (expansion)").

:- doc(author, "The CLIP Group").

:- doc(module, "This module implements the Definite clause
   grammars (expansion).").
...
:- meta_predicate phrase(primitive(goal),?).
phrase(_, _) :-
	errlog:bug(['fix phrase/2']).
:- use_module(compiler(errlog), [bug/1]).	
%phrase(P, _) :-
%	var(P), !,
...
dcg_translate_dcg(X, Y, S, S0, S) :- var(X), !,
        '$module'(M),
	Y=M:phrase(X,S0,S),
        add_ensure_imported_check(M, phrase, 3).

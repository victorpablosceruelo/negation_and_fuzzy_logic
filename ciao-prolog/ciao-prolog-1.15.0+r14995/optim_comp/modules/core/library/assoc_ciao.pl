...
:- doc(module,"This library implements a table.  It takes its name
          ...
          instanciated later (so, this implementation unify with '=='
...
:- pred min_assoc(+Assoc, -Key, -Value) : 
     (assoc_table(Assoc), key(K), value(Value))
...
map_assoc(Pred,Assoc,AssocNew) :- 
	assoc_ciao:map(Assoc, (_(_,Old,New) :- '$trust_metatype'(Pred, pred(2)), Pred(Old,New)),AssocNew).
...
map(assoc_table(Assoc,N,Type),Pred,assoc_table(Result,N,Type)) :- 
	...
	;
	    hiordlib:map(Assoc,(_(K-V,K-R) :- '$trust_metatype'(Pred, pred(3)), Pred(K,V,R)),Result)
	).

map_avl(nil,_,nil).
map_avl(avl(K,Old_Val,Old_L,Old_R,H),Pred,avl(K,New_Val,New_L,New_R,H)) :- 
	map_avl(Old_L,Pred,New_L),
	'$trust_metatype'(Pred, pred(3)),
...
foldl(assoc_table(Assoc,_,Type),DS,Pred,NDS) :- 
	...
	;
	    hiordlib:foldl(Assoc,DS,(_(K-V,DS0,NDS0) :- '$trust_metatype'(Pred, pred(4)), Pred(K,V,DS0,NDS0)),NDS)
	).

foldl_avl(nil,DS,_,DS).
foldl_avl(avl(K,V,L,R,_),DS,Pred,NDS) :- 
	foldl_avl(L,DS,Pred,DS0),
	'$trust_metatype'(Pred, pred(4)),
	Pred(K,V,DS0,DS1),

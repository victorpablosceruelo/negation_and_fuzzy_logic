...
%
% Note: This code does not throw syntax error exceptions. Syntactic
% errors are reported by creating special tokens, which are recognized
% by library(read).
...
:- module(tokenize, [read_tokens/2,token/1,resume_read_tokens/3],[assertions]).
...
% :- pred define_flag(Flag,FlagValues,Default) 
...
read_tokens_solo(0'}, Dict, Level0, ['}'|Tokens]) :-
	...
	Level is Level0 - 1,
        read_tokens(NextTyp, NextCh, Dict, Level, Tokens).
read_tokens_solo(0'", Dict, Level, [string(S)|Tokens]) :-  % "string"
        getct(Ch, Typ),
        read_string(Typ, Ch, S, 0'", NextTyp, NextCh),
        read_tokens(NextTyp, NextCh, Dict, Level, Tokens).
read_tokens_solo(0'', Dict, Level, [Atom|Tokens]) :- % 'atom'
        getct(Ch, Typ),
        read_string(Typ, Ch, S, 0'', NextTyp, NextCh),
        atom_token(S, Atom),
        read_tokens(NextTyp, NextCh, Dict, Level, Tokens).
...
%            read_tokens(0, Ch, Dict, Level, Tokens)
	    % Stop clause reading but provide a suspension
...
based_int_or_quoted([], Base, Base, Typ, Ch, Dict, Level, [Atom|Tokens]) :- !,

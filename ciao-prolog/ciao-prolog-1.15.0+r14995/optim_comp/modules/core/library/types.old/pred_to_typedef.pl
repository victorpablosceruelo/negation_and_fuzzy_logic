...
:- pred escaped_type(+Term, -EscType)

# "Rewrites the term @var{Term} as a type term (by escaping
  functors. Variables are not rewritten.".
...
one_literal_to_type(regtype(Var, Type)):- !, 
  Var = Type.
one_literal_to_type(Lit):-
  Lit =.. [Name, Var|Param],
  Var =.. [Name|Param].
...
regular_type_literal_definition_1(Name, Arity, Lit, Clause,
                 TermVarsSeen, TermVars, ParametricVars, [Arg1|TermVarsSeen], Error):-

...
:- meta_predicate srcdbg_byrd(primitive(goal), _, _, _, _, _).
...
debuggable(G, debug, Pred, Src, _, Ln1, Number) :-
	( '$spypoint'(G, on, on)
...
srcdbg_byrd2(Goal, State, Pred, Src, L0, L1, Number) :-
...
call_hook1(Goal) :-
	...
	( '$predicate_property'(Name/Arity, _, _) ->
	     '$notrace_call'(Goal)

...
:- use_module(compiler(action__expand), [callback__loc/3]).
...
srcdbg_expand1(clause(0, 0), clause(0, 0), _, _) :- !. % TODO: why is it needed now?
srcdbg_expand1(clause(Head, Old_B), clause(Head, New_B), Expand, Dict) :-
	location(Src, L0, L1),
	search_head(Head, Xs, Dict),
	srcdbg_expand_(Old_B, New_B, Src, L0, L1, Xs, _Zs, Expand, Dict).

location(Src, Ln0, Ln1) :-
	action__expand:callback__loc(Src, Ln0, Ln1),
	% code not inserted by the compiler
	\+ Src = ''. 

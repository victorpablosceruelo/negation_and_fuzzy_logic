...
:- doc(author,"(A. Ciepielewski, M. Carlsson, T. Chikayama, K. Shen)").
...
Ciao by D. Cabeza and included in the Ciao version control system. Extended
for source-level debugging by Manuel C.  Rodr@'{i}guez.  (See changelog if
included in the document for more detailed documentation of the later
changes.)

").

%% The following changes has been done to documentation in order to fit the

% Signal processing using (a Prolog version of) updatable arrays to store the data
...
newA(A, I, MU, Yhat, N, M, [X|NA], Emg) :-
        ...
        ;
%            N_1 is N + 1,
            access_array(Emg, [N], YN),
%            emg(N,YN),
            Ni is N-I,
%            Ni_1 is Ni + 1,
            access_array(Emg, [Ni], YNi),
%            aref(Ni, Emg, YNi),
%            emg(Ni, YNi),
...
myreverse([], R, R) :- !.
myreverse([X|XS], R, Rev) :-
	myreverse(XS, [X|R], Rev).

%:- use_module(library(write)).

testLearning(Iterations) :-
        MU is 0.01,
        SampleRate = 0.0001,
        NSamples = 10000,
...
%        statistics(runtime, [_,T1]),
        ...
        getFeatures(NFeatures, NSamples, MU, _Features, Estimation, Iterations,  Emg),
%        statistics(runtime, [_,T2]),
        myreverse(Estimation, [], _Ans).
%          write('Features= '), write(_Features), nl,
%          write('Estimation= '), write_some(20, _Ans), nl.
%        display('Wave building (arrays): '),
%        display(T1),
%        display(' milliseconds'),
%        nl,
%        display('Wave processing (arrays): '),
%        display(T2),
%        display(' milliseconds'),
%        nl.

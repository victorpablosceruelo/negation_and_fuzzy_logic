% Signal processing using dynamic facts to store the data
...
signal(sawtooth, NSamples) :- 
	...
	retractall_fact(nSamples(_)),
	assertz_fact(nSamples(NSamples)),
	sampleRate(Step),
	sawtooth(0, 1.0, Step).
...
getFeatures(NFeatures, MU,  Features, Estimation, Iterations) :-
	...
	getFeatures_(0, A, MU, Features, Estimation, Iterations). 
...
%        display('Wave building (original): '),
%        display(T1),
%        display(' milliseconds'),
%        nl,
%        display('Wave processing (original): '),
%        display(T2),
%        display(' milliseconds'),
%        nl.

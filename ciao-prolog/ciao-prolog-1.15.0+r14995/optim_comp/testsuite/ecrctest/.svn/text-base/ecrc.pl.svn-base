...
%% fixed and automatized for industrial SICStus
%% by Thomas Sjöland SICS 91 07 18

%% Fixed for Ciao Prolog by MCL.  Main chacompare_filesnges:
...
:- doc(author, "M. Carro (for Ciao Prolog) and others").

:- doc(copyright, "M. Carro, the Clip Lab, and the original authors").
...
SICS.  They were adapted to Ciao Prolog by M. Carro, by:
...
@subsection{Testing non-deterministic behavior}
...
@subsection{Testing environment handling}
...
@subsection{Testing indexing mechanisms}
...
@subsection{Testing unification}
...
@subsection{Testing dereferencing}
...
@subsection{Assorted small programs}
...
Date:             ~w~n", 
        ...
        fail.
generate_human_file:-
        findall(T, get_timings(_, _, _, _, T), AllTimings),
...
arithm_average(List, AAVG):- 
        foldl(List, (0, 0), 
             (_(X, (LI, SI), (LO, SO)) :- LO is LI+1, SO is X + SI), (Len,Sum)),
        AAVG is Sum / Len.

% JF: introduced factor so that floats do not overflow
geom_average(List, GAVG):-
	Factor = 100.0, % scale factor to avoid exponent overflow in float
        foldl(List, (0, 1), 
             (_(X, (LI, PI), (LO, PO)) :- LO is LI+1, PO is X * PI / Factor), (Len,Prod)),
        GAVG is exp((log(Prod)+Len*log(Factor))/Len).

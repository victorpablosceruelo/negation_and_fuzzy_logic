...
common_loop(_Reps, N, Goal, LIs, _Name):-               %% Heat up the cache
        repeated_call_failing(1, N, Goal, LIs, _Name, no).
common_loop(Reps, N, Goal, LIs, Name):-               %% Do actual benchmarking
        repeated_call_failing(Reps, N, Goal, LIs, Name, yes).
common_loop(_,_,_,_,_).                           %% Exit with success


%% Perform N repetitions of some goal.
:- meta_predicate repeated_call_failing(?, ?, goal, ?, ?, ?).
repeated_call_failing(N, Reps, Call, LIs, Id, Save) :-
        repeat_loop(N),
        once_run_bench(Reps, Call, LIs, Id, Save),
        fail.

:- meta_predicate once_run_bench(?, goal, ?, ?, ?).
once_run_bench(Reps, Call, LIs, Id, Save) :-
	run_bench(Reps, Call, LIs, Id, Save),
	!.

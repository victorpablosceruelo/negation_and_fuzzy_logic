:- module(boyer, [], [benchmark]).

% TODO: see conditional code below (it should not be there)

% Program: Boyer (toy theorem prover)
% Author:  E. Tick (after Lisp by R. Boyer)
% Date:    November 12 1985

name("Boyer (toy theorem prover)").
repeat_count(1).
solve(R) :- wff(X), ( tautology(X) -> R = proved(X) ; R = not_proved(X) ).
...
% TODO: implement missing parts, merge
...
arg(A,B,C) :- C = '~'('\6\dot'(B, '$arg'(A))). % C = ~B.'$arg'(A).
% TODO: implemented functor/3
% TODO: move to the library
atomic(A) :- '~'('\6\dot'(A, '$arity')) = 0. % ~A.'$arity' = 0.

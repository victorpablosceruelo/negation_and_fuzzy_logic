:- class(queens_solution_widget, [], [oo_syntax, hiord, assertions, fsyntax, mutables, string_type]).

:- doc(title, "A widget to display solutions of Queens").

:- doc(bug, "Merge with solution_widget").
...
sols_step := 1. % number of solutions in each step
  ...
  <tr>\
    ...
    <td align=\"right\" valign=\"top\">\
      <div id=\"time" + ~id + "\" class=\"timerbox\"></div>\
        ...
        % Other solution visualizer
	customsol_init(Div),
        % The box for solutions (display)
  	SolBox = ~js_dom.createElement("div"),
        SolBox.id <- "sol" + ~id,
        SolBox.className <- "solbox",
        SolBox.style.display <- "none",
	Div.append_child(SolBox),
	%
...
% ===========================================================================
% Custom solution visualization
% TODO: Ad-hoc for Queens8, generalize

table_size(8).

customsol_init(Base) :-
	create_table(~table_size, Base).

%create_table(Base) :- fail, !,
%	Td = ~createElement("span"),
%	Td.className <- "chesspiece",
%	Td.innerHTML <- "&#9819;",
%	Base.append_child(Td).
create_table(N, Base) :-
...
%	Table.cellpadding <- "4",
	...
	%
	create_table_(1, N, TBody),
	Base.append_child(Table).

% rows
create_table_(I, N, _) :- I > N, !.
create_table_(I, N, Base) :-
	Tr = ~createElement("tr"),
	Base.append_child(Tr),
	create_table__(1, N, I, Tr),
	I1 is I + 1,
	create_table_(I1, N, Base).

% columns
create_table__(I, N, _, _) :- I > N, !.
create_table__(I, N, J, Base) :-
	% foo.removeChild(fieldNameElement.firstChild);
...
%	Td.width <- "64",
	Td.id <- ~id + "-" + I + "-" + J,
	( Z is (I + J) mod 2, Z = 0 ->
	    Td.className <- "chesspiece1"
	; Td.className <- "chesspiece2"
	),
	Td.innerHTML <- "&nbsp",
	%
	Base.append_child(Td),
	%
	I1 is I + 1,
	create_table__(I1, N, J, Base).

% Clear table
customsol_clear :-
	customsol_clear_(1, ~table_size).

% rows
customsol_clear_(I, N) :- I > N, !.
customsol_clear_(I, N) :-
	customsol_clear__(1, N, I),
	I1 is I + 1,
	customsol_clear_(I1, N).

% columns
customsol_clear__(I, N, _) :- I > N, !.
customsol_clear__(I, N, J) :-
	Td = ~elem(~id + "-" + I + "-" + J),
	Td.innerHTML <- "&nbsp",
	I1 is I + 1,
	customsol_clear__(I1, N, J).

% Display solutions
customsol_display(Xs) :-
	customsol_display_(1, ~table_size, Xs).

% rows
customsol_display_(I, N, _) :- I > N, !.
customsol_display_(I, N, [X|Xs]) :-
	customsol_display__(1, N, I, X),
	I1 is I + 1,
	customsol_display_(I1, N, Xs).

% columns
customsol_display__(I, N, _, _) :- I > N, !.
customsol_display__(I, N, J, X) :-
	Td = ~elem(~id + "-" + I + "-" + J),
	( X = I ->
	    Td.innerHTML <- "&#9819;"
	; Td.innerHTML <- "&nbsp"

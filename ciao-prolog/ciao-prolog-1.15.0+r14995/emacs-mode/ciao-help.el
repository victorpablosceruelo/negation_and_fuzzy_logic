...
(defun ciao-help-existing-manuals (bundlekeys)
  "Add the (existing) manuals for the given `bundlekeys'."
  (let (manuals)
    (dolist (key bundlekeys)
      (let
	  ((inf (ciao-help-info-entry (ciao-get-config key))))
...
;; (defun ciao-goto-ciaosys-manual () 
;;   "Go to the part of the info directory containing the Ciao
;; system manual."
;;   (interactive) 
;;   (ciao-goto-particular-manual (concat (ciao-get-config :ciaosys-name-version) ":")))
;; 
